...
func TestBulkBuildersChargeTheStepBudget(t *testing.T) {
	for _, tc := range []struct{ name, src string }{
		...
		{"math.floor of a long numeral",
			`local s=string.rep("9",100000) for i=1,60000 do math.floor(s) end return 1`},
		{"format %d of a long numeral",
			`local s=string.rep("9",100000) for i=1,60000 do string.format("%d",s) end return 1`},
...
func TestBulkBuildersLeaveOrdinaryCodeAlone(t *testing.T) {
	for _, tc := range []struct{ name, src, want string }{
		...
		// The idiomatic advancing scan must NOT be rejected: each call stops at the first match,
		// so the real total is O(n). Charging the whole subject, or even the whole remainder,
		// billed O(n^2) and rejected a 32 KiB subject that lua5.1 handles at 256 KiB in 2ms.
		{"advancing scan loop",
