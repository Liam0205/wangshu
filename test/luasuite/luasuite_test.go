...
var stopAt = map[string]int{
	...
	"sort.lua":   0,
	"pm.lua":     0,
	// code.lua asserts on generated bytecode (constant folding, jump patching, register
	// allocation shape). It needs no library surface beyond what is implemented, so it runs
	// entirely -- and it is the only file in the suite that checks CODEGEN rather than
	// observable behaviour, which is why the 54%-line coverage figure understated what was
	// missing: the gap was not uniform, it was concentrated in what nothing else tested.
	"checktable.lua": 0,
	"code.lua":       0,
	...
	"gc.lua": 111,
	// debug.sethook / debug.getlocal / debug.setlocal (db.lua:24 onward; exempt: debug advanced
	// surface). Only 23 lines run, kept because they cover debug.getinfo's currentline/source
	// fields against the official expectations rather than our own probes.
	"db.lua": 24,
	// os.tmpname (verybig.lua:89; exempt: os file surface). The first 88 lines are the point:
	// they build and run programs past the >64k constant/jump limits, which nothing else in
	// this suite or in difftest exercises.
	"verybig.lua": 89,
}
