...
两 driver 形态共享同一 leaf 体(`leaf(x)=x*3+1`)+ 同一紧循环骨架(调 leaf 100000 次),
**只差「每次调用如何建拆帧」**;两形态的帧工作量刻意等价(都写 4 段字 @ `segBase+depth*32`
+ `ciDepth` 增减 + `maxOpenIdx` 守卫读),差异**只在这工作在 Wasm 内做还是经 host 跨界回
Go 做**——隔离「两次跨界本身」的净成本。host 桩用 `WithGoModuleFunction`(零分配,对齐 R3.5
生产)。

| 形态 | ns/op(driver 100000 次) | **单次 dispatch+建拆帧** | allocs/op | 判定 |
|---|---|---|---|---|
| **inwasm**(建拆帧全 Wasm 内,**本里程碑形态**) | ~852k–858k | **~8.5 ns** | **0** | ✅ |
| twocross(建拆帧经 h_call + h_return 两次 host 跨界) | ~8.99M–9.07M | **~90 ns** | **0** | 当前 R3.5 后形态 |

**inwasm 比 twocross 快 ~10.5x(8.5ns vs 90ns)**;差值 **~81ns/call** 正是「两次 host
跨界做等价工作」相对「Wasm 内做同样工作」的净成本。两形态 allocs/op 均 0(twocross 用零
分配 host API,公平)。

## 2. 闸门裁决:🟢 GREEN — 零跨界里程碑放行

- **S-F1 闸门 ✅**:in-Wasm 帧建拆 ~8.5ns ≪ 两跨界 ~90ns,**快 10.5x**,allocs 不回退(均 0)。
...
形态已是「零 resync 的热路径」实测(8.5ns),Option A 的 resync 税只落在非热路径,不进
S-F1 的关键比较。生产 Stage 1b 落地后 Stage 4 re-bench 复核。
