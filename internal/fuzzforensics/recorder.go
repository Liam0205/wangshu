// Package fuzzforensics is the post-mortem recorder for silent fuzz
// worker deaths (the unreproducible concat-storm family, #123/#144/#145/
// #150-#152/#156/#157/#159/#162: workers die with "exit status 2" after
// millions of execs, the minimized input replays clean, and the log
// carries no stack).
//
// Two facts make this package work:
...
//
// Mechanism A (autopsy): when the test binary runs as a fuzz worker
...
//
// Wiring: the package that hosts the fuzz targets calls SetupWorker
// from its TestMain and RecordExec at the top of every target, after
// the target's length gate. SetupWorker is a no-op outside worker
// processes, so the same TestMain is correct for ordinary go test runs.
...
var (
	...
	// Written once, never read: reachability IS the use.
	stderrFile *os.File //nolint:unused // keepalive for the fd-2 alias, see above
	flightFile *os.File
...
// SetupWorker arms both mechanisms when the current process is a go
// fuzz worker (the coordinator passes -test.fuzzworker), and does
// nothing otherwise. Call it from the fuzz package's TestMain before
// m.Run.
//
// Setup is best-effort by design: forensics must never turn a healthy
// fuzz run into a failing one, so every error is swallowed.
func SetupWorker() {
...
// RecordExec is called at the top of every fuzz target, AFTER its
// length gate (recorded inputs are therefore always fully recoverable
// — over-long inputs are skipped unexecuted and cannot be the killer).
// One pwrite per exec (~1-2us against the page cache; the same pages
// are rewritten, so there is no I/O accumulation), zero heap
// allocations (review finding 2: fmt + time.Format allocated ~88 B/op
// — steady GC churn added to the exact workload whose memory pressure
// is under investigation; asserted at zero by
// TestFlightRecordZeroAllocs).
func RecordExec(target, src string) {
	if flightFile == nil {
		return
	}
