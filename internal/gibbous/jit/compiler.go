...
func (c *Compiler) Compile(proto *bytecode.Proto, feedback *bridge.TypeFeedback) (bridge.GibbousCode, error) {
	// PJ10 native path preferred when the CFG-based translator accepts.
	// The native emit covers multi-BB shapes (numeric FORPREP/FORLOOP
	// loops with inline SSE bodies) that the historical shape-spec fast
	// paths only partially handle. For any Proto AnalyzeNative accepts,
	// try native first; on any failure fall through to the existing
	// IC / spec / shape pipeline so behaviour stays identical for
	// unsupported shapes.
