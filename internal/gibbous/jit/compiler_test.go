...
// ExecuteCalleeFromInlineFrame mock stub(承 §9.20.9 commit-2 + commit-5l/5p 签名修正)。
// 单测路径不触达(archSupportsFrameInline=false 屏蔽真调用),返 0=OK 兜底。
func (m *mockP4Host) ExecuteCalleeFromInlineFrame(base, callA, callArgCount int32) int32 {
	_ = base
	_ = callA
	_ = callArgCount
	return 0
