...
func TestIndexStoreLineOnEveryPath(t *testing.T) {
	for _, tc := range []struct {
		...
		// stmtAssign's multi-target loop. Every store takes ONE line -- the last line the statement extends
		// to, as PUC's ls->lastline does -- not each target's own operator line. The unsplit target listed
