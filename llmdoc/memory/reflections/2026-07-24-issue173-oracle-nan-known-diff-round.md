...
`internal/oracle/compare_test.go` 21 组 `TestCompareOutput`：正例覆盖 lower /
upper / left-padded / right-padded / multiple tokens / 与 literal 相邻
（`NAN0` / `0NAN` / `BANANA vs BA-NANA`）/ symmetric script padding
（`io.write(" ", 0/0, " ")` 家族 4 例，本地审计 B1 补上）；负例覆盖「plain text 无证据」/
