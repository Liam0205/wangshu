...
- 预期:同款工作流第三轮——[[public-api-incremental-delivery]] guide 的 8 条纪律
  已在前两轮全验证可复用,本轮应零阻塞零意外直接落地。
...
读取,缺任意 key 迭代——**对 string-key map 这个极常见形态,没有 ForEach 等于
没有读出能力**。

**根因**:issue 关闭时只检查了「issue 字面要求的 API 是否全部落地」,没有检查
「落地后的 API 面是否构成写入/读出对称闭环」。
