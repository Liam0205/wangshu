...
description: >
  ...
  **版本核对干净**:失败 run 的 headSha 就是当前 HEAD(`e945a78`)。**本轮推翻了 #248 定下的口径**:
  ...
  LOADK 归第 2 行)。**独立审计阶段又查出第二个同类缺口**:`adjustExprList` 的
  `ends` 参数在赋值右侧 / `return` / 泛型 for 三处仍传 `nil`,实测三者**全都**产生与 #252 一样的
  `:1:` vs `:2:` 用户可见差分(GETTABLE 会 raise),已一并补齐——泛型 for 的末元素行必须在
  `check_match(DO)` **之前**取,否则会被 `do`/`end` 推过头。**远端评审又逮到第三格**:`stmtAssign` 单目标/单值快速路径
  直接调 `storeVar(s.Line, ...)`,`x = A\n.x` 的 GETTABLE 落 1 而 luac 是 2;根因是 `storeVar` 用同一个
  `line` 参数同时干「物化 RHS」(#252)与「发射 store」(#248)两件事,已拆成 `rhsLine` + `line`。
  **我自己的用例 `x = A.x\n` 抓不到它,因为 luac 在那里也报 1 —— 两个模型恰好一致、该用例没有区分力**,
  这正是本轮教训 1 的判据被自己违反了一次。**本地 `close-local-code-review` 未闭环**(两个 blind
  reviewer 在隔离快照里都没交出报告,记 review-integrity defect),最终靠 PR #253 远端评审两轮闭环
  (REQUEST_CHANGES → 修 → APPROVE)。六条教训:近似口径要标注成近似并写下反例形状(且每一格都要重新
  问「这个用例能区分两个模型吗」)/ 一次修复要同时改「谁来传」和「谁来用」两侧 / 修语料驱动的 issue 要
  把同族旧语料一起当验收门 / 给共用函数加可选参数时每个仍传默认值的调用点都要实测偏不偏(回退默认值会
  让漏接线与不需要接同形)/ 预先记录的缺口要附带「它何时会变得可见」的判据 / 核对基线行为用一次性
  clone 而不是在有其他 stash 的工作树里 push/pop。
metadata:
  type: reflection
  date: 2026-09-03
---
...
### 8. 第二个同类缺口:三个仍传 `nil` 的消费点全都偏

`adjustExprList` 的 `ends` 参数设计成「缺失时回退旧行为」,于是赋值右侧、`return`、泛型 for 三处先传了
`nil`,理由是「不在本 issue 已确认的差分范围内」。**这三处实测全都偏**,而且偏在 GETTABLE 上:

| src | luac GETTABLE | 修前 |
|---|---|---|
| `x, y = A.x\n, 1` | 2 | 1 |
| `return A.x\n, 1` | 2 | 1 |
| `for k in A.x\n, 1 do end` | 2 | 1 |

三者的 `FuzzOracleDiff` 都报 `oracle :2:` vs `wangshu :1:`——与 #252 同一个缺陷,只是在另外三个语法
位置。nightly 迟早会把它们当成三个新的 "crash" issue 开出来。

补齐时有一处易错:**泛型 for 的末元素行必须在 `check_match(DO)` 之前取**。`GenForStmt` 这个 AST 节点是
在整个循环体解析完之后才构造的,那时 `p.lastLine` 已经指向 `end`,直接用会把迭代器的 GETTABLE 推到循环
之外。`for k in A.x\n do end`(GETTABLE 归 1)与 `for k in A.x\n, 1 do end`(归 2)这一对把这件事钉住
了——差别只在有没有逗号。

发现它的时机值得记:独立审计流程本身**没有**闭环(两个 blind reviewer 都没交出报告),这个缺口是在为
「把 diff 摊开给用户自己审」准备材料、逐个验证「传 nil 的那三处到底偏不偏」时查出来的。也就是说,收益
来自那份**检查清单**,而不是来自审查流程跑通。

### 9. 第三个同类缺口:单目标赋值快速路径(远端评审逮到)

本地 `close-local-code-review` 没能闭环(见 §11),于是开 PR #253 走远端。评审 bot 第一轮
`REQUEST_CHANGES`,一个阻塞问题——`stmtAssign` 的单目标/单值快速路径直接调
`storeVar(s.Line, ...)`,绕过了 `ExprEndLines`:

```
x = A\n.x     luac GETTABLE 2,我们 1
```

自己读代码 + `luac5.1` 取真值,**确认是真的**。根因是 `storeVar` 用**同一个** `line` 参数干两件事:
物化 RHS(#252 的问题)与发射 store 指令(#248 的问题)。已拆成 `rhsLine` 与 `line`,缺失时回退。

**为什么我自己的测试抓不到**:我写的用例是 `x = A.x\n`,而 luac 在那里也是 1——两个模型恰好一致。
必须让**运算符**跨行(`x = A\n.x`)才能区分。这正是本轮教训 1 说的「构造判别输入」,而我在这一格上
没做到:我用了一个两模型一致的输入去测一个模型差异。

一处归因更正:评审称之为「回归」,不准确。实测 master 上同样报第 1 行,所以它是同族既有缺陷的又一格,
不是本分支引入的——`opLine` 从来只覆盖「索引就地被消费」的路径。修复照做,但归因要说准。

顺带确认 store 侧那个缺口仍不可见:用**能 raise** 的 `__newindex` store 实测两边一致,所以只修
评审指出的那一处,没顺手扩大范围。

### 10. 一次操作失误:`git stash` 撞了另一个 stash

验证「master 上是否也错」时用 `git stash` 来回切换,`stash pop` 与仓里另一个旧 stash
(`feat/p4-beat-p3` 的 checkpoint)撞了,在 `README.md`/`README.en.md`/`translator_native*.go` 四个
**与本轮完全无关**的文件上留下冲突标记。先确认内容完整保存在 `stash@{0}` 里,才用
`git checkout HEAD --` 复原工作树;`stash@{0}` 未动,内容没丢。

教训:核对基线行为要用一次性的临时 clone(本轮做 code review 时已经在用 `git clone --local
--no-checkout` 建快照,却在这里退回了 stash),不要在可能存有其他 stash 的工作树里 push/pop。

### 11. 本地 code review 未闭环,远端才拿到独立结论

`close-local-code-review` 走到第 4 步就断了:两个 blind reviewer 各自在固定 SHA 的隔离快照
(object-independent clone + `bwrap --unshare-net` 网络拒绝边界,实测 `curl` 与 `git ls-remote` 都
解析失败而本地 git 正常)里跑,**都没有交出报告**。三次索要无果,按 skill 的重试上限规则停手,记
`review-integrity defect`——这个范围没有任何独立审查结论,既不是 APPROVE 也不是零发现。

改走远端后,评审 bot 两轮就闭环:`d6a0d15` REQUEST_CHANGES(1 阻塞)→ 修 → `05998e0` APPROVE
(0 发现,范围覆盖完整 diff)。CI 两轮全绿。

值得记的是**收益分布**:三个 `nil` 缺口(§8)是在为「把 diff 交给人手审」准备清单时查出来的,
单目标赋值这一格(§9)是远端评审查出来的,而本地那套隔离快照 + 网络边界的流程虽然前半段证据齐全,
最终一份判断都没产出。
...
| 赋值右侧 / `return` / 泛型 for 先传 `nil` 就行,它们不在本 issue 范围内 | 三处**全都**偏,且都是会 raise 的 GETTABLE,等于把 #252 原样留在另外三个语法位置;「不在范围内」是 issue 的属性,不是代码的属性 |
| 走完 `close-local-code-review` 就能拿到独立结论 | 两个 blind reviewer 都没交报告(review-integrity defect),流程没闭环;真正逮到第二个缺口的是为审查准备的那份检查清单 |
| 补齐三个 `nil` 消费点之后,同类缺口已经找全 | 还剩单目标赋值快速路径(`storeVar`),远端评审第一轮就逮到;共用函数的**每一个**调用点都要查,不能数「主要的几个」 |
| 我为这一格写的用例能验证它 | `x = A.x\n` 在两个模型下期望值相同,对本轮的模型差异没有区分力;必须运算符跨行(`x = A\n.x`)|
| `git stash` 切一下核对基线是安全的轻量操作 | 与仓里另一个旧 stash 撞了,四个无关文件留下冲突标记;正确手法是一次性 clone,而本轮 code review 里已经在用它 |
...
**本轮自己违反了这条判据一次**,值得记下来:单目标赋值那一格(§9)我写的用例是 `x = A.x\n`,而 luac
在那里也报 1——两个模型**恰好一致**,所以这个用例对「运算符行 vs discharge 行」没有区分力。必须让
运算符跨行(`x = A\n.x`)才能判别。也就是说,判据的难点不在「知道要构造判别输入」,而在**每一格都要
重新问一次「我这个用例真的能区分两个模型吗」**——我在主 case 上做到了(`A.x\n\n+1` 就是精心构造的
判别输入),在后来补的那一格上凭手感写了个形似的用例就过了。自查办法:写完用例后反问「如果被否掉的
那个模型是对的,这个用例会给出不同的期望值吗」,答不出「会」就说明它没有区分力。
...
### 教训 4:给共用函数加可选参数时,「暂时传 nil 的调用点」必须逐个验证偏不偏

**核心断言**:`adjustExprList` 加了 `ends []int32` 参数、并让缺失时回退到原来的行,这个设计让「只改真正
需要的调用点」变得很自然——于是赋值右侧、`return`、泛型 for 三处就都先传了 `nil`。当时的判断是「不在
已确认的差分范围内」,但那不是「已验证不偏」,只是「没测」。实测三处**全都偏**,而且偏在 GETTABLE 这条
**会抬错**的指令上:`FuzzOracleDiff` 对三者都报 `oracle :2:` vs `wangshu :1:`,与 #252 是同一个缺陷,
nightly 随时会把它们当成三个新 crash issue 再开一遍。

回退默认值(`endAt` 在缺失时用旧行)让这件事更隐蔽:代码编译通过、既有测试全绿、行为与改动前完全一致,
所以「没接线」和「接了线但不需要」在测试结果上无法区分。

**判据**:给共用函数加一个「可选、缺失时回退旧行为」的参数时,把**所有**调用点列出来,逐个回答「传
nil 的这处,实测偏不偏」。答案必须来自参照实现的真值比对,不是来自「它不在这个 issue 的范围里」。范围
是 issue 的属性,偏不偏是代码的属性,两者无关。自查办法 = 给每个仍传缺省值的调用点各构造一个输入,
断言它与参照实现一致;如果偏了,那它就已经在范围内了。

这与本轮教训 2(「谁来传」和「谁来用」两侧)是同一条数据流纪律的下一格:那条讲**一个**调用点两侧都要
对,本条讲**所有**调用点都要查——修好了一个消费点不代表同一个机制的其他消费点也好了。

### 教训 5:记录一个已知缺口时,要附带「它在什么条件下会变得可见」
...
### 教训 6:核对「改动前是否也这样」要用一次性 clone,不要在工作树里 `git stash` 来回切

**核心断言**:判断一个差分是「本轮引入的回归」还是「同族既有缺陷」,需要在改动前的树上跑一遍同样的
输入,这一步本身是对的、也确实两次纠正了归因(#252 的三个 `nil` 缺口与单目标赋值那一格都被证明**不是**
本分支引入)。问题在手法:我用 `git stash push` / `pop` 在同一个工作树里来回切,而仓里还存着另一个旧
stash(`feat/p4-beat-p3` 的 checkpoint)。`pop` 与它撞了,在四个与本轮完全无关的文件上留下冲突标记,
工作树一度处于坏状态。

讽刺的是本轮做 code review 时**已经**在用正确手法——`git clone --local --no-hardlinks --no-checkout`
建固定 SHA 的隔离快照,恰恰是为了不碰协调端工作树;到了「顺手核一下基线」这一步却退回了 stash,因为
它看起来更轻。

**判据**:任何「在另一个 revision 上跑一遍」的需求,都用一次性的本地 clone(`git clone --local
--no-hardlinks` + `checkout --detach <sha>`),跑完删掉;不要 `git stash` 当时间机器。`stash` 是一个
**共享的全局栈**,而不是「我这次操作的私有暂存区」——它的风险与工作树里有没有别人的东西有关,与你这
次改了什么无关,所以「我只是切一下就回来」这个理由不成立。若已经踩了:先确认内容还在哪个 stash /
dangling commit 里,再动工作树,不要先 `checkout --` 后确认。
...
- **教训 4 → [[prove-the-path-under-test]]**:§4.1(一个 reported case 只是接受面的一个采样,要枚举
  所有到达同一段代码的通道)的新一格——共用函数的**可选参数**制造了同一种采样偏差,而「缺失时回退旧
  行为」的设计让漏接线在测试结果上与「不需要接」完全同形。
- **教训 5 → [[prove-the-path-under-test]]**:§4.2「已知差异类注释必须有执行体」的时间对偶——缺口
  记录要写触发条件,否则下次撞上时认不出是旧账。
- **教训 6 → 暂不 promote**,留在本篇。它是一条 git 操作手法,不属于现有任何一篇 guide 的主题族;
  若日后再出现一次「用 stash 当时间机器踩坑」,再考虑并入工程纪律类文档。
