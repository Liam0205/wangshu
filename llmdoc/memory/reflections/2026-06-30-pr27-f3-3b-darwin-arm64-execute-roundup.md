...
修复:把 user STP 偏移从 `0(RSP)` 改成 `16(RSP)`(避让 LR slot @ [SP+0] + FP slot @ [SP+8],进 caller-saved 区),并把对应的 LDP 偏移同步。详 `8639eb5`。
...
- **trampoline_arm64.s LR slot 偏移 0→16 的具体选择**:[SP+0] 是 Go auto-prologue 写 LR、[SP+8] 是 FP,user 第一个可用 caller-saved slot 从 [SP+16] 起;选 16 而非 32/64 让 Go auto-prologue 的栈帧大小 96 字节仍够用,无需扩 prologue。
...
- **跨平台 trampoline ABI 调试**:linux/darwin/windows 的 ABI 各有 prologue/auto-frame 约定,user asm 直接写 SP-relative 偏移时**必须避让 caller auto-prologue 的 LR/FP slot**——Go arm64 auto-prologue 写 LR @ [SP+0] + FP @ [SP+8],user STP 应从 [SP+16] 起;
