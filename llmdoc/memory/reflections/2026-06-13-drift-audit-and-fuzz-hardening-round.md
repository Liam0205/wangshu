...
**A 群** issue #5/#6 公共面缺口轮 3(`4f855d2..59606d7`)——同款工作流第三轮,过程
...
同**「同族审计」纪律家族**——一处变更必扫同族:性能优化扫同族快路径(同一形态多处
...
**落地三件套**(`docs/design/p1-interpreter/12-testing-difftest.md §4.9`):
...
兜底机制量级关系」**观察 P2+ 是否复现(如配额类 / 时间窗口类测试再次踩同款坑),
...
- P2+ 新增 stdlib 函数若涉及「`len * n`」「`width/precision`」「循环计数由脚本控
  制」三类形态,前置约束:套用 §4.9 阈值口径(分配类 1 GiB / 循环类 1<<24)。
