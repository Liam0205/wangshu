...
description: >
  ...
  (luac 多一条 MOVE),三轮审阅者把它归为「基线就有的 opcode 差异」略过,直到终审者拿例子跑了一下。
metadata:
  type: reflection
  date: 2026-09-19
---
...
#262 的行号比对脚本把 opcode 序列不同的形状单独归一桶、不比行号(后来改成按 opcode 比行号多重集)。四轮审阅者都
看过这个桶,都把它归为「基线就有的 codegen 差异(多发 LOADNIL、常量折进 RK……)」——**归类正确,但没有问一句
「这条多出来 / 少掉的指令会不会让结果不同」**。终审者拿其中一个形状写了个例子跑,才发现少的那条 MOVE 是语义。
