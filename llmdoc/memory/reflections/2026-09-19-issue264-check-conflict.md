...
description: >
  ...
  MOVE 在语句末行,换行落在两者之间的形状按 opcode 比行号多重集就不一致),但单行例子里两条 MOVE 数量相抵、只差
  顺序,按 opcode 名和条数看不出——桶里的差异要逐类构造可观察输入判语义。
metadata:
  type: reflection
  date: 2026-09-19
---
...
换行落在**局部目标名与语句末 token 之间**时两边 MOVE 的行号多重集就不同;换行落在更早的间隙两边仍同行、看不出
(在修复前的树上跑六条冲突模板:pass 1 aligned_line_diff = 403,全部来自四条常量收尾模板,占它们 1188 个形状的约三分之一;
HEAD 上为 0)。所以「dump 比对对这一格是盲的」不成立,准确的说法是:**按 opcode 名和条数分类看不出它,按行号多重集
在局部目标名与语句末不同行的形状上能抓到,但要判它是不是语义 bug 仍然只能构造可观察输入跑一遍**。
