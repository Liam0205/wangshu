...
> 范围:`docs/design/p2-bridge.md`(703 行单文件)扩展为 `docs/design/p2-bridge/`(子目录 8 文件 7453 行)。仿 P1 13 篇详细设计 + 00-overview 形态,补 PB0-PB7 里程碑、人月分解、实装级代码骨架、F1-F7 visitor 设计、跨文档回填请求收口表。驱动:用户 goal「针对 P2,仿照 P1 的做法,将 P2 设计稿扩展为详细的施工指南」。
>
...
**Why**:P1 首次验证时还有「子代理中断恢复」教训(原反思 §1),本轮更稳——同款工作流第二次跑零阻塞,验证机制级稳定,而非首轮巧合。

**How to apply**:任何后续大型文档起草任务(如 P3/P4 详细设计扩展)**直接复用本工作流**,不再需要预演——但每次任务前**仍要主助理亲写 00-overview**(纲领必须主助理拍板,不能并行外包,因为它定下后续所有子文档的派生约束)。
...
**Why**:子代理的拍板倾向是「按当前需求选最简方案」(YAGNI),用户作为长期持有者更看重「未来扩展接口零成本」(预设接口比事后扩接口便宜 100 倍)。这两种视角在大型设计期都需要,**子代理拍板 + 用户裁决是健康的协作模式**。

**How to apply**:大型起草任务收尾时,主助理用 multi-doc-drafting guide §3「主动盘点不确定决策」纪律,把子代理拍板但「影响 × 不确定度」高的决策列出来,用 `AskUserQuestion` **自包含地**问用户(每个选项的具体后果要写清,不留指代——P1 设计评审轮反思教训)。本轮 3 条决策(AST 协议 / profile 归属 / P1 04 改时机)用一次 AskUserQuestion 收口,用户裁决落地无歧义。
