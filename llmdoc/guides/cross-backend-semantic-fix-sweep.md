...
的目标判 VLOCAL),却没有一条让它内部「此前索引目标的表 / 键寄存器 == 本局部」为真,拷贝 MOVE 从未发出——这一格从未
进过扫描面,是终审者手写例子跑出来的(#264)。一个条件若比较两个目标之间的关系,模板就要同时构造两者。**进了 opcode
序列差异桶的也要逐类构造可观察输入判语义,条数相同也不能放过**:这一格修复前 luac 多的拷贝 MOVE 与望舒末常量多出的
MOVE 数量相抵、只差顺序,按 opcode 名和条数看不出;局部目标名与语句末不同行的形状上两条 MOVE 行号不同、按行号
多重集能抓到,但那只说明「有差异」,是不是语义仍要跑例子
