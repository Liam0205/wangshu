...
没有「局部变量作多目标赋值目标」的形状;终审补的七条局部目标模板进了 `check_conflict`,却没有一条让它内部
「此前索引目标的表 / 键寄存器 == 本局部」为真,拷贝 MOVE 从未发出——这一格从未进过扫描面,是终审者手写例子
跑出来的(#264)。一个条件若比较两个目标之间的关系,模板就要同时构造两者。**进了 opcode 序列差异桶的也要逐类
构造可观察输入判语义,条数相同也不能放过**:这一格修复前 luac 多的拷贝 MOVE 与望舒末常量多出的 MOVE 数量相抵、
只是顺序不同,按 opcode 名和条数都对得上,dump 比对对它是盲的
