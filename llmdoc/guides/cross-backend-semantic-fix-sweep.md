...
范围之后又扩了两次:与 PUC 逐字节差分时「枚举权威 C 源码」是同一条纪律换了对象(见「PUC 语义由 C 实现
定义」节的六个刻度);而**共享层**里同一条调用约定在多条返回 / 退出路径上各做一次时,漏掉其中一条也属于
本 guide 的不对称家族(见「共享层恢复动作的兄弟路径不对称」节)。
...
值),**20 组「先写下预测再实测」全部吻合**——推导与实测其实一致,当时不一致只是因为模型漏了一项。后果是
...
- 反思实例:`2026-07-08-issue67-amd64-nodehit-crossrun-round` /
  ...
  `(e - i + 1) > INT_MAX` 里带着 `e`,窗口随 `e` 平移;补上 `e` 之后 20 组预测全部吻合实测,推导与实测
