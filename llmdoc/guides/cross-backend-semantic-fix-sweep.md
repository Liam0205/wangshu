...
上一节的两处错误是**并列**的(GETGLOBAL 与 GETTABLE 各错一次)。本条是它的另一种拓扑:两处错误在**同一
条数据流的上下游**——一个值被**传**下来、又被**用**掉,传的人给错了、用的人还覆盖了传进来的值,于是同一个
量有两个独立的出错位置。

**实证(#252)**:GETTABLE 的行号错,成因分两侧。**用的一侧**:`dischargeVars` 里 `e.opLine != 0` 时运算符
行**永远优先**,调用方传进来的 `line` 被无条件忽略。**传的一侧**:`stmtLocal` 传的是语句起始行而不是
lastline。只删 `opLine`(修用的一侧)会让 `A.x\n\n+1` 变对、却让 #248 的 `local v = A\n.x` 退化;只改传的
一侧则被 `opLine` 覆盖、完全没有效果。两侧都改才对。同一形状后来还出现第三次:`storeVar` 里**一个参数被用
在语义不同的两处**(物化 RHS / 发射 store),这已经不是「传错了」而是「该拆成两个参数」。

**为什么容易漏**:有回退/优先级逻辑时(`if x != 0 { 用 x }`),**用的一侧会掩盖传的一侧的错误**——传错了
也看不出来,因为传进来的值根本没被用。反过来删掉优先级逻辑,又会让原先靠它兜住的调用点立刻退化,很容易
被读成「删错了」。

**检查项**:改行号/位置/上下文这类「被传下来的量」时,先插桩打印**每个失败形状**的调用栈加上「传入的值 /
实际用到的值」两个数,按「传入的对不对」把形状分组:传入的对而结果错 → 改用的一侧;传入的就错 → 改传的
一侧;两组都有 → 两侧都要改。本例分组后一次定位了三处,不需要试错。另附一条:若同一个参数在函数体内服务
**语义不同的两个用途**,拆参数而不是让调用方猜(见 [[prove-the-path-under-test]] §4.7a / §4.7b)。反思
[[2026-09-03-issue252-discharge-line-is-lastline]] 教训 2。

### 合取型叠加:两处缺陷要同时在场症状才出现,修任一处症状就消失(2026-09-13,#260)

前两条讲的都是**析取**型叠加:任一处错,症状就在,少修一处症状仍然不对,所以「修完还不对」会提醒你
继续找。本条是它的反面,也更危险:两处缺陷是**合取**关系,要**都**在场症状才出现,修好任何一处症状
立刻消失。先看到哪处就修哪处,seed 变绿,另一处继续潜伏,等下一个恰好凑齐条件的 seed。

**实证(#260)**:P4 native 上 `s=nil s=0` 之后 `s+0` 报「global 's' is nil」。缺陷一:inline GETGLOBAL
的「slot != Nil」守卫拿 `0xFFFE<<48`(TagUserdata)当 Nil 位,对真 Nil 永不触发。缺陷二:`rawSet` 删键
不 BumpGen,被删槽位 `next>=0` 留在链上,同键再插落到别的槽位,IC 快照指向的旧槽位变成 Nil。两者
合起来:旧槽位是 Nil(缺陷二)且 Nil 没被守卫挡住(缺陷一),Nil 被当成 `s` 的值。分别只撤一处修复,
seed **都通过**。

**为什么容易漏**:「seed 变绿」在析取型下是充分的完成信号,习惯会把它带到合取型里。而且第二处往往
是在修第一处时顺带看见的,「顺带看见」的东西没有 issue 编号,没有 seed,很容易被记成 followup。

**检查项**:修第二处可疑缺陷前先做**撤回矩阵**——每处单独撤、全撤、都不撤,对原 seed 各跑一遍。只要
出现「单撤仍绿」,那个 seed 就不是任何一处的判别输入,必须为每一处另造只因它变红的输入。构造思路是
**消掉另一处缺陷的参与条件**:#260 用数组部(没有 key→slot 间接,gen 不会变)孤立 Nil 守卫;用「别的
键落进被删槽位」(旧槽位不是 Nil)孤立 gen。判别输入到手后再做一次 2×2 矩阵,每格按预期红绿。反思
[[2026-09-13-issue260-nil-immediate-and-delete-gen]] 教训 2。
...
望舒 `LineInfo` 并排比对的脚本,喂 90 多个**手挑**的跨行形状,十几分钟扫出三族红格:运算符全家、单目标
赋值的 store 行、数值 for 表头——后两族是 doc-gaps 里挂了两周的「已知残留」,本来要等两个新 issue。
但手挑有手挑的盲区:第一轮独立审阅用 **31 个模板 × 在 token 间隙插换行(最多两处)= 1875 个组合枚举
...
作为多目标赋值目标」的形状(`local a , b = ...` 是声明不是赋值),而且比对脚本把 opcode 序列不同的形状
直接归入 OPSEQ 一类、不再比行号,这类形状恰好因为我们多发一条 LOADNIL 全落在 OPSEQ 里。终审者把两边按
...
**检查项**:对齐工作的对象是那张表。参照实现能被本地调用时(`luac5.1` / `lua5.1` / cgo oracle),
**先**写 dump 比对脚本,形状用**枚举**生成而不是手挑:按 `lparser.c` 的非终结符各写一个模板(每种语句、
`body`、`simpleexp` 各分支、`primaryexp` 各后缀、构造器各字段形式、`funcargs` 三种、带 upvalue 的块退出),
**并且每个非终结符的每个产生式分支在每个可出现的位置都要有一个模板**(方括号键位置上的 `exp` 少了
and/or 分支,19538 个形状就漏了一格),在每个 token 间隙插换行或注释、取到两处,几分钟生成两万个形状;**嵌套 proto 一起比**,不要只看主 proto;
...
哪个桶是不比的」。**模板还要让参照实现里的每个条件分支各为真一次**:`lparser.c` 里凡是 `if (v->k == VLOCAL)` 这种按操作数
类别分叉的地方,模板里就要有让它为真的形状——#262 的三套模板没有一条「局部变量作多目标赋值目标」,所以
`check_conflict` 这一格从未进过扫描面,是第一轮全范围终审者手写例子跑出来的(#264)。**进了 opcode 序列差异桶的也要
逐类问语义**:对每一类差异构造一个能观察结果的输入跑一遍,确认是纯形式差异(多发 LOADNIL、常量折进 RK)还是有语义
——补上冲突模板后,少的那一条 `MOVE` 与多发的 LOADNIL 挤在同一个桶里,按 opcode 名看不出前者是语义 bug
([[2026-09-19-issue264-check-conflict]])。反思
[[2026-09-18-issue262-operator-line-is-posfix-lastline]] 教训 3。
