...
语义 bug**(CLOSE 发在回边之后从未执行)。最终一轮把全部 DIFF 修掉,19538 形状逐指令行号 0 差异,
opcode 序列差异集合与基线逐条相同。
