...
哪个桶是不比的」。**opcode 序列差异桶还要逐类问语义**:对每一类差异构造一个能观察结果的输入跑一遍,确认是纯形式
差异(多发 LOADNIL、常量折进 RK)还是有语义——#262 的桶里少的那一条 `MOVE` 就是多目标赋值缺 `check_conflict`
的语义 bug(#264),#262 的三轮独立审阅都把它归为「基线就有的 opcode 差异」略过,直到第一轮全范围终审拿例子跑了一下
