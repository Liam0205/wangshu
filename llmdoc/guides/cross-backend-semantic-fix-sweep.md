...
望舒 `LineInfo` 并排比对的脚本,喂 90 多个跨行形状(运算符全家、赋值/`return`/表构造/控制流各位置),
十几分钟扫出三族红格:运算符全家(算术 / 比较 + JMP / `and`/`or` 右操作数 / CONCAT 链 / 一元)、单目标
赋值的 store 行、数值 for 表头——后两族是 doc-gaps 里挂了两周的「已知残留」,本来要等两个新 issue。
剩下的 DIFF 顺手归类成「确认不可见」(见 [[prove-the-path-under-test]] §4.2a),不再是模糊的「还有些没对齐」。

**为什么容易漏**:seed 给的是一个点,修复的自然边界也是那个点;而口径的作用域是一张表(语法位置 ×
发射点)。没有工具把表摊开时,「同族还有哪些」只能凭记忆列,列不全是常态。

**检查项**:对齐工作的对象是那张表。参照实现能被本地调用时(`luac5.1` / `lua5.1` / cgo oracle),
**先**写 dump 比对脚本、枚举语法位置批量扫、按 DIFF 分类,再修;修完再扫一遍确认只剩「已归类不可见」。
自查办法:修完 seed 后问「同一条口径还作用于哪些语法位置,我扫过了吗」——答案应当是一份清单加每格
红绿,而不是「应该没了」。反思 [[2026-09-18-issue262-operator-line-is-posfix-lastline]] 教训 3。
