...
但手挑有手挑的盲区:独立审阅用 **31 个模板 × 在 token 间隙插换行(最多两处)= 1875 个组合枚举形状**
再扫一遍,在我没挑到的两个位置(泛型 for 生成器、构造器 `[k]=v` 键)各抓出一处会 raise 的差异。最终
一轮把全部 DIFF 修掉,1875 形状逐指令行号 0 差异。
...
`simpleexp` 各分支、`primaryexp` 各后缀、构造器各字段形式、`funcargs` 三种),在每个 token 间隙插换行、
取到两处,几秒钟生成上千个形状;按 DIFF 分类,能修就修,修完再扫到 0。自查办法:修完 seed 后问「同一条
口径还作用于哪些语法位置,我扫过了吗」——答案应当是枚举生成的清单加每格红绿,而不是「应该没了」;
如果清单是手写的,再问一次「有哪个非终结符没有模板」。反思
