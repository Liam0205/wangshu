...
应当在 nightly 上自然归零,**等实测归零再删,就有数据背书而不是靠推断**。
...
- checklist 上写着「评估并删除」时,**「评估结论是证据还不够」也是一次合格的执行** —— 把结论
  与理由写进文档、把撤除条件写成一个可观测的量(本轮是「nightly 上归零」),比为了勾掉一格而
  删掉一层还需要的保护划算。
...
- 与 [[design-claims-vs-codebase-physics]] 构成对偶双防线:那是**实现前**重验设计稿主张,本 guide 是**实现后**证明在测路径。两篇在「上游修复来了之后怎么办」这一点上接口:那篇 §5.1 负责判定**上游到底修没修**(判据取那份源码,并区分「存在」与「引入」),本篇 §4.6a 负责判定**为它加的兜底能不能撤**(证据要与触发频率同量级) —— 两个问题都会被同一轮升级触发,而两个答案可以不同(#180 就是「确认修了」+「兜底先留着」)。
