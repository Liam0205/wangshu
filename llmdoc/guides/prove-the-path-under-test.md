...
### 2.1 位置/行号类测试的载体必须触发延迟加载,否则被测指令根本没发射(2026-08-27,#248)
...
**实例(#248)**:`internal/frontend/compile` 里索引表达式跨行时的行号有两处独立错误——`exprIndex` 把
对象交给 `exp2AnyReg` 时传了运算符的行(该传对象自己的行),以及 `eIndexed` 这个 `expDesc` 不带行号、
GETTABLE 用「之后由谁 discharge 就取谁的行」。仓库里早就有 `TestIndexExprLineIsOperatorLine`
(`internal/frontend/parse/parser_test.go`),而它在两个 bug 都存在的整段期间**一直是绿的**:它用局部
变量做对象(`local v = t\n .x`),局部变量已经在寄存器里,`exprIndex` 对它不发射任何指令、GETTABLE
立即 discharge——两个 bug 依赖的「对象加载被延迟」这个前提在这条路径上从不成立。新测试
`TestIndexLinesAcrossNewline` 改用**全局**做对象(触发 GETGLOBAL 延迟加载),抓到了两个方向相反的错误。

**判据**:写位置/行号类测试之前,先问「这个写法会让编译器**发射**我关心的那条指令吗」——不发射就测不到。
判定方法:操作数已经在寄存器/已知位置(局部变量、编译期常量)时,codegen 很可能就地复用、不发射新指令;
...
#### 2.1a 用例要能区分两个候选模型,否则它只是「碰巧同意」(2026-09-03,#252)

**适用场景**:同 §2.1,以及任何「参照实现按规则 A,我们按规则 B」的对齐工作。

**核心断言**:一个位置类用例的价值取决于**两个候选模型在它上面给出不同期望值**。规则 A 与规则 B 在大量
...
必须让运算符跨行:`x = A<nl>.x`(luac 2,漏接线时得 1)。

**判据**:写完一个用例后反问「如果被我否掉的那个模型才是对的,这个用例的期望值会不一样吗」——答不出
「会」,它就没有区分力,不要把它算进覆盖。注意这条的难点不在「知道要构造判别输入」(#252 在主 case 上做
到了),而在**每加一格都要重新问一次**:同一轮里,先构造了判别输入、后面顺手补的用例却退回形似写法,是
很自然的滑落。反思 [[2026-09-03-issue252-discharge-line-is-lastline]] 教训 1。
...
在运算符刚被消费时跑(左操作数在此物化),`luaK_posfix` 在右操作数解析完之后跑(右操作数、运算指令、
比较的 JMP、CONCAT 都在此发射)——所以「lastline」在这一格里同时指两个不同的行。一个只让两个候选不同的
形状,对第三个候选没有区分力。

**实例(#262)**:#252 的表里用 `local v = A.x<nl><nl>+1` 证明「算术运算符是 discharge 点」,这个形状里
`+` 与 `1` 同在第 3 行——运算符行与 posfix 行**重合**,所以 `exprBin` 用运算符行 `e.Line` 发射 MOD 也
全绿。nightly 的 `error("boom"%<nl>0)` 把两者拆开(`%` 在 1、`0` 在 2),PUC 报 2、我们报 1。同一批
用 `luac5.1 -p -l` 扫出来的还有:比较的右操作数与 JMP、`and`/`or` 的右操作数、CONCAT 链(每个操作数由
**跟在它后面**的 `..` 推入,取那个 `..` 的行;最后一个与 CONCAT 取末操作数行)、一元 UNM/NOT/LEN(只有
prefix 一个点,在操作数之后)。

**判据**:对齐参照实现的行号时,对每个 AST 节点类型回到 `lparser.c`/`lcode.c`,列出它发射的**每一条**
指令各在哪个 `luaX_next` 之后;然后为每条指令构造一个让所有候选行**两两不同**的输入(通常至少三行)。
自查办法:测试表里每个形状数一数「它让几个候选行不同」——数目小于候选数,就还有一个候选没被排除。
反思 [[2026-09-18-issue262-operator-line-is-posfix-lastline]] 教训 1。

### 2.2 能编译不等于进了那条路径 —— 新 tier / 新 build tag 的 harness(2026-08-29)

**适用场景**:给一个新的执行层(tier / 后端 / build tag 组合)加差分或 fuzz harness 时。

**核心断言**:**build tag 是累加的,所以给一个 harness 加上新 tier 的 tag 之后,
它照旧编译、照旧通过 —— 而它跑的可能仍然是老路径。** 新 tier 的代码被链接进了二进制,
但从来没有被进入。这是 §2 的原始命题在一个更前置的层次上的形式:那边问「走了哪条实现」,
本条问「新那一层有没有被进入过一次」;而它比 §2.1 更难察觉,因为**一个忘记提升的 harness
与一个通过的测试在输出上完全一样**,连一条 skip 都不留。

**实例(2026-08-29)**:本仓的 `FuzzOracleDiff`(p1 对内嵌 PUC 的差分)在
`wangshu_p3` / `wangshu_p4` 的 tag 下**本来就能编译并通过** —— 它构造的是普通 `State`,
从不提升,tier 代码被链接进来但从未进入。新增 `FuzzOracleDiffTiered` 时如果忘记调
`SetForceAllPromote`,它就退化成「把解释器对 PUC 测了第二遍」,而这件事从输出上无从区分。
所以该目标配了 `TestTieredOracleDiffActuallyPromotes` 断言 `PromotionCount()` 增长,
并用变异实测过(删掉 `SetForceAllPromote` 之后 `PromotionCount did not grow (0 -> 0)` 直接挂)。
...
真的被写、类型也真的对,看起来完全像在工作。自查:`grep` 那个标志的**读取点**,不只是写入点。

**(b) 变异实测要作用在 payload 上,不只作用在那个开关上。** 同一个守护测试删掉
`SetForceAllPromote` 会变红(所以它防住了「整个 harness 忘记提升」),但把 payload 换掉
它**不会**变红 —— 而且弱到什么程度是实测出来的:**把 payload 换成空字符串,
那条 `after > before` 断言照旧成立**(实测 9 → 10)。原因是 `PromotionCount` 是**编译侧**
的量,而 force-all 会提升被编译出来的那个 **main chunk** 自己;只要还有一个可编译的 Proto
存在,这个计数就增长,不需要任何被测函数存在、更不需要它被执行。实测对照:
...
所以这条断言实际证明的是「force-all 开着」,不是「被测的那段代码进了 tier」。
要后者得读一个**执行侧**的计数器(本仓 P4 是 `peroptranslator.NativeRunCount`)。
**编译侧计数器与执行侧计数器回答的是两个问题**,与 §8「单 Run 命中数 vs 跨 Run 稳态
dispatch」是同一类单位选择错误 —— 这一条也是 §9.5「harness 的守护测试自己也要过 §2」
的实例:守护测试不过 §2 时,它守护的东西就没有被守护。
...
**适用场景**:某两层之间的一致性不是直接比出来的,而是经第三层传递得来时。

**核心断言**:**`A == B` 且 `B == C` 推出 `A == C`,但这条链继承 `B == C` 那次比较的
每一个盲区,而且对「B 与 C 一致、而 A 的两个变体各自也一致地错」这类情况没有直接观察能力。**
传递结论在文档里读起来与直接结论等价,而它的证据强度不等价。

**实例(2026-08-29)**:本仓 p3/p4 的 fuzz 差分**不拿 PUC 当参照,而是拿 p1 当参照** ——
...
因为「tier 与 PUC 分歧而 p1 不分歧」正是这个目标存在的理由。

**判据**:当 A 与 C 的一致性是通过 B 传递得来的,先问「有没有一条**直接**比 A 与 C 的路」;
没有的话,那个盲区要如实写进文档,不要让传递结论看起来与直接结论等价。
新加的直接轴要**沿用中间那条链的跳过集合**,新增的 tier 专属跳过要当红旗看。
反思 [[2026-08-29-conformance-coverage-and-tiered-oracle-diff]] 教训 2。
...
- 仓内 `test/luasuite/testdata/*.lua` 是 PUC 5.1 官方测试套移植,现有文件与上游**逐字节相同**。
  ⚠️ **但「覆盖度与官方一致」是错的**(2026-08-29 实测纠正):套件里的文件数少于上游,而且多数文件被
...
- 加新 e2e 语料前先 `grep -l <feature> test/luasuite/testdata/`——**命中之后还要确认那一处在 `stopAt` 之前**;
  确认在跑且已含 happy path + 重要边界,才**不写冗余语料**,以「luasuite 覆盖到了这一处」作证据
  (权威性高于手写)。命中的行落在截断之后时,那个特性等于没被套件覆盖。
...
#### 4.2a 「这条指令不会 raise,所以行号偏了也不可见」的作用域是指令不是路径;而且 raise 不是唯一可见通道(2026-09-18,#262)

**适用场景**:把一处与参照实现的行号差异记为「已知但不可见」时;审计既有的这类记录时。

**核心断言**:「不会 raise」是**某一条指令**的属性。同一个 codegen 函数往往发射好几种指令,它们能不能
raise 各不相同;拿其中三条不会 raise 的当依据,把整条路径判为不可见,就把会 raise 的那条一起豁免了。
并且 raise 不是行号的唯一可见通道:`debug.getinfo(f, "L").activelines` 把 LineInfo 的键集合原样暴露给
脚本,任何行号偏移在那里都能被比对出来。

**实例(#262)**:`storeVar` 的 store 行在 #248 与 #252 两轮都被记为不可见缺口,依据是「SETGLOBAL/
SETUPVAL/MOVE 不会 raise」——但 `storeVar` 还发 SETTABLE,对 nil 对象做 SETTABLE 会 raise,`__newindex`
...
> **同一个函数上的第二次(2026-08-11,#244)**:这条判据在 `unpack` 上又被需要了一次,而这次漏掉的量是
> `e`(默认 `#t`)。#244 量的是 PUC 的**崩溃窗口**而不是上限:实测得到「只有 `-2147483648` 与
> `-2147483647` 会崩」,但那批测量全部取自 `unpack({}, i)` 这一种写法,而崩溃条件
> `(e - i + 1) > INT_MAX` 里带着 `e`,所以 `unpack({1,2,3},-2147483646)` 同样让 PUC 段错误、却落在守卫
> 之外。**判据在同一个函数上两次被需要,说明它最容易被跳过的情形是:边界看起来只与一个显式参数有关。**
> 补一条自查:写下那个边界之前,先写出决定它的**表达式**——表达式里的每一项都是一个取样维度。详见
> [[cross-backend-semantic-fix-sweep]] 第八个刻度与反思
...
### 4.6a 什么时候可以撤掉一个兜底机制:证据要与它的触发频率同量级(2026-08-27,#180)

**适用场景**:某个 bug / 上游缺陷的兜底机制(条件重试、宽松判据、fallback、额外守卫)在缺陷
被上游修掉之后,要判断能不能删。

**与 §4.6 的关系**:§4.6 讲**该撤而没撤**(skip 与它防的 bug 一起走),本节讲**撤之前要有多少
证据**。两条是同一件事的两个方向,而且这个方向的失效方式更隐蔽:§4.6 的代价是「一类输入静默
退出比对」,本节撤错的代价是「真失败被当噪声吞掉」,而吞掉之后 CI 是绿的。

**核心断言**:一个观察能不能支持「这层兜底可以撤了」,取决于它与那层兜底的**历史触发频率是否
同量级**。如果在「机制仍然需要」这个假设下,这次观察本来也极可能给出同样的结果,那这次观察就
...
一个量级,在原来那个频率下一次这么短的 smoke 本来就极可能是 0 次,所以这个 0 在「修复生效」与
「修复没生效」两个假设下是同一个结果。
...
- 撤掉一个「防误判」机制之前,要有与它**历史触发频率同量级**的观测。自查办法:说「这一轮跑
  干净了所以可以删」之前,先算一遍「按历史频率,这一轮跑这么久本来该出现几次」——答案接近 0
  就说明这一轮什么都没证明;
- **优先用生产环境的自然观测,而不是一次专门跑的验证**。专门跑的验证时长通常远短于机制的触发
  周期,而自然观测(本仓是 nightly)的时长本来就是那个周期;
- 先问**撤错的失效方向**:如果失效是「多花点资源」,证据门槛可以低;如果失效是「真失败被当成
  噪声」或「一类输入退出比对」,门槛要高,因为这两种错误都不会自己报出来;
- checklist 上写着「评估并删除」时,**「评估结论是证据还不够」也是一次合格的执行** —— 把结论
  与理由写进文档、把撤除条件(**两条**:历史同量级的归零观测,且确认没有别的机制会产生同样症状)写成一个可观测的量(本轮是「nightly 上归零」),比为了勾掉一格而
  删掉一层还需要的保护划算。
...
#### 4.7a 给共用 helper 加「缺失时回退旧行为」的参数时,每个仍传默认值的调用点都要实测(2026-09-03,#252)
...
**实例(#252)**:`adjustExprList` 加了 `ends []int32` 并在缺失时回退,于是赋值右侧、`return`、泛型 for
三处先传了 `nil`。实测三处**全都**偏,而且偏在会 raise 的 GETTABLE 上,`FuzzOracleDiff` 对三者都报
`oracle :2:` vs `wangshu :1:`——等于把同一个缺陷原样留在另外三个语法位置,等着 nightly 开成三个新 issue。
补齐之后仍漏了第四处(`storeVar` 的单目标快速路径),由远端评审查出:那一处更隐蔽,因为它连参数都不是
「传了 nil」而是**用同一个 `line` 参数同时干了两件不同的事**(物化 RHS 与发射 store)。

**判据**:改完之后 `grep` 出该函数的**全部**调用点,逐个回答「传缺省值的这处,实测偏不偏」,答案必须来自
与参照实现的真值比对。给每个仍传缺省值的调用点各构造一个输入并断言与参照一致;偏了就说明它本来就在范围
内。附带一条:如果某个参数在函数体内被用在**语义不同的两处**(本例 `line` 既做物化行又做 store 行),那
它该拆成两个参数,而不是让调用方猜该传哪个——参见 §4.7b。反思
[[2026-09-03-issue252-discharge-line-is-lastline]] 教训 4。
...
#### 9.1b 检查「代码有某个属性」的测试不能作用在文件文本上,注释会满足它(2026-08-09,#236–#241)
...
`--connect-timeout` and `--max-time`」)。改成先截取 curl 那条调用、再在调用文本里查。

**为什么容易错**:shell 脚本没有 AST 可查,「检查代码属性」最方便的实现就是文本搜索,而文本搜索的
作用域默认是整个文件。更糟的是**这类脚本的注释里必然提到那些标志**(注释就是在解释为什么要限时),
所以注释与代码在文本上不可区分,而且注释写得越好、测试越容易假绿。
...
要读成两半:有 positive marker,而且它只能被被测代码满足。同时这也是
[[unreproducible-crasher-triage]]「一个防住某个数值的回归测试必须用变异实测确认」在**非数值属性**上
的形式(那条改回一个数值,本条删掉一个标志),而**变异实测是本轮唯一发现这个缺陷的手段** —— 那段
grep 逻辑读起来完全正常。反思
`memory/reflections/2026-08-09-issue236-241-curl-timeout-misread-as-enospc.md` 教训 5。
...
括号外、作为**调用参数**,而建标尺时只取了「索引作为被调用者」的形状),只有排在最后的 `make fuzz-oracle`
...
[[2026-09-03-issue252-discharge-line-is-lastline]] 教训 3。

#### 9.6a 加 skip 守卫时,两侧用例要沿「机制」的每个维度取,不能只沿被报的那个写法取(2026-08-11,#244)

**适用场景**:给差分 harness 加一条「这类输入两侧一起跳过」的守卫时。

**核心断言**:「该 skip 的确实 skip、紧邻不该 skip 的仍在比较」这个两侧检查是必须的,但它防住的只是
「在**已采样的那一格里**宽了或窄了」。**两侧用例的取样维度由机制决定,而选用例时手边只有那个 seed 的
形状。** 沿 seed 形状取样时,两侧检查会以为自己已经验过宽度。

**两种失效的反馈完全不对称,而且都不指着守卫说话**:
...
就平移,详见 [[cross-backend-semantic-fix-sweep]] 第八个刻度)。同时 `i >= 2147483648` 被一律跳过,而
`4294967297` 窄化成 `1`、两侧都返回 3——**窄化之后的值才是判据的输入,源码里的字面量不是**(与 §4.5c 那轮
差点按文本删掉 `4294967298` 那个 seed 是同一个机制)。

**判据**:加 skip 守卫时,写下**守卫的条件**与**机制的条件**,**逐字对照两者的自由变量**——机制里有的量
而守卫条件里没有,那个量就是漏掉的取样维度(本轮守卫只读 `i`,机制读 `i` 与 `e`)。两组用例都沿机制的
...
[[2026-08-11-issue244-oracle-segv-unpack-int32]] 教训 3。
...
- **写位置/行号类测试、要挑被测对象时**(§2.1) → 先问「这个写法会让编译器发射我关心的那条指令吗」;局部变量/已知常量往往不发射新指令,测不出位置类 bug,要用需要延迟加载的对象(全局、另一层索引、函数调用结果)
- **给一个新 tier / 新后端 / 新 build tag 组合加差分或 fuzz harness 时**(§2.2) → build tag 是累加的,老 harness 加上新 tag 之后**照旧编译、照旧通过而跑的是老路径**(`FuzzOracleDiff` 在 tier tag 下就是这样),所以要配一条能观测到进入新层的断言,并做三次变异实测:删掉进入那一层的调用必须变红、**把 payload 换成空的也必须变红**、以及 `grep` 那个标志的**读取点**确认断言真的有执行体。编译侧计数器(`PromotionCount`,force-all 会提升 main chunk 自己)与执行侧计数器(`NativeRunCount`)回答的是两个问题
- **某两层的一致性是经第三层传递得来时**(§2.3) → 问「有没有一条**直接**比这两层的路」;没有就把盲区如实写进文档,不要让传递结论读起来与直接结论等价(p3/p4 曾只有 tier == p1 且 p1 == PUC 这条链,它继承 p1 的全部盲区)。新加的直接轴要**沿用**中间那条链的跳过集合,新增的 tier 专属跳过当红旗看
...
- **给差分 harness 加一条 skip 守卫时**(§9.6a) → 把**守卫的条件**与**机制的条件**逐字对照自由变量:机制里有而守卫里没有的量就是漏掉的取样维度;两侧用例(该 skip 的 / 不该 skip 的)都沿机制的每个维度取,不要沿被报的那个写法取。自查:说「两侧用例都写了」之后再问「这两侧是沿哪个量取的两侧」(#244:两侧检查做了,而两侧都取自 `unpack({}, i)`,于是 `e > 0` 时窗口整段挪出守卫范围——太窄那一半让整个测试二进制 SIGSEGV,太宽那一半把两侧都返回 3 的输入 skip 掉)
