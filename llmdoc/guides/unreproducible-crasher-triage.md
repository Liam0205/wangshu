...
> 现在共用常量 `fuzzStepBudget`(`fuzz_budget_test.go`,值 `1<<19`,理由见下面「上限的余量」一节),
...
修法是把 harness 的 step budget 从 `1<<20` 减半到 `1<<19`(`fuzz_budget_test.go` 的
`fuzzStepBudget`,`fuzz_auto_test.go` 与 `fuzz_p4_test.go` 四处 `SetStepBudget` 共用它)。**产品代码
一行没改**——这是 harness 的余量问题,不是 VM 的缺陷。最慢子测试 1.34 秒 → 0.56 秒,六个 seed 全部
拿到 ≥1.8 倍余量,corpus 全量重放 5.5 秒 → 2.9 秒。回归
...
**降低 fuzz 预算不等于降低覆盖,但这句话要证明**。这一族**每一种写法在两个预算下都会触发**,fuzzer
走同样的路径、只是更早停下,缩小的是「单个输入能消耗多少 wall-clock」——而那恰好是看门狗量的东西。
不过这个推理本身也是一个待检验的断言:预算变小之后,某些输入可能在到达被测代码之前就退出,于是测试
变绿的原因悄悄从「两侧一致」换成「这个输入根本没跑完」,而表现是**测试照旧通过**。所以本轮**注入了
一个真实的 P1-vs-P4 分歧**(把升层侧的返回值截断),确认新预算下 harness 仍然 FAIL、撤掉注入之后
通过。判据:调紧 harness 的资源限制之后,用「注入一个必须被发现的缺陷」证明检测能力未受影响,而不是
只看现有用例是否仍然通过;方法论见 [[prove-the-path-under-test]] §9.2。

反思 [[2026-08-04-issue224-225-watchdog-margin]] 教训 2 / 教训 3。
...
- 反思实例:`2026-07-11-issue123-unreproducible-crasher-round`(七角度复现矩阵 + 分诊 + corpus
  ...
  ——`1<<20` 允许的 64 MiB 投射到 CI 是 12–13 秒对 10 秒看门狗,六个 seed 两个已超、四个余量 < 1.4 倍,
  预算减半到 `1<<19` 后全部 ≥1.8 倍,产品代码零改动)。
