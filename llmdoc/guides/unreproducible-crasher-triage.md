...
## 最小化要先剥掉「看起来相关」的外壳,逐层删、每删一次重跑(2026-08-27,#248)

版本核对确认「值得继续查」之后,下一步往往是拿着 seed 本身开始分析——而 seed 里往往裹着好几层看起来
相关的构造,注意力天然会先落到最显眼的那一层。

**核心断言**:seed 的表面结构不是它的因果结构。fuzzer minimize 出来的 seed 只保证「删了会不再复现」
这件事此前**没有被验证过**,不保证「每一层外壳都是必要的」。最显眼的构造(协程、`pcall`、嵌套括号)
会天然地把调查方向吸引过去,而它完全可能只是偶然裹在真正相关的那一层外面的壳。

**实证(#248)**:seed 是
`co=coroutine.create(function()(0\n)(A.A)end)print(coroutine.resume(co))`,带协程、带 `pcall`(经
`coroutine.resume`)、带括号包裹的调用。一开始盯着协程去查——把协程壳换成直接 `pcall`
(`print(pcall(function() return A\n.A end))`)之后分歧照样出现,协程是**噪声**。继续剥掉外层构造,
最终稳定在一个维度上:索引表达式跨了换行(`A\n.x`)。这个维度与协程、`pcall`、调用完全无关,而
seed 的原始形式把它们全部混在一起。

**判据**:最小化时逐个删掉外层构造(协程壳、`pcall`、括号包裹、嵌套调用),每删一次都重跑,直到删
不动为止,不要对着 seed 的原始形式直接找因果。自查办法:说「这个 bug 与 X 有关」之前,先构造一个去掉
X 之后仍然复现的版本;构造得出来就说明 X 是噪声,得不出来才有资格把 X 列进因果链。

这与本 guide「一批 crasher 先问会不会被同一个改动一起解决」是同一族纪律的另一面:那条讲**多个** issue
之间要先判断是不是同一件事,分组的粒度是「issue」;本条讲**单个** issue 内部,seed 里哪些结构是根因
的必要条件、哪些只是偶然裹在外面的壳,同样需要逐一验证而不能从字面结构直接读出,粒度是「seed 内部的
一层构造」。反思 [[2026-08-27-issue248-index-line-across-newline]] 教训 3。
...
### 给一条报错的命令加超时,作用域要是它所在的步骤,不是那条命令本身(2026-08-19,#236–#241 后续)

上面两条管的是「一个步骤失败会不会静默吞掉后续」;本节管**同一个步骤内部**——一个步骤可能由好几条
命令组成,而给其中一条命令加超时容易被误当成「这一步已经界住了」。

**实例**:#236–#241 那一轮给取包脚本里的 curl 加了 `--max-time` / `--retry`,但装 oracle 那一步
实际是四条命令(`apt-get`、取包脚本、`make`、`check-oracle.sh`),只有当时报错的那一条被框住,另外
三条留在原地、无界。下一次(2026-08-18)真的从邻居命令那里挂了 5 小时 50 分,被 job 超时掐掉,几乎
丢了整轮覆盖——rolling-seed 与 auto-mode 被 skip、两个 go-fuzz 步骤根本没启动。普查之后发现无界的
步骤是三个不是一个(装 oracle、上传日志、triage)。

**为什么容易错**:「给报错的那条命令加超时」读起来像「这个问题解决了」——那条命令确实不会再无限挂,
而症状(报错、CI 红)确实消失了。但作用域选错了:命令级的 fix 只覆盖被点名的那一条,它的邻居、以及
**以后新加进这一步的任何命令**都不在覆盖范围内。这个错误没有任何自动信号——直到下一条无界命令真的
挂住,而那时症状是全新的(不同的命令、不同的日志),看起来像一个新问题。

**判据**:给某条命令加超时之前,先列出**同一执行单元(同一个 CI 步骤 / 同一段 shell 脚本)里所有
命令**,并优先选一个对「以后新增的命令」也生效的机制——CI 里是 step 级 `timeout-minutes` 而不是
逐条包 `timeout`,shell 脚本里是给整个函数/脚本设一个外层看门狗而不是给每个子调用分别加。这是
[[cross-backend-semantic-fix-sweep]] 「一个分支的所有出口都要对一遍」在**修复作用域**上的对偶:那条
讲读到一处代码后要看这个分支所有的出口,本条讲加一个修复后要看这个修复实际覆盖到了执行单元的哪个
范围。

### job 超时与 step 超时回答的是两个不同的问题,不能靠求和去配

**实例(同轮)**:在没有意识到上一条判据之前,曾经把 nightly job 的 `timeout-minutes` 抬到 420 分钟
连续推荐了四轮,而 GitHub 托管 runner 单 job 硬上限是 **360 分钟**——设 420 也会在 360 被杀,推荐
了四轮却从没查过这个平台限制。修正之后又犯了另一个错:据「所有 step 上限之和必须低于 job 上限」这个
框架,算出某个 leg 最坏情况 548 分钟、据此得出「只能大幅压缩」的结论。

**为什么容易错**:「总量不能超支」听起来是显然成立的约束,而它确实对**总量**成立——但 step 级
timeout 存在的理由完全不是控总量,是防止**单条命令**卡死悄悄吃掉 job 的全部预算。job timeout 才是
真正的预算(它衡量的是这一整个 job 实际会跑多久),step timeout 是给每个执行单元的独立上限(它衡量
的是「这一步正常情况下该多久,超过就大概率是卡死了」)。这两者不是同一个约束:五个 step 各设
100 分钟上限,job 设 350 分钟,只要它们不同时全部命中上限(通常如此),job 就不会超时,即使
step 上限之和是 500。按「求和」去配会把每个 step 的上限压得远低于它实际需要的宽度,逼出过度削减。

**判据**:给一批步骤 / 一批资源定上限前,先问一句「我要防的是**总量超支**,还是**单点卡死**」——
job 级超时 / 总预算类量防的是前者,step 级超时 / 单点资源上限防的是后者;是后者时,数值按**该步
实测耗时的 1.5~2 倍**取,不要试图让一批 step 上限求和后低于 job 上限。推荐任何配额数值之前,也要
先确认平台上有没有硬上限(GitHub Actions 单 job 360 分钟就是一例),写下数字的同时写下它的上界
来源,不要凭直觉抬。这条判据与「一个资源上限允许的量必须与外部看门狗差一个数量级」(见下「上限的
余量」一节)是同一物理事实在不同资源维度上的应用:那一节讲 fuzz per-input 看门狗与 step budget,
本节讲 job timeout 与 step timeout,而这一次犯的新错误是把「总量约束」错当成「单点约束」去套用,
方向刚好相反。

反思 [[2026-08-19-nightly-step-timeouts-and-budget]] 教训 1/2/5。
...
- 反思实例:`2026-07-11-issue123-unreproducible-crasher-round`(七角度复现矩阵 + 分诊 + corpus
  ...
  返回 3 的输入 skip 掉) ·
  `2026-08-19-nightly-step-timeouts-and-budget`(**两个问题都先诊断错了的一轮**:nightly job 一周内被
  job 超时掐掉三次。问题一是给报错命令加超时的作用域错——同一步里三条邻居命令留在无界状态,下一次
  从邻居那里挂了 5h50m;普查后发现无界的步骤是三个不是一个。问题二是量错了成本中心——连推荐五轮
  「压 auto-mode」而它只跑 2 分钟,真正占 job 87% 时长的 native go-fuzz 步骤从没被量过,根因是
  workflow description 里的「4 targets」是过时计数、实际是 6 个,6×45m=270 分钟与实测吻合。另有
  两条自我纠正:「抬到 420」不可能(GitHub 单 job 硬上限 360,推荐四轮从没查过平台限制),以及
  「step 上限之和必须低于 job 上限」这个框架本身是错的——job 超时是总预算,step 超时防的是单点卡死,
  两者不该按求和去配)。
