...
本仓现状如实记:#236–#241 那一轮修好了去重(见下),但**「本轮什么都没测」这句话仍然没有出现在
任何地方** —— infra issue 的 body 说的是失败原因的类别,不是「这一轮的探索预算是零」。已知缺口。
...
`${{ github.run_id }}`(三个 job 共享)之后,第一个报的 job 开 issue、第二三个评论。
...
**判据**:给自动化加去重时,先问「**这类事件的天然单位是什么**」(这里是一次 run),再选键。自查
办法:对每一维问「这一维变化时,是同一件事还是两件事」——答「同一件事」的维度不能进键(tier 变化
时仍是同一次上游抖动 → tier 不进键;run 变化时是两次事件 → run 进键)。配套要点:**被去掉的那一维
