...
- **P1 实测确认**:M14 时点 table 还走 Go map 旁路,仅靠去装箱三档基准已全数过 ≥2x 门槛(simple 2.28x / arith 2.40x / loop 2.30x);收尾轮落地 arena 原生表存储 + IC 后,simple/arith 升至 3.18x/3.10x 而 loop 持平(FORLOOP 回边不走 IC,长稳轮性能回收后现为 2.30x)——两段数据共同印证「去装箱是主力、IC 是表访问档的辅力」的结构(05 §3)。
