...
- P1:逐指令计一个单位 + 上述 CONCAT 字节计费,是各档里最精确的,但因为 CONCAT 字节计费,也**不再是纯逐指令精确**——即使没有循环回跳,单次大 CONCAT 也可能触发预算;
...
预算值因此在**不同档位与不同负载之间是近似值**,不是精确指令数。如果业务只是把预算当作「防脚本挂起 / byte-heavy 风暴」的兜底(绝大多数场景),各档粒度完全够用;需要更细的计量口径时以 P1 为准,但也要把 CONCAT 的字节计费算进去。
