...
> 依赖面:[p2-bridge](./p2-bridge/00-overview.md)(热度 / TypeFeedback / 可编译性——P2 是编译层共享前端,P4 直接消费)、
> [p3-wasm-tier](./p3-wasm-tier/00-overview.md)(分层结构的第一个用户;P4 继承其全部结构,只换发射后端)、
...
| 沿用 P2 可编译性闸门(F1..F7,[p2-bridge: 03-compilability §3 F1-F7](./p2-bridge/03-compilability-analysis.md)) | 为 varargs/coroutine/debug 补编译路径(原则 4:走 fallback 不做完备性) |
...
P4 的投机不自己采样,**直接消费 P2 的 `TypeFeedback`**([p2-bridge: 02-ic-feedback §4 TypeFeedback / 05-p3-p4-interface §4 P4Feedback](./p2-bridge/02-ic-feedback.md))。三棒接力在 P1/P2 文档早已铺好:
...
| feedback([p2-bridge: 02-ic-feedback §4 TypeFeedback](./p2-bridge/02-ic-feedback.md)) | 投机模板发什么 | guard 检查什么 |
|---|---|---|
...
P2 精心构造的「零 deopt 状态机」([p2-bridge: 04-try-compile-fallback §2 状态机 / §1 零 deopt](./p2-bridge/04-try-compile-fallback.md):升层单向、`TierGibbous` 是吸收态)在 P4 被**有意打破**——投机引入了「运行期假设可能被打破」,deopt 边必须出现:
...
对照表(承 [p2-bridge: 04-try-compile-fallback §1 fallback ≠ deopt](./p2-bridge/04-try-compile-fallback.md) 的表再延一列):
...
| 重编译后仍反复 deopt | 标 `TierStuck-speculation`:该 Proto 永久只发无投机的通用模板(仍比解释快——dispatch 税照省),或干脆永久解释。**吸收态,防抖**(同 [p2-bridge: 04-try-compile-fallback §7 不重试纪律](./p2-bridge/04-try-compile-fallback.md) 的不重试纪律) |

阈值数值与 [p2-bridge: 01-profiling §5 阈值定标](./p2-bridge/01-profiling.md) 同款待定:依赖真实负载校准,只影响时机不影响正确性。
...
| **异步抢占**(信号可落任意 PC) | 生成码在**循环回边插抢占检查点**:load `jitContext.preemptFlag`,置位则经 exit stub 退到边界(GC safepoint / 调度让出共用此点);直线段长度有界 ⇒ 不可抢占窗口有界 | [p2-bridge: 01-profiling §3 路线 B 旁路计数](./p2-bridge/01-profiling.md) 已确认回边是 P3/P4 翻译时自插检查点(不靠 P1 伪指令) | epoch interruption / checkexit 标志检查的生成模式 |
...
3. **混层调用走统一 CallInfo 协议**:JIT 函数 CALL 目标若也有 JIT 码,同世界内直跳;若是解释 tier,经 trampoline 出去由 crescent 执行,RETURN 后回来——协议即 P3 的解释器↔编译层互调协议([p3-wasm-tier/04-trampoline](./p3-wasm-tier/04-trampoline.md)),只是被调方从 wazero 换成本地码。
...
- **结构上不预设结论**:`gibbous/wasm` 与 `gibbous/jit` 同 tier 同接口([architecture](./architecture.md) §1 的包布局已并列;P2 的 `P3Compiler` 接口对两后端同形,[p2-bridge: 05-p3-p4-interface §2 P3Compiler 接口](./p2-bridge/05-p3-p4-interface.md)),**共存的结构自由度零成本保留**——这正是架构决策与策略决策的分离:结构允许共存,策略按上表数据裁剪,决策推迟到 P4 验收时用数据定,记入 [doc-gaps](../../llmdoc/memory/doc-gaps.md)。
...
- 编译执行的线程模型:升层触发线程同步编译(模板编译微秒级,可能够用)vs 后台 goroutine 编译 + 安装屏障——继承 P3 的同款决策([p3-wasm-tier/04-trampoline](./p3-wasm-tier/04-trampoline.md)),若 P3 被跳过则 P4 自决。
- 多 State 并发下 JIT 代码与 profile 的共享语义(承 [p2-bridge: 00-overview §9 风险与缺口](./p2-bridge/00-overview.md) 同款并发缺口)。
...
[p2-bridge](./p2-bridge/00-overview.md)(§3.4 TypeFeedback / §7.2 P4 供料接口 / §5 零 deopt 对比基线) ·
[p3-wasm-tier](./p3-wasm-tier/00-overview.md)(被继承的分层结构与互调协议;§8 去留决策的对象) ·
