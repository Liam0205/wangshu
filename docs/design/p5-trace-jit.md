...
> 依赖面:[p4-method-jit](./p4-method-jit.md)(全部系统基建的来源)、[p2-bridge](./p2-bridge.md)(热度/feedback 前端)、
...
| **megamorphic 调用点的稳定子集** | 解释器/分发器型脚本:一个调用点多目标,但热路径上目标稳定 | P2 feedback 标 `FBTableMega`/`FBUnstable`([p2-bridge](./p2-bridge.md) §3.4),P4 整点放弃投机 | trace 按**实际走过的路径**特化:每条 trace 只含一个目标 + guard,多态点裂成多条单态 trace |
...
录制起点选择沿用 LuaJIT 经验:**热循环回边**是首要起点(loop trace 是列内核收益主体),热 side exit 续录次之,函数入口 trace(up-recursion 等)最后——P2 的回边计数基建([p2-bridge](./p2-bridge.md) §2)直接复用,只是阈值另设。
...
| 热度计数 / TypeFeedback / 可编译性闸门 | [p2-bridge](./p2-bridge.md) | 复用:trace 阈值另设;feedback 辅助录制期决策;**NYI 黑名单沿用原则 4**——录制遇到不可处理形状(varargs/coroutine/debug 同款清单 + trace 特有的 NYI)弃 trace 走下层,不做完备性 |
...
[p2-bridge](./p2-bridge.md)(热度与 feedback 前端 / F1..F7 闸门 / §5.1 投机谱系表) ·
