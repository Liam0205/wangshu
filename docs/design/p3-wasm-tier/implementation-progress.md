...
**P3 实现:PW0 spike 闸门通过 + PW1-PW8 + PW4b 已交付(含 VS0 值栈 arena 化),PW9 总验收未启动。** PW2 trampoline 端到端;PW3 算术+比较;PW4/PW4b relooper + FORPREP/FORLOOP/TFORLOOP;PW5 表 IC inline 跳哈希;PW6 CALL/TAILCALL 跨层互调;PW7 CLOSURE/CLOSE(VARARG 白名单拒);PW8 线程级 tier 规则(协程不升层)。**全 38 opcode 除 VARARG 外全部可翻译;翻译器 + 跨层 + 升层门禁全链路就绪,仅剩 PW9 端到端总验收(V1-V18 + 性能 ≥2x)。** 设计文档集已齐备(00-08 共约 8800 行)。
...
- ✅ **PW8 线程级 tier 规则**(本提交;运行期守卫 `th==st.mainTh`(PW6 起已落地)+ 升层门禁 onMain bool;协程内 hot 函数保持 TierInterp,主线程同 Proto 升层;RW-6/RW-7 回填)
...
| RW-8 | [04 §6.4 / §9](./04-trampoline.md) | P2 [04 §4.4](../p2-bridge/04-try-compile-fallback.md) installGibbous 增「multi-State 共享 Proto trampoline 注册幂等保证」 | 🔶 **PW6 部分**(GibbousCode 经 bridge.gibbousCodes map 全局唯一,各 State trampoline 复用同一 code——幂等天然成立;但 multi-State 共享 gibbous 执行的并发验收留 PW9。P2 04 文档登记待 update 轮补) |
...
连带扩展)✅ 已落地(见 §2.2 表);RW-3(P1 08 正文)登记留文档轮。**P3 翻译器 + 跨层 + 升层门禁
全链路就绪,仅剩 PW9 端到端总验收。**
