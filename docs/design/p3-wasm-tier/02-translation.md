...
> Prior art 对照:HotSpot C2 的方法 JIT 也是「寄存器分配到机器寄存器」+「OOP map 帮 GC 找根」组合;**我们走 (A) 是因为没有 OOP map 这层基建**——P1 的 GC 是基于「root-set + arena 内偏移寻址」的简化模型([../p1-interpreter/06](../p1-interpreter/06-memory-gc.md) §1),把寄存器藏到不可见的 locals 里就破坏了根集合的简洁性。等到 P4 自管 codegen 阶段引入 stackmap,locals 才有用武之地([../p4-method-jit](../p4-method-jit.md) §3.2)。
...
- **R(A+3) 循环变量 v**:每次回边 `setReg(v, idx)` 后才被循环体读;若编译器静态分析循环体不读 v(纯递增),v 可不写回(进一步优化,留 PW9)。
...
- **locals 缓存的槽选择算法**(§2.3 / §3.5.2):FORLOOP 三槽(idx/limit/step)外是否扩展到其它循环局部热槽,留 PW9 实测后定标。基线全 memory-resident。
- **IC 快照失效后是否重编译**(§3.4 / [06-ic-feedback-consume](./06-ic-feedback-consume.md)):失效计数 → 重编译预算,与 P4 的再训练机制([../p4-method-jit](../p4-method-jit.md) §3.4)统一评估。P3 基线失效后该点永久走助手(正确但慢)。
- **批量 module + call_indirect 直调收益边界**(§1.2):每 Proto 一 module 的实例化开销实测后定批量阈值;批量编译的「免 Go 往返」收益 vs「一损俱损 + 批次划分复杂度」成本权衡,留 PW9 评估。
