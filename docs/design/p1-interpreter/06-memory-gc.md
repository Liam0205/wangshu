...
### 6.1 形态定稿:Lua 栈即根 + host 显式 handle

承 §5.1 的澄清,shadow stack 的**最终形态**定为**二元**:
...
不总有现成 Lua 栈槽可塞(且塞了要管理 top,易错);显式 handle 更贴合 Go 代码的 `defer pop` 习惯。**二元形态是性能与人体工学的最优切分。**
...
- **例外:多步构造的中间结果**。如 `CONCAT R(B)..R(C)` 右结合逐对拼接,中间串 `tmp = R(C-1)..R(C)` 在拼下一对前是临时值。
  ...
  **P1 定稿:中间结果一律落寄存器槽**(CONCAT 把累积串写回 `R(A)` 或临时槽,逐步推进),不依赖 shadow stack——
  这样解释器主循环**完全不碰 shadow stack**(§6.1 形态 1),热路径零登记开销。详见 [05](./05-interpreter-loop.md) 的 CONCAT 实现。
...
| **shadow stack 形态** | **二元**:Lua 执行现场「栈即根」(零登记)+ host 执行期「显式 push/pop handle」(§6.1) | 性能(热路径零开销)+ 人体工学(host 的 `defer pop`);补 Go 精确栈扫描看不见 arena 引用的盲区 |
