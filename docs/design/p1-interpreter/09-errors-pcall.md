...
> 报错说什么话、栈回溯长什么样、保护调用的四个内建(error/assert/pcall/xpcall)如何完成」。
...
// 便捷形式:解释器内在错误用 raisef 拼措辞 + 位置 + 变量名(§9.1)。
...
| 写 reg 的指令 | reg 的来源 | 推断出的名字 + 种类 | 错误后缀形式 |
|---|---|---|---|
...
**P1 完成策略**:`getobjname` **只逆推「肇事 pc 往前最近一条无条件写该寄存器的指令」**(不跨基本块、不处理
...
- **`bad argument` 系列**(stdlib host,#17):`bad argument #<n> to '<fnname>' (<reason>)`,如
  `bad argument #1 to 'setmetatable' (table expected, got number)`(07 §1.1 已用此格式)。`<reason>` 形式多样,
