...
- **NaN/Inf 文本**:Lua 5.1 在多数平台输出 `inf`/`-inf`/`nan`(或 `-nan`,平台相关)。这是**平台相关项**——官方 5.1 的 NaN/Inf 文本依赖底层 libc。望舒固定输出与**锁定的官方 5.1.5 参照平台**一致(§2.6 锁版本同时锁参照平台的 libc 行为);若 gopher-lua 输出不同(它用 Go 的 inf/nan 文本),落 §3.3 gopher 偏差行,以官方为准。**NaN 符号 spelling 例外(#173,2026-07-24 定性)**:PUC/x86/glibc 的 NaN 符号显示还取决于**NaN 位模式**(`0/0` vs `-(0/0)` vs `tonumber("nan")` vs `tonumber("-nan")`)× verb 大小写,是双向的——追一个 CPU-libc 组合的显示需改 VM 值层 NaN-boxing 对齐 x86,IEEE 754 又不赋 NaN 符号数值语义,收益低风险高;`internal/stdlib/stringlib.go` `cFormatSpecialFloat` 只硬编码最常见的负号路径(`0/0` 大写 verb → `-NAN`,小写 → `nan`),剩余 spelling 差异走 oracle harness 侧 **span-anchored** 机制化跳过——prelude 按每次 NaN 渲染事件把字节区间入 `__nan_spans` FIFO(print/io.write 数值 NaN 走 tostring 区间,string.format 有 NaN 入参时按输出中每个 nan/NAN token 位置入队),`__oracle_readout()` 以多行 header 传给 Go 侧,`internal/oracle.CompareOutput` walk 两侧输出:span 外的字节两侧 byte-equal(script 自出字面 `NAN`/`-NAN` 差异一定被抓到),span 内允许 `knownNaNSignDifference` 判定的 sign spelling 差异(含 printf 宽度一列偿还与对称 script padding)。**绝不在 `NormalizeOutput` 里全局替换 `NAN`/`-NAN`**——那会误吞脚本自己输出的普通字符串。已知 harness 限制记录在 `TestExec_NaNSpansKnownLimit`:Lua 5.1 无法区分 `string.format` 结果与同值 script literal 的 identity,scritp 若先 emit 与 format 结果同值的字面量会误消费 provenance 首项造成 sporadic Different(狭窄 collision,扩宽判据会误吞脚本字面差异所以是有意识的 trade-off)。详见 llmdoc/memory/reflections/2026-07-24-issue173-oracle-nan-known-diff-round.md。
