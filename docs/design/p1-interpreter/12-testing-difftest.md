...
| **UB 且参照实现直接崩** | 差分侧**跳过那个区间**(产品侧无事可做) | `unpack` 的索引在 int32 边界让 PUC **段错误**(#244):`luaB_unpack` 的 `n = e - i + 1` 是 int 运算,`n <= 0` 那个溢出检查只拦到「回绕成非正」的一半,回绕成**正的巨大值**时走进 `lua_checkstack(L, n)`,而它的 `size` 也是 int、条件再上溢一次,于是按巨大值扩栈崩掉;内嵌 oracle 拿到 SIGSEGV(栈迹在 cgo 里)、真的 `lua5.1` 二进制同样 dumped core,而望舒对整段抬 `too many results to unpack`、从不崩。skip 的执行体同为 prelude 的 sentinel。**边界口径见 §4.9f**——现行守卫区间两个方向都不对 |
...
### 4.9f 一条 skip 的**区间**要按机制定,不能按被报的那个写法实测出来(#244,2026-08-11)
...
窗口时抬 sentinel。现行守卫是 `i <= -2147483647 or i >= 2147483648`,注释写明这个窗口是**实测**出来的
...
补上 `e` 之后规则完全可推导:**`crash ⟺ i32 <= e32 且 (e32 - i32 + 1) > INT_MAX`**(`i32`/`e32` 是窄化
之后的 int32 值,`e` 默认 `#t`、由第三参数覆盖);20 组「先写下预测再实测」全部吻合,推导与实测本来一致,
当时不一致只是模型漏了一项。
...
| 方向 | 实例 | 实测后果 |
|---|---|---|
| **太窄**(漏崩) | `A(unpack({1,2,3},-2147483646))` / `A(unpack({},-1,2147483647))` | 经真实 `FuzzOracleDiff` 确认**整个测试二进制被 SIGSEGV 拿下**;真 `lua5.1` 二进制同样 dumped core。#244 的 crash 家族**还活着** |
| **太宽**(漏比对) | `unpack({1,2,3},4294967297)` 窄化成 `i32 = 1`、两侧都返回 3 | 经真实 `FuzzOracleDiff` 确认现在拿到 sentinel → **SKIP**,静默削掉覆盖面(§4.2 那类失效) |

**判据**:一条 skip 的区间要按**机制的条件表达式**定。把**守卫的条件**与**机制的条件**逐字对照自由变量
——机制里有的量而守卫里没有,那个量就是漏掉的取样维度(此处守卫只读 `i`,机制读 `i` 与 `e`);两侧用例
(该 skip 的 / 不该 skip 的)都要沿机制的每个维度各取两侧,而不是沿被报的那个写法取。**注意「两侧检查」
这个动作本身做过并不够**:#244 做了两侧检查,而两侧都取自同一种写法,于是它只验到「在已采样的那一格里
宽了或窄了」。两种失效的反馈还不对称——太宽表现为**测试通过**,太窄表现为**下一次 nightly 再开一个
issue**,都不指着守卫说话。并且注意**窄化之后的值才是判据的输入**,源码里的字面量不是(与 §4.9d 那轮差点
按文本把 `4294967298` 当同类 seed 删掉是同一个机制)。这与 §4.9b「skip 与产品规则的形状必须逐字对应」
同域:那条比的是 skip 与**产品规则**的形状,本节比的是 skip 与**参照实现机制**的形状。方法论见
`llmdoc/guides/prove-the-path-under-test.md` §9.6a 与 §4.5、
`llmdoc/guides/cross-backend-semantic-fix-sweep.md` 第八个刻度。
