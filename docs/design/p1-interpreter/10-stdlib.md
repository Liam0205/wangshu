...
| repl 类型 | 替换为 | 说明 |
|---|---|---|
| **string** | 模板串,`%1`-`%9` 替换为对应捕获,`%0` = 整体匹配,`%%` = 字面 `%`,**`%` 后的其它非数字原样吐出**(§6.5.2) | `gsub("hello","(l)","[%1]")` → `he[l][l]o` |
...
- **string repl**:模板里 `%n` 引用捕获——这是**替换期**的捕获引用(区别于 pattern 里的反向引用 `%1`)。`%0`/`%%`
  特殊,`%` 后的其它非数字**不报错、原样吐出**,末尾的裸 `%` 吐一个 NUL 字节(§6.5.2)。结果用 Go 缓冲
  累积(§3.3),末尾 intern。
...
#### 6.5.2 字符串 repl 里 `%` 的转义规则(#234,2026-08-05)

`add_s` 对 `%` 的处理只有一条规则加一个边界情况,而**望舒原先两个都没跟上**。PUC 的写法:
...
| 模板里的写法 | lua5.1 的结果 | 望舒原先 |
|---|---|---|
| `%%` | 一个字面 `%` | 同 ✅ |
| `%0` | 整体匹配 | 同 ✅ |
| `%1`–`%9` | 对应捕获,越界抬 `invalid capture index` | 同 ✅ |
| **`%` 后的其它非数字** | **原样吐出**,`gsub("a","a","%z")` 是 `"z"` | 抬 `invalid use of '%' in replacement string` ❌ |
| **末尾的裸 `%`** | `i++` 之后**越过长度读 `news[i]`**,读到 `lua_tolstring` 保证的 NUL 终止符,于是**每次匹配吐一个 NUL 字节**——`gsub("aaa","a","x%")` 是 `x\0x\0x\0`(hexdump 实测),`gsub("aaa","a","%",1)` 是 `"\0aa", 1` | 要求 `i+1 < len(rb)`,把末尾的 `%` 当成了字面量 ❌ |

`internal/stdlib/stringlib.go::st2gsubRepl` 现在两条都对齐:`%` 分支不再要求 `i+1 < len(rb)`,越界时取
零值(那正是 PUC 会读到的 NUL);其它非数字从抬错改成原样追加。用例见
`fuzz_234_test.go::TestGsubReplacementEscapeMatchesPUC`(四条改了行为的 + 三条必须不变的:`%%`、`%1`、
越界下标仍抬错)。

**两条判据**(方法论见 `llmdoc/guides/cross-backend-semantic-fix-sweep.md`):

1. **一个分支的所有出口都要对一遍,不只对 seed 走到的那个**(该篇「PUC 语义由 C 实现定义」第七个刻度)。
   #234 报的只有末尾 `%` 那一半,而「其它非数字原样吐出」这一半在 base 上同样分歧、是**既有缺陷**,
   只是没有 issue 记它。定位到参照实现的某一行之后,那一行往往是一个**分支的入口**,要把每个出口写成
   一行「这个出口在望舒是什么行为」逐行核对。
2. **照抄一个「越界读」怪癖的判据来自「我们比较的是什么」**(该篇「有定义 vs UB」表的第五格「有定义但
   可疑」)。PUC 读了一个超出长度的字节;这**不是** UB(`lua_tolstring` 保证那里有一个 NUL,读它有定义、
   结果确定),也不是参照实现自相矛盾,但显然是怪癖而非规则。决定照抄的是**口径**:oracle 逐字节比较,
   不照抄的话凡是以 `%` 结尾的替换串就永远不可比,而且那个差的字节还会流进长度、比较、表键。所以
   `st2gsubRepl` 的注释里写清了这是怪癖不是规则。
...
> **计费倍率的诚实说明(第五次审计)**:文档此前承诺「1 MiB ≈ 16K 步」(即 1 步 / 64 字节)。
> 实测最小可行预算(1 MiB,setup 基线 16385):`string.rep` 恰好 16385(准确);`string.format("%s", s)` 总计 32769(算子本身约 1 倍);`gsub("(a+)","%1")` 总计 65537——**算子本身 3 倍**,因为主串、每次匹配的替换、以及每个 `%n` 引用的捕获各记一笔。原因是**结构性的**:一个「拷贝穿透」的算子在两个不同作用域里
> 分别记了「消耗」与「产出」,而那是同一批字节——消耗侧的记账在 verb / 展开循环内部,产出侧在外层
> 增量处,二者无法在不放弃「记在真正写字节的那一层」这条纪律的前提下完全去重。
>
> **保留 2 倍并把它写进文档,而不是继续追求精确 1 倍**:1 MiB 在最坏(gsub 的 `%n` 形式)4 倍下占 1<<20 预算的 6.25%,
> 合法程序不会被拒(审计实测确认「没有现实写法被拒」);而真正要防的是 10 秒看门狗,四次
> `prog.Run` 在这个倍率下仍远在其内。追求精确 1 倍已经花掉两次尝试,收益只是文档数字对齐——
> 所以改文档。**上限的作用是拦住失控,不是精确计量。**
