...
> 实测最小可行预算:`string.rep` 恰好 16385(准确),而 `string.format("%s", s)` 与
> `gsub("(a+)","%1")` 是约 2 倍。原因是**结构性的**:一个「拷贝穿透」的算子在两个不同作用域里
> 分别记了「消耗」与「产出」,而那是同一批字节——消耗侧的记账在 verb / 展开循环内部,产出侧在外层
> 增量处,二者无法在不放弃「记在真正写字节的那一层」这条纪律的前提下完全去重。
>
> **保留 2 倍并把它写进文档,而不是继续追求精确 1 倍**:1 MiB 在 2 倍下占 1<<20 预算的 3.1%,
