...
> **搜不到邻居时按默认 1 小时处理，而找到的邻居偏移与基准**相同**时 delta 取 0**（该字段无效果）——这两条与 `maxStrides` 是耦合的：equal-offset 的 delta 取错时，把上限从 447 收到 381 实测是"持平到略差"，所以前几轮都没能单独定下其中任何一条。改对之后用 C 写的 `mktime` 参照程序实测：220 个时区 × 5 个年份 × 2 个月份 × `isdst` 真假共 4400 例**零不一致**，另在完全独立的留出集（另外 220 个时区 × 另外 5 个年份 × 另外 3 个月份，6600 例）上同样**零不一致**。原先登记的豁免因此撤掉了。 glibc 对其中一部分（`UTC`、`Asia/Shanghai`、`Asia/Kolkata`）仍按默认 1 小时响应 `isdst=true`，对另一部分（`Africa/Windhoek`、`Asia/Damascus`，它们是靠保留夏令偏移废除 DST 的）则不响应；区别来自 `mktime` 在 tzdata 历史里对附近 transition 的有界搜索，而 Go 不暴露 transition 表。先前那版无条件 `+3600` 的兜底让前一组对了、把后一组**弄坏了**，所以现在取更窄的行为并登记为豁免（`corners_test.go::exemptions`）。实测：9 个有 DST 的时区零差异，5 个无 DST 的时区在 `isdst=true` 上偏移 1 小时。
  都与 `lua5.1` 一致。
