...
> shadow stack 纪律的库级落地、base/string/table/math/os/io/coroutine 七个子库的完整清单与语义、
...
- **为什么兜底注册名**:`getobjname` 在复杂调用形态(`local f = string.format; f(...)`)推断不出 field 名,
...
## 3. shadow stack 纪律的库级落地(具体化 06 §6.3)
...
| `gcinfo()` | 返回当前 arena 使用的 KB 数(5.1 遗留) | `()` → number | △ | **5.1 有 `gcinfo`**(已废弃但 5.1 存在);= `collectgarbage("count")` 的旧形态(§4.6) |
...
func hostTonumber(vm *VM, th *Thread) int {
    if th.Arg(2) == value.Nil {                   // 无 base 形态
        v := th.Arg(1)
        switch {
        case value.IsNumber(v):
...
- **`gcinfo()`**(5.1 遗留全局):= `collectgarbage("count")` 的旧形态,返回 KB 数(**整数**,5.1 `gcinfo` 返回
...
| `load(func [,name])` | △ **简化**:5.1 的 `load` 取一个 reader 函数,反复调它拿源码片段拼成完整 chunk 再编译。P1 实现 reader 循环 + Compile | `Compile` + reader 重入(callLuaFromHost) | reader 函数重入(§1.4);P1 可先支持「reader 返回完整串一次」简化形态,完整分块记缺口 |
...
> **P1 定稿:module/package 系统是缺口或极简实现**。完整 5.1 `require` 需要:`package.path`(Lua 模块搜索路径
> 模板 `./?.lua;...`)、`package.cpath`(C 模块,望舒无 cgo → **不适用**,roadmap §0)、`package.loaders`(加载器
> 链)、`package.loaded`(已加载缓存)、`package.preload`(预加载表)。
>
> - **P1 极简形态(若做)**:实现 `package.loaded`(缓存)+ `package.preload`(宿主预注册模块)+ 一个「从文件系统
>   按 `package.path` 查找 `.lua` 文件并 loadfile」的纯 Lua 加载器。**不做 C 加载器**(无 cgo)。
> - **P1 缺口形态(更可能)**:`require` 只查 `package.preload`(宿主显式预注册的模块表)与 `package.loaded`
>   缓存,**不做文件系统搜索**(嵌入式宿主通常不从文件系统加载模块,而是宿主预注册)。`module` 函数 P1 不做
>   (它改全局环境的方式 5.1 特有且生态在 5.2+ 已不推荐)。
> - **为什么裁剪**:望舒首个目标宿主是**多运行时规则引擎**(roadmap §8 / overview),脚本由宿主提供与编译
>   (`Compile`),**不依赖运行期 `require` 从文件系统加载模块**。module 系统是「独立脚本程序」的需求,嵌入式
>   规则引擎场景弱。roadmap §5 原则 4「不可分析/不需要的形状走 fallback,不做完备性」——module 系统列入 P1 缺口。
>
> **记 §15.2 缺口**:`require`/`module`/完整 `package` 系统的 P1 范围(极简 vs 缺口)待与首个宿主需求确认;
> conformance 测试若含 require 用例,标注为 P1 已知限制。
...
- **5.1 的 `%q` 精确转义集**:双引号、反斜杠、换行(5.1 把换行转成 `\` + 真换行符)、`\0`、`\r`、其它控制字符
  转 `\ddd`。**这套转义是差分敏感的**(每个特殊字符的转义形态须与 5.1 逐字节一致),P1 **不编造**,以 Lua 5.1
...
| 函数 | 语义 | 参数 → 返回 | P1 | 备注 |
|---|---|---|---|---|
| `table.insert(t, [pos,] v)` | 插入 v(默认末尾,或指定 pos 处后移) | `(t, [pos,] v)` → 无 | ✅ | 两形态:`insert(t,v)` 末尾;`insert(t,pos,v)` 插入并后移(§7.2) |
...
- **`insert` 两形态**:2 参 = 末尾追加;3 参 = 指定位置插入并后移。**参数个数区分**(`NArgs()`),5.1 语义。
...
> **5.1 无整数子类型(roadmap §6),全排除**。`math.log(x, base)` 二参形态(5.2)——**P1 单参**(5.1)。
...
```go
// math.random([m [, n]]):三形态(对齐 5.1 math_random)。
...
- **三形态范围语义(5.1)**:① `random()` → 实数 `[0, 1)`;② `random(m)` → 整数 `[1, m]`;③ `random(m, n)` →
...
> **定稿口径:os 库的危险函数受 `Options` / openlibs 配置控制**(11 §1.2 `Options`,§10 openlibs)。P1 提供机制:
> openlibs 时可选择「安全子集」(只注册只读的 time/clock/date/difftime)或「完整 os 库」(含 execute/exit/文件
> 操作)。**默认倾向安全子集**(嵌入式宿主跑外部脚本的常见诉求);宿主信任脚本时显式开完整库。
>
...
> - **这呼应 roadmap §6 非目标精神 + 嵌入安全**:望舒是**嵌入式** VM,脚本运行在宿主进程内,任何「进程级操作」
>   都越界(脚本不该能杀宿主、不该能跑 shell)。安全子集是嵌入式 VM 的合理默认。
...
| `io.close([file])` | 关闭文件(默认默认输出) | △ | file 方法 `file:close` 的全局形态 |
...
>
> **为什么这样裁**:① 测试套主要需要 `print`/`io.write` 输出(对拍输出);② 嵌入式规则引擎(首个宿主)脚本极少
...
- **`io.open` 受文件系统访问控制**:嵌入式宿主可禁止脚本打开任意文件(沙箱)。P1 的 openlibs 可选「无文件 IO」
  子集(只 stdout/stdin/stderr,不含 `io.open`)。
...
```go
// internal/stdlib —— 打开标准库(luaL_openlibs 等价)。NewState 在 Options.OpenLibs=true 时调。
func OpenLibs(vm *VM, cfg LibConfig) {
    openBase(vm)                                   // base:print/type/pairs/...(§4);恒开(核心)
    openString(vm)                                 // string 库 + 设 string 元表 __index(§5.0)
    openTable(vm)                                  // table 库(§7)
    openMath(vm)                                   // math 库(§8)
    if cfg.OS    { openOS(vm, cfg.OSSafe) }        // os 库(§9;OSSafe=true 仅只读子集,§9.3)
    if cfg.IO    { openIO(vm, cfg.IOSafe) }        // io 库(§10;IOSafe 仅 stdout/stdin/stderr)
    openCoroutine(vm)                              // coroutine 库(§11,机制 08)
    // debug 库:09 §13 范围(traceback/getinfo 子集),按需
    if cfg.Debug { openDebug(vm) }                 // debug:traceback/getinfo(09 §13)
}

// LibConfig:库加载配置(经 wangshu.Options 暴露,11 §1.2)。
type LibConfig struct {
    OS     bool   // 开 os 库(默认 true,但危险函数受 OSSafe 控)
    OSSafe bool   // os 安全子集:仅 time/clock/date/difftime,不含 execute/exit/文件(§9.3)
    IO     bool   // 开 io 库
    IOSafe bool   // io 安全子集:仅 stdout/stdin/stderr,不含 io.open/popen(§10.4)
    Debug  bool   // 开 debug 库(09 §13)
}
```
...
| **`%q` 转义** | `string.format` `%q` | 每个特殊字符的转义形态 | **严格**(以 5.1 addquoted 为准,§5.2.2) | 12 |
