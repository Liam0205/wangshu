...
这与 wazero compiler engine 的组织同款([../roadmap.md](../roadmap.md) §7)——共享骨架 + amd64/arm64 各自 compiler——是「纯 Go 多架构机器码生成」的可行参照。
...
- **per-arch 子包从主包看是 emitter trait 的实现**(§2.4):主包不 import per-arch 子包,经 build tag 在编译期选定 emitter 注入。这与 wazero `internal/engine/compiler` 的 `arch_amd64.go` / `arch_arm64.go` 同形。
...
wazero compiler engine([../roadmap.md](../roadmap.md) §7 prior art)是「纯 Go 运行时机器码生成可行」的存在性证明,内部组织正是「共享骨架 + amd64/arm64 各自 compiler」:`internal/engine/compiler/compiler.go`(架构无关 IR 节点定义、CFG、寄存器分配抽象、guard 语义)+ `impl_amd64.go` / `impl_arm64.go`(per-arch 发射函数 + 寄存器分配实现 + 寻址模式 + 立即数编码)+ `encoder_amd64.go` / `encoder_arm64.go`(指令字节流编码)。
...
骨架经 build tag 选定(类比 wazero `arch_amd64.go` / `arch_arm64.go`):
...
- **Go 版本演进的影响**:Go ABI0 / G 寄存器协议 / 抢占机制随版本演进,callee-saved 集合可能变——CI 跑 Go 1.22 + 1.24 + tip(承 wazero 同款),变化时立即修。
