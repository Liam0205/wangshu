...
「投机叠在子集之内」的物理含义:即使某 Proto 在投机维度反复失败被标 `TierStuck-speculation`(永久只发通用模板,详 [./04-osr-deopt.md](./04-osr-deopt.md) §3.4),仍比解释快(dispatch 税照省)。**P4 的最坏场景 ≈ P3 的常规场景**——这是「投机叠加」非「投机替代」的工程红利。
