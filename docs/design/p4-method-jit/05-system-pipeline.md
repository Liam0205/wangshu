...
> [../p3-wasm-tier/05-safepoint-gc](../p3-wasm-tier/05-safepoint-gc.md)(三类 safepoint:分配点 / 层边界 / 回边——本文 §6 同款,只是「物理形态」从 wazero 内部机制切换为自管原生码)。
>
> P1 已铺垫的料:
...
> [../p1-interpreter/06-memory-gc](../p1-interpreter/06-memory-gc.md) §1.3(reloadFrame stk 重载纪律——P4 同款的 arena base 重载);
...
> [./06-backends](./06-backends.md)(amd64/arm64 后端实装——本文先给协议骨架,具体 asm/寄存器约定/指令编码留 06)。
>
...
P4 阶段:wazero 不再上场,P4 直接发射原生机器码 ⇒ 四项税**整笔账由 P4 自付**。这不是「重做 wazero」——本文 §1 逐项给出 P4 的概念方案,其每一项的物理可行性在 wazero 当前实装中已被验证,wazero 的 platform 子包、compiler engine、arch_amd64.s/arch_arm64.s 等是采石场(参考实现的代码出处)。
...
| **P3** | **依赖**:作为 Wasm 运行时,P3 翻译产物经 wazero 的 compiler engine 跑成原生码;arena 收养 wazero memory 作为共见物理内存([../p3-wasm-tier/03-memory-model](../p3-wasm-tier/03-memory-model.md));四项税由 wazero 内部机制承担 | 四项税外包 |
| **P4** | **采石场**:wazero 不在 P4 build 链路上(若 §6 决定退役 P3,则连构建产物也不带),但其 platform/compiler engine/arch_*.s 是 P4 系统管线每件套的参考实现来源 | 四项税自付 |

「采石场」是工程上的精确含义——**P4 的 trampoline asm、mmap+mprotect 翻面序列、icache flush 序列、自管栈切换 SP 序列,都从 wazero 同位实装移植/借鉴**(Apache 2.0,license 兼容)。每税/件,本文都给出 wazero 对应实装位置的指针。
...
| **§6** | 两个 safepoint 在 P4 的物理形态(承 [../p3-wasm-tier/05-safepoint-gc](../p3-wasm-tier/05-safepoint-gc.md) 三类布点) | 本文 |
| **§7** | 不变式清单(本文承担 5 条) | 本文 |
| **§8** | 风险与开放问题 | 本文 |
| **§9** | 回填请求(若有) | 本文 |

**关键边界**:本文是「P4 系统管线」的单一事实源——四项税逐项方案、四件套、jitContext、世界边界三不变式、trampoline 三出口、arena base 重载、两 safepoint 物理形态全部在本文定稿;具体 asm 与 jitContext/机器栈精确布局留 [./06-backends](./06-backends.md);OSR exit 物化语义留 [./04-osr-deopt](./04-osr-deopt.md)。
...
#### 1.1.5 wazero 采石场:compiler engine native call 栈切换

参考实装位置:wazero `internal/engine/compiler` 的 native call 路径——每次进 Wasm 函数前,compiler engine 把执行栈从 goroutine 栈切到 wazero 自管栈(`callEngine.stack`,Go 堆 `[]uint64`),用一段 arch-specific 入口 asm 调整 SP 寄存器。其 amd64/arm64 入口 asm(`arch_amd64.s` / `arch_arm64.s`)的 SP 切换 + caller-saved 保存序列即 P4 trampoline「进」的直接蓝本。

> **风险标注**:wazero 自管栈的元素是 `[]uint64`(对齐 wazero IR 的 64-bit 栈),P4 自管机器栈的元素是 `[]byte`(机器寄存器宽度 + spill 槽对齐由 per-arch 决定,留 [./06-backends](./06-backends.md))。形态相似但元素类型不同——移植时要按 P4 的发射后端约定重整布局,不是字节对字节抄。
...
「不可抢占窗口」= 两次回边检查点之间的最长直线段长度。Lua 字节码的直线段(回边到回边、调用到调用)长度天然有界([../p2-bridge/03-compilability-analysis](../p2-bridge/03-compilability-analysis.md) F5 大函数闸门已限编译单元尺寸),且每次跨层调用 / 助手调用 / 分配点都是隐式抢占点(都经 trampoline 出 ⇒ 回 Go 世界 ⇒ Go runtime 可抢占)。所以抢占延迟有上界,符合 Go runtime 对协作式抢占的隐含约定(等价 STW 延迟数十微秒到毫秒级,与 wazero 同档)。

#### 1.2.4 与 P3 PW9-a gcPending inline 的对位

P3 在 [../p3-wasm-tier/05-safepoint-gc](../p3-wasm-tier/05-safepoint-gc.md) §1.3 已定:回边 inline 一次 `i32.load $gcPending` + 条件跳助手——同款形态,只是物理通道是 wazero linear memory 加一次条件分支,P4 是机器寄存器加一次条件分支。`gcPending` 与 `preemptFlag` 在 §3.3 的 jitContext 是相邻字段(都是 i32),回边检查点可一次 load 两字 + 单次 test(per-arch 优化,留 [./06-backends](./06-backends.md))。
...
#### 1.2.5 与 context cancellation(WithCloseOnContextDone 同款)的协作终止

宿主侧的取消(`Program.Call(ctx, ...)` 的 ctx done)在 P4 经同一通道生效:当 ctx 被取消,Go 侧的取消钩子置 `jitContext.preemptFlag` ⇒ 下一次回边检查点命中 ⇒ JIT 退到边界 ⇒ Go 侧助手发现 ctx 已 done ⇒ 抛 cancel error 经 status 链冒泡。这是 issue #4 落地的 `SetCancelHook` 机制([../p1-interpreter/12-testing-difftest.md](../p1-interpreter/12-testing-difftest.md) 公共面契约)在 P4 的物理兑现——P3 经 wazero 的 `WithCloseOnContextDone`(wazero API),P4 自付同款机制,通道是 jitContext.preemptFlag。

#### 1.2.6 wazero 采石场:epoch interruption / checkexit 标志检查

参考实装:wazero `internal/engine/compiler` 的 epoch interruption / `checkExit` 机制——同样在循环回边 emit 一次 load 标志 + 条件分支跳到 exit handler。wazero 的标志位住 `callEngine` 字段,P4 的住 `jitContext`,语义同位。
...
P1 第一天就定:**arena 内对象互引用是 GCRef = 48-bit 字节偏移,不是 Go 指针**([../p1-interpreter/01-value-object-model](../p1-interpreter/01-value-object-model.md) §2 与本文 §1.4 写屏障白赚的同款根因)。这条纪律本就为「不让 Go GC 看到 arena 内部图」设计,顺便让 arena 内部引用对栈移动也免疫——arena 对象间用偏移寻址,arena 整体在 Go 堆上的地址变化(grow 后 realloc 到新 backing)不影响内部引用,只需 Go 侧重取一次 backing slice 视图([../p1-interpreter/06-memory-gc](../p1-interpreter/06-memory-gc.md) §3)。
...
参考实装:wazero `internal/engine/compiler` 的 native entry 只传一个 `*callEngine` 指针入口,生成码经该指针读 module 状态(linear memory base、function table、tier 标志等)。P4 jitContext 是 wazero callEngine 的同位物——单一 Go 堆对象、固定寄存器、间接寻址。
...
#### 1.4.4 wazero 采石场:linear memory 即同款自管值世界

参考实装:wazero linear memory 是 `[]byte` backing,Wasm 生成码所有 i32.store/i64.store 指令的目标都是该 backing 内偏移——Go GC 完全不可见。P4 arena 与 wazero linear memory 在「自管值世界 ⇒ 写屏障白赚」这条上**物理同源**:都是「Go 堆上一段不含指针的字节数组」,Go GC 把它当普通整数数组看,内部如何被 JIT 写入与 Go GC 无关。这也是 [../p3-wasm-tier/03-memory-model](../p3-wasm-tier/03-memory-model.md) §1.2「为什么是『收养』而非『双份』」的对偶面——值世界一旦统一,写屏障同时对两个执行层都白赚。
...
|---|---|---|---|---|
| **GC 精确栈扫描** | 自管机器栈 + trampoline 切 SP,边界 syscall 语义 | 调用链状态全在 arena(P1 05 §7) | trampoline 进出 asm + 自管栈池化 | compiler engine native call 栈切换 |
| **异步抢占** | 回边插 preemptFlag 检查 + 经 trampoline 让出 | 解释器 opcode 末尾检查机制(P1 05 §5.3) | 回边检查点指令(几条)+ exit stub | epoch interruption / checkexit 标志 |
| **栈移动** | jitContext 经固定寄存器,Go 堆对象不移动 | GCRef 非 Go 指针(P1 01 §2) | 固定寄存器约定 + 间接寻址纪律 | trampoline 只传 callEngine 指针 |
...
代价:小 Proto 单独占一页(最小分配粒度 = OS 页),内存浪费。缓解:P2 F5 大函数闸门已限编译单元尺寸下限(太小不升层),最小升层目标尺寸的 Proto 至少能填半页;若实测浪费超阈值再做合并(留 [./06-backends](./06-backends.md) tuning)。

#### 2.1.3 释放策略与 GibbousCode.Dispose 协议对接(承 [../p2-bridge/05-p3-p4-interface](../p2-bridge/05-p3-p4-interface.md) §6.1)

`GibbousCode` 接口在 P2 已定:`Dispose()` 释放后端资源。P3 实装下 Dispose ⇒ wazero module Close;P4 实装下 Dispose ⇒ munmap 当前段 + 释放 jitContext。
...
参考实装:wazero `internal/platform` 的 `MmapCodeSegment` 函数族——平台分流(linux_amd64.go / linux_arm64.go / darwin.go / windows.go / freebsd.go),封装了「mmap RW + 写入 + mprotect RX」三步。每平台对系统调用的 syscall 包装是 P4 直接可移植的代码。
...
参考实装:wazero `internal/platform/mmap_*.go` 各平台的 mmap 子文件——尤其 `mmap_darwin_arm64.go` 实现了 MAP_JIT + pthread_jit_write_protect_np 的 Go 端调用骨架(经 `syscall.Syscall1` 直调 `pthread_jit_write_protect_np`,不依赖 cgo),P4 macOS arm64 节直接借鉴。
...
参考实装:wazero `internal/engine/compiler/arch_arm64.s` 与 `internal/platform` 的 arm64 cache flush 路径——wazero 同样在每段 Wasm function 编译完后做一次 IC IVAU + DC CVAU 序列。其 cache line size 检测、按 line 循环、屏障指令选择(DSB ISH vs DSB SY)是 P4 直接可借鉴的。
...
#### 2.4.4 trampoline 形态参数化:per-arch(具体 asm 在 06-backends)
...
#### 2.4.5 wazero 采石场:compiler engine arch_amd64.s 等

参考实装:wazero `internal/engine/compiler/arch_amd64.s` 与 `arch_arm64.s` 的 native entry / exit asm——即「Go 调用方 → wazero 自管执行环境 → Go 调用方」的两段 stub,寄存器保存 + SP 切换 + 跳入 / 返回的完整序列。P4 可以直接以这两段为骨架做 wholesale 移植,只需替换 ABI 寄存器选用与 jitContext 字段偏移。
...
`jitContext` 是 P4 系统管线的核心数据结构——Go 堆对象,生命周期与 Thread 同(每 Thread 一份,与 P3 wazero 的 callEngine 同位)。完整字段(per-arch 扩展留 [./06-backends](./06-backends.md)):
...
P4 阶段的 arena 物理形态:
- **P3 退役场景**(参 [./07-p3-retirement.md](./07-p3-retirement.md) §3 缺省倾向):arena backing 切回 P1 形态,纯 Go 堆 `make([]uint64, n)`(承 [../p3-wasm-tier/03-memory-model](../p3-wasm-tier/03-memory-model.md) §0.4「P4 唯一的差异是不再经 wazero memory 中介」),`BackingFn` 切回 `DefaultBacking`;
...
**P3 与 P4 在「跨层调用走统一 CallInfo」这条上同形态**(承 [../p3-wasm-tier/04-trampoline](../p3-wasm-tier/04-trampoline.md) §0.4 协议规范);P4 唯一差异是「同世界内直跳」可优化(P3 PW10 R3 已落地的 call_indirect 同款优化的 P4 对位,留 [./06-backends](./06-backends.md))。
...
helper 表(jitContext.helperTab)的典型条目(承 [../p3-wasm-tier/04-trampoline](../p3-wasm-tier/04-trampoline.md) §3.3 helper 列举,P4 同款集):
...
| ...其它 | (per-opcode 慢路径) | (P1 同款) | (per-opcode 同款) |

helper 集与 P3 同款,正反映 [./02-template-direction.md](./02-template-direction.md) §4 P4 边界:**只换执行引擎,不重新发明慢路径逻辑**。Go 侧的 helper 函数与 P3 imported helpers 是同一份代码(同样调 P1 的 `Arena.Alloc` / `metamethodCall` / 等),区别仅在「物理通道」:
...
- (b) helper 内的 Alloc 调用越界(常见,与 P3/P1 同款触发)。
...
P3 wazero memory grow 后 Go 侧需重取 backing slice 视图([../p3-wasm-tier/03-memory-model](../p3-wasm-tier/03-memory-model.md) §1.7);P4 同款机制——只是「视图」是机器寄存器中的 arenaBase 值。两层在「grow 后须重取基址」这条物理上同源。
...
P3 PW6 落地了「`h_call` 返回 i64 同载新 base 刷新 + 错误负哨兵」机制,根因是嵌套调用的 growStack 重定位值栈段在 arena 内的位置([../p3-wasm-tier/04-trampoline](../p3-wasm-tier/04-trampoline.md) base 刷新协议)。
...
[../p3-wasm-tier/05-safepoint-gc](../p3-wasm-tier/05-safepoint-gc.md) §1 已定的三类 safepoint 模型在 P4 一字不改,只是物理形态从「Wasm 直线代码 + imported helper」切换为「原生码 + trampoline + helper」。
...
### 6.2 P4 自付:每类 safepoint 在 P4 的物理形态
...
**默认策略**:每个回边一次检查(per-iter 一次),与 [../p3-wasm-tier/05-safepoint-gc](../p3-wasm-tier/05-safepoint-gc.md) §1.3 P3 同款。若实测吃性能(amd64 实测后定),可优化:
...
**P4 不直接处理 Go runtime 的抢占信号**——它经 `preemptFlag` 的协作机制让出。这条与 [../p3-wasm-tier/implementation-progress.md](../p3-wasm-tier/implementation-progress.md) §0.1 PW0 spike 修正的事实(wazero 同款不可被异步抢占,靠 ctx 协作)同源:**P4 在这条上不弱于 wazero,只是把同款解法自付了一次**。
...
物理后果:四项税防线由 trampoline 边界守住,JIT 内不需要为每税单独发明对策。违反此不变式的提案直接判否——它会让 GC 精确栈扫描税(§1.1)与栈移动税(§1.3)同时失效。
...
> P4 帧与 crescent 帧、P3 wazero 帧共享同一 CallInfo 结构(bit50 `callStatus_gibbous` 在 P4 同样标 1);跨层调用走 [../p3-wasm-tier/04-trampoline](../p3-wasm-tier/04-trampoline.md) 同款协议,只换物理通道。
...
回边检查点的密度调优是 Go runtime 版本敏感的——Go 1.x 系列的抢占机制随版本演进(1.14 引入异步抢占、1.21 调度器优化等),P4 行为可能随 Go 版本漂移。wazero 的跟进历史(同款问题)是预警源:每次 Go 大版本升级前,P4 须跑 GC 压力 fuzz + 长循环抢占测试,确认行为未漂移。
...
承 [../p2-bridge/00-overview](../p2-bridge/00-overview.md) §9 同款并发缺口。
...
- **自管机器栈精确布局**:§3.4 是逻辑布局,实际 size / red zone / guard 页机制 / 溢出检测 per-arch 调,留 [./06-backends](./06-backends.md);
...
- **macOS arm64 MAP_JIT 的 per-thread 状态与 goroutine 调度交互**:§2.2.2 给方向,实际 pthread_jit_write_protect_np 调用与 goroutine.lock 的协调留 [./06-backends](./06-backends.md);
- **自管栈池化与 Thread 生命周期对齐**:每 Thread 一份机器栈 vs 全局池,留 [./06-backends](./06-backends.md) tuning;
...
- P3 [05](../p3-wasm-tier/05-safepoint-gc.md) 三类 safepoint ⇒ 本文 §6 直接复用,只换物理形态。
...
[./06-backends](./06-backends.md)(amd64/arm64 后端实装 / per-arch trampoline asm / jitContext 精确字段 layout / 自管机器栈精确布局 / helper 表 ABI / icache flush asm / MAP_JIT 调用骨架) ·
...
[../p3-wasm-tier/05-safepoint-gc](../p3-wasm-tier/05-safepoint-gc.md)(三类 safepoint 模型,P4 同款) ·
[../p3-wasm-tier/implementation-progress.md](../p3-wasm-tier/implementation-progress.md)(§0.1 PW0 spike 修正:wazero async-preemption-unsafe,P4 同款) ·
