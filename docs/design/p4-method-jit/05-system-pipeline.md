...
type JITContext struct {
    ...
    // ----- 第 4 组:自管栈占位(实现保留字段,PJ10 阶段未真的打开切 SP) -----
...
- **第 4 组**:自管机器栈的 backing 字段。实现保留字段,但 PJ10 native emit 当前不切 SP——mmap 段直接跑在 Go goroutine 栈上,靠 §4.3 exit-reason 协议出去而不是靠 SP 切换。
