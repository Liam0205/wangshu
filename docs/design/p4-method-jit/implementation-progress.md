...
**当前 PJ5 现状**:简化形态全走 `host.CallBaseline / TailCall / Self` 等 baseline doCall 路径(byte-equal P1 解释器),**无投机失败 deopt 路径**——本批是 OSR exit 协议的工程基础,等段内 EmitCallInline 投机模板真接入时激活。
