...
**P4 实现:PJ0 包骨架已完成 + PJ1 spike 检查 🟢 + amd64 工程组件已完成**(2026-06-25,详 §5/§6)。**设计文档集已齐备**(00-08 + implementation-progress 共 10 文件约 8200 行,审查后口径):

- [00-overview](./00-overview.md)(319 行):文档地图 + PJ 里程碑 + 人月分解 + 跨文档定稿决策速查
- [01-launch-judgment](./01-launch-judgment.md)(810 行):启动检查 + luajc 档锚点 + 立项决策树
...
- ✅ **P4 PJ0-PJ7 接上扩展全过线**(2026-06-25/26):
  ...
  - **PJ7 接上 ~25 类形式 byte-equal**(getter 族 + setter 族 + 比较折叠族)
  - 14 个 host helper 全实现,P4HostState 接口完整
  - pc off-by-one 修复 + 多行错误消息 byte-equal 实证
- ⏳ **P4 立项判定 PJ2+ 启动**:当前需要外部输入(承 [01-launch-judgment §3](./01-launch-judgment.md))——三条硬前置:① 真实宿主负载需求(首个目标宿主规则引擎的列内核确认触及 luajc 档需求)② 资源到位(+1-2 人年人力承诺)③ 设计文档齐备(已就绪)。**P3 现状是 P4 立项的双向信号**:**loop 2.95x over P1(= 7.2x over gopher-lua,已超 luajc 档 4.4x over gopher-lua)**——列内核 loop 形式 P3 已超 luajc 档;但 **table 0.88x / call 0.52x / mixed 0.99x 仍 ≪ luajc 档列内核形式**——P4 立项动机**不在 loop 而在非 loop 形式**(call 0.52x 是 bench kernel 结构性架构边界);⇒ **若真实宿主负载主要是 loop 形式可暂不立项,若需 ≥luajc 档列内核能力(尤其 table/call/mixed 路径)则立项**(详 [01 §3.2 反向问题 / §3.3 P3 现状对照](./01-launch-judgment.md))。
...
- ⏳ **2 项余**(全是条件性 / 用户决策性):
  ...
  - RJ-34 验收基准分档加宿主真实负载形式(等 PJ11 用户定下来真实宿主负载实测)
...
| # | 来源 | 内容 | 兑现 PJ |
|---|---|---|---|
| RJ-1 | [08 §13.1 RB-1](./08-testing-strategy.md) | [P1 12 §3.8 Runner 抽象表](../p1-interpreter/12-testing-difftest.md):`WangshuGibbousJIT` runner 注释从「P3+ 新增,本文预留,不实现」拆为「P3 已实现 + P4 待实现」两行,避免 P4 实现时该注释跨度过广 | ✅ 已完成(2026-06-28) |
| RJ-2 | [08 §13.1 RB-2](./08-testing-strategy.md) | [P1 12 §7 P4 行](../p1-interpreter/12-testing-difftest.md):现稿已预留 P4 行,加链接指向 [P4 08 §4 / §5](./08-testing-strategy.md) 使 P4 字段从「文字承诺」升级为「具体口径接入」 | ✅ 已完成(2026-06-28) |
| RJ-3 | [04 §12](./04-osr-deopt.md) | [P1 05 §7 doCall / callResult 枚举](../p1-interpreter/05-interpreter-loop.md):新增 `callDeoptResume` doCall 出口——收到 GibbousCode.Run 返回 status=2 时的处理(reloadFrame + 续跑同帧);P4 阶段新增,P1/P2/P3 不需要 | ✅ 已完成(2026-06-28) |
| RJ-4 | [04 §12](./04-osr-deopt.md) | [P1 05 §9 错误冒泡纪律](../p1-interpreter/05-interpreter-loop.md):OSR exit 路径不应设置 `state.pendingErr`——exit 是「投机失误」非「语义错误」,不与错误冒泡互斥;若同发以错误优先(承 [P4 04 §7.4](./04-osr-deopt.md))| ✅ 已完成(2026-06-28) |

### 2.2 对 P2 文档的回填请求

> **方案 A 决议(承 [04 头注 + §5 + §12](./04-osr-deopt.md))**:**P2 三态枚举 `TierInterp/TierGibbous/TierStuck` 不变,P4 投机生命周期由 P4 自管(`internal/gibbous/jit` 端 map `p4SpecState[proto]`,枚举 `P4Speculative / P4Deoptimized / P4StuckSpeculation`)**。原 RJ-8(P2 04 §2.1 加 `TierGibbousJIT/TierStuckSpeculation` 枚举)/ RJ-9(P2 01 §2.2 加 `ProfileData.deoptCount`)/ RJ-10(P2 01 §2.2 加 `ProfileData.recompileCount`)三项**全部撤回**——P4 自管投机生命周期不需 P2 实现改动。RJ 总数从 37 → 34。

| # | 来源 | 内容 | 兑现 PJ |
|---|---|---|---|
| RJ-5 | [03 §11.1 RB-1](./03-speculation-ic.md) | [P2 05 §5.5 P4 deopt 兜底与重训练](../p2-bridge/05-p3-p4-interface.md):与 [P4 03 §7.3 / §7.4](./03-speculation-ic.md) 字面同源化;明确 P2 接受 RequestRefresh 后 CAS 装新 feedback,P3 旧指针仍可读 | ✅ 已完成(2026-06-28) |
...
| RJ-7 | [03 §11.1 RB-3](./03-speculation-ic.md) | [P2 05 §5.6 P4 不依赖 P2 状态机硬纪律](../p2-bridge/05-p3-p4-interface.md):[P4 03 §8](./03-speculation-ic.md) 直接复刻该节,P2 文档侧加引用「本节具体形式见 P4 §3 §8」| ✅ 已完成(2026-06-28) |
...
| RJ-12 | [07 §11.3](./07-p3-retirement.md) | [P2 04 considerPromotion 接口扩展](../p2-bridge/04-try-compile-fallback.md):增加平台维度,P4 平台走 jit promote,P3 平台走 wasm promote——**仅在 P4 验收决策为「留中层」时触发**;若决策为退役则无需 | ✅ **自动消解**(2026-07-01 D2 决议为「主动保留」而非「留中层」,承 [07 §5.6](./07-p3-retirement.md);considerPromotion 无需扩展平台维度,主动保留形式下 build tag 层已足够隔离 P3/P4 build 面) |
| RJ-13 | [08 §13.3 RB-6](./08-testing-strategy.md) | [P2 06 §1 验收口径总表](../p2-bridge/06-testing-strategy.md):P2 V1-V22 在 P4 build 下不豁免——[P4 08 §0.2](./08-testing-strategy.md) 字面承诺,P2 06 加「P4 build 下 P2 V1-V22 仍跑」纪律 | ✅ 已完成(2026-06-28) |
| RJ-14 | [08 §13.3 RB-7](./08-testing-strategy.md) | [P2 05 §6 GibbousCode.Run status=2](../p2-bridge/05-p3-p4-interface.md):现稿已预留,加引用「P4 测试如何验 status=2 路径见 P4 §8 §4(V19 OSR 状态等价)」| ✅ 已完成(2026-06-28) |

### 2.3 对 P3 文档的回填请求

| # | 来源 | 内容 | 兑现 PJ |
|---|---|---|---|
| RJ-15 | [01 §9.1](./01-launch-judgment.md) | [P3 01 §0.3 检查双向性](../p3-wasm-tier/01-spike-gate.md):补对位指针指向 [P4 01 §0.3](./01-launch-judgment.md)——P3 spike 检查双向性与 P4 立项判定双向性同源逻辑 | ✅ 已完成(2026-06-28) |
| RJ-16 | [01 §9.1](./01-launch-judgment.md) | [P3 01 §5.4 跳跃路径下设计资产复用表](../p3-wasm-tier/01-spike-gate.md):补指针指向 [P4 01 §2.4](./01-launch-judgment.md)——P3→P4 设计资产继承清单的两个视角互补 | ✅ 已完成(2026-06-28) |
| RJ-17 | [01 §9.2](./01-launch-judgment.md) + [07 §11.1](./07-p3-retirement.md) | [P3 00-overview §1 边界表](../p3-wasm-tier/00-overview.md):补一行指向 [P4 01](./01-launch-judgment.md) + [P4 07](./07-p3-retirement.md)——P3 总览只列 P3 实施层(同 tier 不同后端),立项检查 + 去留决议两个 P4 视角未点到 | ✅ 已完成(2026-06-28) |
| RJ-18 | [07 §11.2](./07-p3-retirement.md) | [P3 01 §0.2 检查单点决策不可绕过](../p3-wasm-tier/01-spike-gate.md):补对位指针指向 [P4 07 §0.3](./07-p3-retirement.md)——P3 spike 检查(开工)与 P4 P3 去留检查是 P3 生命周期上的两个检查,形式平行 | ✅ 已完成(2026-06-28) |
| RJ-19 | [03 §11.2 RB-4](./03-speculation-ic.md) | [P3 06 §1.1 P3/P4 物理同形 ≠ 语义同义](../p3-wasm-tier/06-ic-feedback-consume.md):[P4 03 §0.2 / §5.3](./03-speculation-ic.md) 把这条对偶面在 P4 视角全展开,P3 06 加引用「P4 视角对偶兑现见 P4 §3 §5」| ✅ 已完成(2026-06-28) |
| RJ-20 | [03 §11.2 RB-5](./03-speculation-ic.md) | [P3 06 §6.1 IC 失效是否触发重编译,留 P4 评估](../p3-wasm-tier/06-ic-feedback-consume.md):[P4 03 §7.3](./03-speculation-ic.md) 给 P4 端重训练协议——P3 IC 失效永久 miss 与 P4 投机 deopt 反复失败统一在 P4 RequestRefresh + 重编译协议处理(详见 [P4 04 §5](./04-osr-deopt.md))| ✅ 已完成(2026-06-28) |
| RJ-21 | [04 §12](./04-osr-deopt.md) | [P3 04 §1.2 / §1.4 bit50 写入纪律](../p3-wasm-tier/04-trampoline.md):CallInfo bit50 在 OSR exit 后的语义——清 0 还是保留 1(承 [P4 04 §7.2](./04-osr-deopt.md))**倾向清 0**(差分友好),P4 完成时实测确认 | ✅ 已完成(2026-06-28,清 0 纪律已字面化;实测确认留 Spike 1 接上) |
| RJ-22 | [08 §13.2 RB-3](./08-testing-strategy.md) | [P3 08 §0.4 P3 退役协议预设位](../p3-wasm-tier/08-testing-strategy.md):落实「若 P3 退役,V1-V18 编号保留,P4 接续承担」,[P4 08 §10](./08-testing-strategy.md) 已字面化,P3 08 加引用「具体迁移协议见 P4 §8 §10」| ✅ 已完成(2026-06-28) |
| RJ-23 | [08 §13.2 RB-4](./08-testing-strategy.md) | [P3 08 §4.4 P3 是 P4 投机正确性验证的预演](../p3-wasm-tier/08-testing-strategy.md):现稿已字面承诺,加引用「P4 视角具体验证形式见 P4 §8 §4 / §5」| ✅ 已完成(2026-06-28) |
| RJ-24 | [08 §13.2 RB-5](./08-testing-strategy.md) | [P3 implementation-progress §11 PW9 验收对账](../p3-wasm-tier/implementation-progress.md):加 P4 视角延伸——[P4 08 §3.7](./08-testing-strategy.md) force-all 非空保证援引 RW-10 教训,P3 implementation-progress 加引用「P4 force-all-jit 一样的纪律见 P4 §8 §3.7」 | ✅ 已完成(2026-06-28) |
...
| P4 子目录内部 | **8 项**(RJ-30~37,其中 7 项已完成双向协同)| 已对接 7 项(RJ-30~33 / RJ-35~37),余 1 项 RJ-34 ⏳ PJ11 |

**核心纪律**:承用户裁决「本期只记录不主动改 P1/P2/P3 现稿」,所有非「已对接」项标 ⏳。立项后按 PJ 完成节奏分批兑现。RJ-11 / RJ-30~33 / RJ-35~37 共 8 项已对接(无需 PJ 完成兑现;RJ-29 实状态是 ⏳ PJ0(主助理决议)不计入「已对接」)。
...
### 3.1 影响 PJ 启动形式(高影响 / 高不确定度)

| 决策 | 定稿 | 出处 | 复核点 |
|---|---|---|---|
| **立项判定** | 立项前置检查(三档:全启 / 部分前置 / 跳过)| [01 §3](./01-launch-judgment.md) | **外部输入**:① 真实宿主负载需求 ② 资源到位 ③ P3 现状是否够;**P4 生死攸关** |
...
| **自管机器栈大小** | 待定(每帧栈预算) | [05 §3.4](./05-system-pipeline.md) | PJ0 / PJ1 amd64 trampoline 完成实测 |
...
| 后续维护 | 每轮里程碑完成后追加对账行 | 立项后追加 PJ0-PJ11 进度行;若立项判定否决则记录「P4 跳过」决策 + 判定数据 |
...
**为何 PJ0 不阻塞②③**:PJ0 的本质是「包骨架 + build tag 隔离 + bridge 注入」,**不引入任何机器码生成、原生码段管理、或 P4 投机面**——SupportsAllOpcodes 全 false ⇒ 所有 Proto 仍走 crescent,P4 build 行为与 P1-only 等价。这层骨架只是「让 P3+P4 共存的 build tag 协议成立」+「为 PJ1+ 立项后启动时不需返工 build 系统」预先铺好——是「检查停下不亏」(roadmap §5 原则 3)在 P4 内部的兑现:即便 PJ1+ 永不启动,PJ0 的产出已让 P3 退役/P4 立项的决策面在工程层面就绪。
...
| # | 裁决项 | 决议 |
|---|---|---|
| 1 | PJ1 是否完成 JMP | ✅ 含 JMP(00 §4 PJ1 6 op,与始话对齐;forwardJump fixup 表在 PJ1 同批完成) |
...
## 6. PJ1 实现对账(2026-06-25 部分,承 §1 PJ 表头注)

**状态**:🔶 **PJ1 部分**——spike 检查 🟢 + amd64 mmap+W^X+trampoline+emitter 主库版 + LOADK/RETURN 单测 byte-equal **已完成**;**未接入 GibbousCode.Run end-to-end byte-equal**——SupportsAllOpcodes 仍全 false,完整端到端接入留 PJ2+(承下文 §6.4 范围裁决)。

### 6.1 spike 检查 🟢(承 06 §1.7)
...
### 6.2 主库 amd64 后端完成(`internal/gibbous/jit/amd64/`)
...
**PJ1 简化形式适用范围**:LOADK / RETURN 类「无 helper / 无栈帧 / 无外部状态」直线模板。这些 opcode 完成能让 trampoline 单一 CALL+RET 即可,不需要完整 jitContext。PJ2 算术 / PJ4 表 IC 引入 helper 调用时,trampoline 升级到完整版。
...
| 验收项 | 实测 | 命令 |
|---|---|---|
| spike 四档检查 | ✅ | `cd spike/p4tramp && go test -v ./... && go test -bench=. -benchtime=2s -count=3 ./...` |
...
承 [01 §3.4 三档策略](./01-launch-judgment.md):若 ② ③ 一直未到位,P4 最终决议「跳过」时本期产出作未来重启的「PJ1 工程组件已完成,PJ2+ 待启」存档。

---

## 7. PJ2 / PJ3 工程基础对账(2026-06-26 完成,承 §1 PJ 表头注)

**状态**:✅ **PJ2 投机模板 12+ 形式完整接入** + 🔶 **PJ3 工程基础 + 物理 spike**(接上 FORLOOP 字节级内联)。
...
## 8. PJ3 FORLOOP 字节级 inline 接上 + luajc 档突破(2026-06-26 完成)
...
形式对位 wrap-kernel × 50(避免 apples-to-oranges 工作负载错配):
...
**当前已完成**(5 类 FORLOOP 形式完整 byte-equal P1):
...
| 形式 | iter | P4 (ns/op) | crescent (ns/op) | gopher (ns/op) | **P4 vs cres** | **P4 vs gopher** |
|---|---|---|---|---|---|---|
...
**全部远超 luajc 档 4.4x 基线**——5 类形式 7-25x over gopher-lua。

**body inline 形式分析**:
- 空 body 形式 ~3 cycles/iter(addsd+ucomisd+jmp),20-25x over gopher
- body+=1 单 op ~8 cycles/iter(多 5 SSE 指令链:load/mov K/movq/sseOp/store),10x over gopher
- iter 数越大 boundary 占比越小,加速比逼近上限

---

## 9. PJ4 表 IC ArrayHit 字节级 inline 接上(2026-06-26 完成)
...
1. **Code 形式**:长度 2 或 3,`[0]=GETTABLE A B C / [1]=RETURN A 2 / [2]?=dead RETURN`;
...
| 形式 | iter | P4 ns | P1 crescent ns | **P4 vs P1** |
|---|---|---|---|---|
...
2. **NodeHit / SETTABLE / SELF 未接入**:NodeHit 需键比较 + node 段 24 字节寻址;SETTABLE 需反向写槽 + __newindex 元方法 deopt;SELF 是 GETTABLE 变体 + 多写 R(A+1)=R(B)。形式识别 + 模板拆分留下一阶段。
...
3. **`Makefile` 三条 P4 注释更新**(从陈旧 "PJ0 阶段:行为等价 P1" 改为):
   - `test-p4`:PJ0-PJ4 + PJ7 + PJ11 已完成:LOADK/MOVE/算术/比较/UNM/LEN/NOT/NEWTABLE/GETTABLE/SETTABLE/SELF/FORLOOP 接上 + IC 六路径字节级 inline
   - `conformance-p4`:白名单已扩 ~25 类形式 + IC 六路径,但 conformance 用例多为单次小脚本,~91% 不达 P4 升层检查 — 真 P4 路径验收以 difftest-p4 为准
...
- 单形式层:SpecTableHits 增量 = 1 实证(IC 六路径全覆盖)
- 整套层:PromotionCount > 0 fail-stop(difftest-p4 + conformance-p4 兜底)

### 9.7 严密 IsTable guard 升级(2026-06-26 完成)
...
### 9.8 PJ4 NodeHit 字节级 inline 接上(2026-06-26 完成)

承 §9.5 #2 已知边界:NodeHit 接入完整 PJ4 表 IC 覆盖。

**NodeHit 形式**:`function(t) return t["x"] end` 或 `function(t) return t[K] end` 其中 K 是字符串常量(或数字键且键在 hash 段而非 array 段)。luac 编 GETTABLE A B C 同 ArrayHit,但 IC[0].Kind=NodeHit。
...
**形式识别**(`analyzeGetTableNodeHit`):
- 与 `analyzeGetTableArrayHit` 一样的 Code 形式 + IC kind 检查(改 NodeHit)+ feedback 检查
- **stableKey 编译期固化**:`proto.Consts[KIdx]`(LoadProgram 已 intern 字符串,数字编译期就装好)

**Compile 主路径**:`Compile` 内先尝试 ArrayHit,失败再尝试 NodeHit(两路径形式字节重叠但 IC kind 区分)。两路径都用 `incSpecTableHits` 探针 + Run 端 `icArrayHit=true` 让 deopt 走 host.GetTable byte-equal P1(P1 icGetTable 兼容 ArrayHit + NodeHit)。

**e2e 实证**:
- `TestPJ4_TableNodeHit_E2E_WarmupThenForce`(`t["x"]` 形式,字符串键):Phase 2 SpecTableHits 增量 = 1 ✅
...
### 9.9 PJ4 SETTABLE ArrayHit 字节级 inline 接上(2026-06-26 完成)

承 §9.8 已知边界:SETTABLE 接入完成 PJ4 SETTABLE 首套实现。

**SETTABLE 形式**:`function(t, v) t[K] = v end` 中 K 是数字常量 + 命中 array 段(luac 编 SETTABLE A B C / RETURN A 1,其中 A=R(t) / B=K idx >=256 / C=R(v))。
...
### 9.10 PJ4 SELF ArrayHit 字节级 inline 接上(2026-06-26 完成)
...
P4 设计文档 §0 自估 +1-2 人年完整工程,分批次推进。本节明示已完成的 PJ5 工程基础 + 剩余 PJ5/PJ8/PJ9/PJ3 的完整工程量估算,作为渐进推进路径的清晰指引。

**PJ5 工程基础已完成**(2026-06-26):
...
**PJ8 剩余接上工程量**(估 +0.5-1 人月 + 物理 runner,):
...
### 9.13 PJ8 arm64 字节级模板矩阵完整(2026-06-26 完成)

**目标**:arm64 端 PJ2/PJ3/PJ4 三 op 字节级模板矩阵齐全,**对位 amd64 完整**,等物理 runner 接上(trampoline asm + mmap+RX 端到端)即可执行,工程组件层 PJ8 字节级模板交付完成。
...
**arch 检查解耦**(承 §9.13.2 review fix):FORLOOP Compile 块原检查 `info.isForLoop && archSupportsSpec()` 改 `archSupportsForLoop()`,因 FORLOOP 经 callJITFull 主路径不经 spec trampoline,arm64 端 archSupportsSpec=false 不应阻塞 arm64 FORLOOP emitter 调用;arm64 端 archSupportsForLoop 已可返 true 启用全套四形式。
...
(空 body 形式 `retB=1` 无返回值,恰好无害;body / body2 形式不是)

**根因**:godoc 自陈"FORLOOP 经 callJITFull 不经 spec trampoline,不依赖 archSupportsSpec",但实际检查用了 `archSupportsSpec()`,导致 arm64 emitter 字节级 byte-tested 但**永不被调用**。
...
- 形式 A0/B0:`MOVE/GETUPVAL + CALL A 1 1 + RETURN 0 1` — 0 参
- 形式 A1K/B1K:`... + LOADK A+1 + CALL A 2 1 + RETURN 0 1` — 1 K 常量参
- 形式 A1R/B1R:`... + MOVE A+1 + CALL A 2 1 + RETURN 0 1` — 1 reg 参
- 形式 A2K/B2K + A1K1R/B1K1R + A1R1K/B1R1K + A2R/B2R:2 参四组合 K+K/K+R/R+K/R+R(长度 5)
- 形式 A3*/B3*:3 参四组合 K+K+K/K+K+R/.../R+R+R(长度 6,`... + (LOADK|MOVE)×3 + CALL A 4 1 + RETURN 0 1`)— 8 子

getter 子态(N=1 返值 retB=2,22 子):
- 形式 AR1/BR1:`MOVE/GETUPVAL + CALL A 1 2 + RETURN A 2 + dead RETURN` — 0 参 1 返
- 形式 A1KR1/B1KR1:`MOVE/GETUPVAL + LOADK + CALL A 2 2 + RETURN A 2 + dead RETURN` — 1 K 参 1 返
- 形式 A1RR1/B1RR1:`MOVE/GETUPVAL + MOVE + CALL A 2 2 + RETURN A 2 + dead RETURN` — 1 reg 参 1 返
- 形式 A2KR1/B2KR1 + A2RR1/B2RR1 + A1K1RR1/B1K1RR1 + A1R1KR1/B1R1KR1:2 参四组合(长度 6)
- 形式 A3*R1/B3*R1:3 参四组合 K+K+K/K+K+R/.../R+R+R(长度 7)— 8 子

N>=2 返值 getter 子态(0 参 N=2/N=3 返,4 子):
- 形式 ARetN2/BRetN2:`MOVE/GETUPVAL + CALL A 1 3 + MOVE×2 + RETURN A=callA+2 B=3 + 隐式 RETURN B=1`(长度 6,N=2)
- 形式 ARetN3/BRetN3:`MOVE/GETUPVAL + CALL A 1 4 + MOVE×3 + RETURN A=callA+3 B=4 + 隐式 RETURN B=1`(长度 7,N=3)
...
**关键交付**:
- `internal/gibbous/jit/host.go::P4HostState.CallBaseline`:新接口,语义是 baseline doCall 分派(host/crescent/__call/全形式 gibbous 一律同步跑完),**绕过 tryIndirectCallee R3 indirect 哨兵** — 因为 PJ5 简化形式没有 wasm-level 段内 call_indirect 通道,若返 indirect 哨兵会让被调帧悬挂但永不执行(UAF 风险)
- `internal/crescent/gibbous_host.go::State.CallBaseline`:实现(复用 DoCall 的 baseline 分支,只跳过 tryIndirectCallee 快路径)
- `internal/gibbous/jit/compiler.go::analyzeCallVoidForm`:形式识别(MOVE/GETUPVAL + CALL B=1 C=1 + RETURN B=1 守卫严密)
...
- TAILCALL inline(承 `function() return g() end` 编 TAILCALL 而非 CALL,需独立形式识别 + 帧复用协议)
...
- `internal/gibbous/jit/host.go::P4HostState.TailCall`:新接口,语义对位 `crescent.State.TailCall`(已实现)三态分支:
...
- `internal/gibbous/jit/compiler.go::analyzeTailCallForm`:形式识别长度 4/5/6 共 8 子态,严密校验 TAILCALL.C=0 + dead RETURN.B=0 + 隐式 RETURN.B=1 + dead RETURN.A=callA 等不变式。retPC 指 dead RETURN(2/3/4),retA=callA,retB=0(host 尾完成路径走 DoReturn 多值 to-top)。
...
- 多 reg 参形式(2/3 reg 参 — 当前 2 K 参已通)
- 含返 N 值形式(N>=2)
...
### 9.16 PJ5 调用族 inline 完整形式学矩阵汇总(2026-06-27 完成)

承 §9.14/§9.15 PJ5 CALL void / TAILCALL 接上主路径。本节汇总累计推进的完整形式学矩阵(204 + 70 = 274 子形式),作为 PJ5 当前覆盖面的单一参考。
...
- **返值形式**:setter(0 返)/ getter(1 返)/ N>=2 返值 getter(0 参 N=2/N=3 + 1 K/reg 参 N=2)/ tail(透传)

**CALL void 子形式计数**(204 = setter 102 + getter 1 返 94 + getter N>=2 返值 8):
...
**TAILCALL 子形式计数**(70 = 双 callee × {0/1K/1R/2/3/4/5 参四组合 + 6 参 32 排列}):
- TA0/TB0 + TA1K/TB1K + TA1R/TB1R(共 6 子)
- TA2*/TB2* + TA3*/TB3* + TA4*/TB4* + TA5*/TB5*(2 参四组合 × 双 callee × 4 长度 = 32 子)
- 6 参 TAILCALL ,TailCall}`:实现(复用 DoCall/doTailCall baseline 分支,绕过 R3 indirect 哨兵 / 同步驱动 callee 链)
- `internal/gibbous/jit/compiler.go::{analyzeCallVoidForm,analyzeTailCallForm}`:形式识别(长度 3..10 / 4..9,严密 luac 子形式校验)
...
`internal/gibbous/jit/compiler.go::analyzeSelfCallForm` 识别长度 4..6 形式,共同结构 `[0]=MOVE/GETUPVAL` (recv → R(callA)) + `[1]=SELF` (R(callA)=method, R(callA+1)=self) + `[2..]=参数 + CALL/TAILCALL + RETURN`。

| 长度 | 形式 | 描述 |
|---|---|---|
| 4 | M0/U0 | 0 参 0 返 CALL void(`function(o) o:m() end`)|
| 4 | TM0/TU0 | 0 参 TAILCALL(luac TAILCALL 长度 4 dead 形式)|
| 5 | MR1/UR1 | 0 参 1 返 CALL getter(`local r = t:m()`)|
| 5 | TM0_5 | 0 参 TAILCALL 长度 5(`return t:m()` 实际形式:含 dead RETURN A=callA B=0 + 隐式 RETURN B=1)|
...
- `internal/gibbous/jit/compiler_pj5_self_test.go`:9 形式识别单测(M0/U0 void + M0 tail + MR1 getter + M1K/M1R void + 拒识别短码 / 非 SELF / SELF.C reg form)
...
**子形式计数**(完整覆盖):0..7 参 × {void / 1 返 / tail} × 双 receiver(M/U)+ N=2/N=3 返值 0/1 参 ≈ **2×(8 void + 8 getter + 8 tail) + 4 N>=2 = 52 子形式**;长度区间 4..11 通过 analyzeSelfCallForm4/5/6/7/8/9 + analyzeSelfCallFormN(N=6/7) 分流识别。

### 9.18 PJ5 OSR exit 协议骨架(2026-06-28 完成)

承 docs/design/p4-method-jit/04-osr-deopt.md §5 + §11 字段定义,完成 P4 内部投机子状态机骨架(方案 A:P4 自管,P2 三态枚举不变)。
...
承 §9.10 PJ4 EmitSelfNodeHit 字节级模板(166 字节)复用 + §9.17 PJ5 SELF inline 升级,完成 PJ5 SELF + CALL 形式的 **SELF 段字节级 inline**(IC NodeHit 命中时跳过 host.Self round-trip)。
...
probe wangshu frontend 验 `obj:method` 无 args 形式 → **parser 报语法错** `function arguments expected`,Lua 5.1 严格语法 `:` 方法引用必须接 `(args)`。即:
...
**CALL 段瓶颈 profile + 摊薄验证**(承 [perf-optimization-workflow](../../../llmdoc/guides/perf-optimization-workflow.md) §1 profile 先行 + §5 跨形式基线对照):
...
**P4 ratio = 0.95x —— P4 比 crescent 快 5%!** method 体含 FORLOOP 时,P4 升层 method 体(PJ3 FORLOOP 字节级 inline 大幅加速)主导,caller SELF+CALL trampoline 开销被摊薄,P4 反超。**完整画面**:简单 method 体(count++)→ trampoline 占比大 → 1.12x 慢(bench 形式放大);计算密集 method 体(FORLOOP)→ method 加速主导 → 0.95x 快(真实 OOP 业务场景)。
...
**形式完整覆盖矩阵**(承 §9.19 后续批次,本节 2026-06-28 同日扩):
...
probe luac 实证 N=K 返形式 cC=K+1 一致(N=4 返 cC=5 / N=5 返 cC=6 / ...)。加 `isValidSpecCallRetCount(cC int)` helper(compiler.go line 2591):`cC == 1 || (cC >= 3 && cC <= 16)`,允许 0 返 + 2..15 返。sed 替换 7 处把关 `(cC != 1 && cC != 3 && cC != 4)` → `!isValidSpecCallRetCount(cC)`。

上界 16(N=15 返)选定:Lua 5.1 CALL C 字段最大 255 但实用 method 多返值典型 N<=8;N<=15 保守覆盖几乎所有真实业务。

**N=4 返多形式 e2e**(承 91dcf07,4 用例 SpecSelfCallSpecHits 0→1):
...
**N=4 返多形式 difftest**(承 84a031d,5 用例 oracle/crescent/p4-jit 全 byte-equal):
...
- **基础形式 8 用例**(承 PJ5 SELF spec template 初批 §9.19):WarmupThenForce + 1KArg + 1RegArg + 3Args + TailCall_M0 + Getter_M0 + UpvalRecv + TailCall_1RegArg
...
| Spike | 形式边界 | 关键工程 | 验证点 | 预估工程量 |
|---|---|---|---|---|
| **1** | 0 参 void CALL(callee `function(self) ... end`,setter 形式) | EmitFrameBuildVoid0Arg amd64/arm64 + jitContext.ciSlotAddr + Compile 把关加 callee.NumParams=0 + !IsVararg + !NeedsArg | byte-equal P1 + SpecFrameInlineHits=1 命中实证 + benchmark 0 参 setter 反超 crescent | 1 周级 |
...
| **3** | vararg 支持 | EmitFrameBuildVararg:三步重排字节级 inline + IsVararg 分支 | vararg callee 形式 e2e + difftest | 1.5 周级 |
| **4** | 多返值多形式(N>=2 返 + multi-ret + 可变 nresults)| EmitFrameTeardownMultiRet:nresults 解码 + 多退少补 byte-level | retB={0,1,2,N>=2} 全形式 e2e | 1 周级 |
...
#### 9.20.4 把关条件(Spike 1 起手)

Spike 1 启用 `useFrameInline=true` 需同时满足:
- analyzeSelfCallSpecForm 既有把关(IC NodeHit + FBSelfMono + stableKey)
- callee Proto 是 PJ5 SELF inline 已识别的 closure(`function(self) ... end` 单形式)
...
**deopt 路径**:
- 任一把关失败 → 降级 host.CallBaseline(byte-equal P1,无 frame inline 段执行)
...
func HelperRunCalleeAfterFrameInline(jitCtx *JITContext, base int32, retA int32) int32 {
    // 实现:从 jitContext.ValueStackBase 取栈 / 调 executeFrom 跑 callee /
...
**(6.5) R14 ABI 违约修复**(2026-06-28 已完成,承 PR #26 外部审查):
...
**(7) Spike 1 接上 Step C-E 渐进路线**(承 §9.20.3 + 本节):

- Step C-1:helper 函数 `HelperRunCalleeAfterFrameInline(jitCtx *JITContext, base int32, retA int32) int32` 实现 + `//go:nosplit` + 字节级 SP 协议单测
...
func HelperRunCalleeAfterFrameInline(jitCtx *JITContext, base int32, retA int32) int32 {
    ...
    //
    // **结论**:Spike 1 接上需 trampoline 改造支持 "exit-to-host-then-resume"
    // 协议(类似 wazero exit reason code 路由),工程量大于单 helper 实现。
    panic("not implemented: Spike 1 Step C-1 待 trampoline exit-resume 协议完成")
}
```

**(6) 阻塞点**(本批文档暴露):

trampoline 当前是 "一次性同步跑完 mmap 段 + RET" 协议,不支持 mid-段 exit-to-host。要接上 Spike 1 helper call 需:
...
#### 9.20.8 Spike 1 字节级模板字节级编码完整文档化(2026-06-28 实现指南)

承本会话 Spike 1 全套字节级 emit 模板已完成 + §9.20.6 helper call ABI 协议设计 + §9.20.7 Step C-1 拆解,本节固化所有 Spike 1 字节级模板的精确字节编码,供 future Spike 1 接上者实现指南。
...
#### 9.20.9 trampoline exit-resume 协议详细设计草案(2026-06-28 future session 实现基线)

承 §9.20.7 (6) 阻塞点 + (7) 修正后路线:Spike 1 接上的核心瓶颈是 trampoline 改造支持 "exit-to-host-then-resume" 协议。本节固化协议详细设计草案,供 future Spike 1 接上 session 直接实现。
...
type JITContext struct {
    ...
    // **本节新增** (Spike 1 接上 + exit-resume 协议):
...
func dispatchInlineHelper(jitCtx *JITContext) uintptr {
    ...
    case HELPER_RUN_CALLEE:
        // Step C-1 helper 真实现:经 jitCtx 取 hostStatePtr → State
...
承 §9.20.9 实现路线 + 单 session 内交付:**12 commits(`da8033f..9a17744`)完成 Spike 1 接上完整端到端**,amd64 archSupportsFrameInline=true,SpecFrameInlineHits=1 prove-the-path 命中实证。

**实现顺序差异**(设计草案 5 commits → 实现 12 commits 细分):
...
| 5c | runFrameInlineDispatcher 直 host 路由实现 | 98e299e |
| 5d | crescent.ExecuteCalleeFromInlineFrame readCISegInto 版 | a1d7a6e |
| 5e | FrameInline e2e GatingClosed_NoHits 验证 | 6763e64 |
| 5f | ExecuteCalleeFromInlineFrame 反查 closure GCRef → callee Proto → enterLuaFrame 接上策略重定 | ff306f6 |
| 5g | analyzeSelfCallSpecForm useFrameInline 把关 | 25f3f6a |
| 5h | archSupportsFrameInline 翻 true(amd64)+ e2e GatingOpen_HitsOne 改判 ≥1 | 9a17744 |

**关键设计差异**(实现现实 vs 设计草案):
1. **dispatcher 路由实现位置**:草案 §9.20.9 (4) 假设 trampoline asm 内 `CALL ·dispatchInlineHelper(SB)` 跨包 CALL Go 函数;实现跨包 + Plan 9 ABI 复杂度高,改 **Run 端 Go 函数做 dispatcher**(commit-5a-c),trampoline asm 段返后纯透传 RAX 不解读,全 Go 端做 helper request 路由 + 二次 callJITSpec 重入 resume entry。工程复杂度降一个数量级。
2. **callee Proto 元数据策略**:草案 §9.20.5 P3 PW10 同源参考期待 mmap 段完整 emit 帧建(arena proto cache 段 + 5 word 真实计算 + Compile 期 callee.NumParams/MaxStack 把关),helper 只跑 executeFrom;实现 P4 Spike 1 采用更保守的「mmap 段写 placeholder + helper 内反查 closure GCRef → callee Proto → 调 enterLuaFrame + executeFrom 完整重做帧建」策略(commit-5f),放弃零跨界目标但保证正确性 + 工程可达(无需 arena proto cache 段 + P2 Bridge 200+ 行级改造)。
3. **把关简化**:草案 §9.20.4 期待 `callee.NumParams=0 + !IsVararg + !NeedsArg + MaxStack≤32` 编译期把关;实现因 callee Proto 元数据策略重定,改 Compile 期仅把关 `callArgCount=0 + isCallVoid + !isTailCall`(0 参 setter 形式);callee 端运行期由 enterLuaFrame 内 nargs=0 把关自然兼容,IsVararg=true 时 callee 体读 vararg 区会读到 0 元素退化但不崩。
...
- Spike 1 简化策略**未带来 simple setter 路径的性能提升**,但 **method 体加速主导的反超场景(0.95x)继续保持**;真正消除 simple setter trampoline 占比需 Spike 2-4(args 装载 + 多返值多形式完整 inline + arena proto cache 段 + word0/1/2/4 真实计算消除 helper 端 enterLuaFrame round-trip)

**Spike 1 接上验收数据**(amd64 archSupportsFrameInline=true 后):
- ✅ make test-p4 全过 21 binary(crescent.test 含 PJ5 SELF e2e + 接上 GatingOpen_HitsOne)
...
- ✅ TestPJ5_SelfCall_E2E_SpecTemplate_OSRExitToDeopt / DeoptStorm(OSR exit 协议在接上路径上仍正确工作)
...
| 4 | 多返值多形式(callC=1..16) | commit-5q | helper 接 nresults;callC-1 算 | TestPJ5_FrameInline_E2E_Spike4_Getter: s=4200, RunHits=99 |
...
- analyzeSelfCallSpecForm 把关扩 callArgCount<=7
- vararg 形式:enterLuaFrame 内部自动处理(NumParams<nargs 时 nVarargs=nargs-NumParams,栈下区重排)

**Spike 1/2/3/4 全套验收数据**(amd64,commit-5r 后):
- ✅ make test-p4 全过 21 binary
- ✅ difftest 全过(byte-equal P1 + crescent + p4-jit 三方)
- ✅ TestPJ5_FrameInline_E2E_Spike2_3KArg:RunHits=99(3 K 参)
- ✅ TestPJ5_FrameInline_E2E_Spike3_Vararg:RunHits=99(vararg 形式)
...
- ✅ bit50 协议定下来(2026-06-29 用户确认清 0,差分友好)
- ✅ P3 去留决议 D2 定下来(2026-07-01 用户选主动保留,[07 §5.6](./07-p3-retirement.md) 形式:代码留 + build tag deprecated + wazero 版本锁死 + 不承诺双后端 CI 持续绿,是 §10.2 应急保留方案;区别于 §6 退役 / §7 留中层)
...
- ✅ Spike 2 接上(N 参 fixed args 0..7)
- ✅ Spike 3 接上(vararg callee)
- ✅ Spike 4 接上(多返值多形式 callC=1..16)
...
- ✅ bit50 协议定下来(2026-06-29 用户确认清 0)
- ❌ P3 退役决议(PJ11 验收时用户定下来)
...
## 11. P3 去留决议数据进档(2026-06-28 收集,等 PJ11 验收时定下来)

**当前 P4 完成度**:PJ0-PJ4 + PJ5(CALL void 220 子 + TAILCALL 102 子 + SELF inline 完整 0..7 参 + SELF spec template N=2..15 返全形式)+ PJ7 + PJ11 luajc 档突破已完成。剩 PJ5 Option B 帧建立内联(Spike 1 字节级 emit 全套就位,接上留 trampoline exit-resume 协议改造)/ PJ8 物理 runner / PJ9 双架构差分套 渐进推进中。

**承 [07 §5.1 缺省倾向](./07-p3-retirement.md):P4 验收通过后,P3 退役**(`internal/gibbous/wasm` 代码留版本史移除主分支)。

**等 PJ11 验收时由用户定下来的决议输入**:
...
**D2 决议**(2026-07-01 用户定下来):**主动保留 P3**([07 §5.6](./07-p3-retirement.md) 形式,§10.2 应急保留方案):

- **定下来结果**:既非 §6 退役也非 §7 留中层,走第三种形式「主动保留」:
...
- **RJ-12 影响**:自动消解(不走 §7 留中层,considerPromotion 无需扩平台维度,主动保留形式下 build tag 层已足够隔离)
...
**P3 退役决议数据(§11)已就位 amd64**,等 PJ11 双架构验收(arm64 物理 runner)即可由用户定下来。
...
| 单测(字节级模板 + 把关) | **20 单测**(analyzeSelfCallSpecForm 5 反向 + isValidSpecCallRetCount 11 表驱动 + analyzeSelfCallForm 2 codeLen + 2 V11 bridge) | internal/gibbous/jit + bridge |
...
- **Option B Spike 1 字节级 emit 模板全套**(承 §9.20 + §9.20.6 + §9.20.7):amd64 110/120 字节 BuildVoid0ArgSkeleton + arm64 156/164 字节对位 + helpers.go panic 占位 + archSupportsFrameInline 检查 + 探针;接上 4-5 周工程(等 trampoline exit-resume 协议改造)
...
7. ✅ **Spike 1 接上完成**(2026-06-28 §9.20.10-13 完整端到端 amd64 + zero-cross 优化)
...
#### 9.20.14 darwin/arm64 W^X 真实现 + arm64 检查翻 true(2026-06-29 macos-latest CI 接入)

承 §13.4 双架构闭环 prerequisite 1 + tmp/wangshu-p4-todo.md §三 darwin/arm64 真实现路径(用户定下来方案 A:macos-latest CI 接 darwin/arm64 端到端,无前期硬件投入)。
...
## 14. PJ10 通用 per-op 翻译器交付对账(2026-06-30 / 2026-07-01 两轮完成,承 §1 PJ 表头注)
...
承 §14.2 native emit 覆盖 + §14.3 V15b heavy 达标基础,2026-07-02 本轮把 amd64 native 接受门从「18 op mmap-safe 线性子集」扩到「26 op」,把 GETTABLE / SETTABLE / NEWTABLE / GETUPVAL / SETUPVAL / GETGLOBAL / SETGLOBAL / CALL / UNM 九个原本走 head-op 回放 fallback 的 op 也接进 native mmap 段,通过新完成的 **exit-reason 协议**完成语义。详细协议描述见 [10 §14.3](./10-per-op-translator.md)。
...
### 15.1 Spike 0:spike 检查(commit `9c49d72`)✅
...
**更新(2026-07-07,issue #69 / PR #73)**:这里描述的 F7-a 边界已放开,详 §17。`AnalyzeNative` 不再因字符串字面量 LOADK 拒收整个 proto,字符串常量的 GCRef 现在可以作为 imm64 烤进 mmap 段。
...
顺带清掉了因此变成孤儿的 `shimSelf` / `shimConcat` / `shimSelfAddr` / `shimConcatAddr`(与早先 LEN 退役同样的清理规则;两个 arch 现在都经 exit-reason 下降 SELF / CONCAT)。

`e2e_self_concat_test.go` 的 build tag 拓宽到也覆盖 arm64 native runner(`(amd64 && linux) || (arm64 && ...)`)——三个断言是 arch-neutral 的(promote + native run + byte-equal),一个文件同时服务两个 arch。darwin/arm64(M5 Pro)实测三个用例全过。

验证(darwin/arm64):`make test-p4` / `difftest-p4` 全绿,`golangci-lint` 干净,cross-build linux/amd64 OK,90s `FuzzP4ForceAllPromote -parallel=4` 全绿(第一轮 120s 触到已知的 go#75804 假超时,未写出 crasher,corpus 未变;见 issue #63)。
...
## 17. issue #69 字符串常量 LOADK 纳入 native 接受面(2026-07-07,PR #73)

承 issue #69。本 PR 放开 F7-a 门:此前 `AnalyzeNative` 见到任何含活跃字符串字面量 LOADK 的 proto 就拒收整个 proto。这是 P4 接受面最大的隐性缺口——一个 `s = "x"` / `t.name .. "-"` / `obj:m("key")` 就让整个函数(连同其中的算术、循环、CALL)全部掉回解释器。
...
spike 评估了三个方向,最终决定走**方向 A(直接把字符串槽的 GCRef 作为 imm64 烤进 mmap 段)**,镜像已经在用的 EQ-K(#56)的写法。方向 A 成立依赖三个前提,全部已验证:
...
- **删拒收**:两个 arch 的 `AnalyzeNative` 删掉 stringConst 拒收分支。
- **emit 零改动**:`emitLOADK` / `emitLOADKArm64` 本来就烤 `proto.Consts[Bx]`;私有化之后字符串槽已经是真正 intern 过的 bits,所以 emit 不需要改。
- **注释修正**:改掉一处过时注释(「string→Nil 占位」)——那条注释针对的是 Program 级 proto,私有化后的 proto 上已不适用。
...
字符串字面量 proto 现在能 promote 了(`NativeRunCount` 从 0 变成 >0)。**这是接受面改动,不是逐 op 加速。** 字符串 op 密集的 kernel 仍然约等于解释器速度——每个 CONCAT / `#s` / 表的 string-key 访问仍然走 exit-reason 往返出段到 Go。这条改动的价值在于「不再因为一个字符串字面量就拖垮整个函数升层」,实际收益取决于 proto 里非字符串 op(算术、循环、CALL 等)所占的比例。
...
**密度门的前提对 intrinsic CALL 是错的**:intrinsic CALL 不是往返,是一条内联指令(和 arith 一样便宜),不该算进「需要摊薄的 CALL」。修法:
...
- **`internal/gibbous/jit/jitcontext.go`**:`NewJITContext` 调 `AllocSpillStack`(64 KiB,`spillBase` = 对齐后的高地址端 / `spillTop` = 低地址端);新增 `savedGoSP` 字段在切 SP 期间暂存 goroutine SP;新增偏移常量 `JITContextSpillBaseOffset` / `JITContextSavedGoSPOffset` + `TestSpillStackLayout` 断言 `.s` 文件里硬编码的偏移与 struct 一致。
- **`internal/gibbous/jit/amd64/trampoline_spec_amd64.s` + `internal/gibbous/jit/arm64/trampoline_arm64.s`**:从 `[jitCtx+off]` 读 `spillBase`,非 0 则暂存 SP 并切到 spillBase(进段前),出段后切回(在恢复 callee-saved 之前)。两条硬约束:① 切 SP 前先把 codeAddr 读进寄存器(`+N(FP)` 是 SP 相对寻址,切 SP 后会指向自管栈垃圾);② 恢复时不能覆写 RAX/R0(它带段的 exit-reason status)。空 jitCtx(`spillBase==0`)要保护(底层模板单元测试传 jitCtx=0)。
- **`internal/gibbous/jit/peroptranslator/call_ic.go`**:`segToSegDepthCap` 16 → 128。

### 20.2 验证

- **amd64 端到端**:`TestI86_DeepRecursionGCStress` cap=128 GOGC=1 下 3/3 不崩,全 p4 单元 + difftest + conformance 绿,FuzzAutoPromote 90s 干净。
- **arm64**:镜像实现 + 交叉编译通过,执行正确性交 CI arm64 矩阵 + 另一台 arm64 机器。
- **spike**:`spike/p4spillstack`(amd64 四道 gate 全绿 + DECISION.md)。

### 20.3 文档同步

05 §3.3 第 4 组字段 + §7.5 不变式已从「保留字段 / PJ10 阶段未打开切 SP」更新为「issue #89 已接线」;05 §8.2「自管机器栈精确布局 / 自管栈池化」开放问题已标 size 定为 64 KiB;06 §4.1.5(amd64)/ §4.2.5(arm64)trampoline 伪码 + 寄存器表 §4.1.1 / §4.2.1「栈指针」行 + §4.3.2 调用约定保留位已更新为切 SP 的真实序列。
...
### 21.2 顺带修复:doTailCall 尾调用 host 函数丢多返回值(共享三层的既有 bug)

P4 验收测试的多返回值 host 尾调用用例暴露一个 clean master 上就存在的 bug:`doTailCall` 给 `callHost` 传父帧定长 `ci.NResults()`,而 PUC lvm.c OP_TAILCALL 是 `luaD_call(L, ra, LUA_MULTRET)`。定长分支写完结果把 top 重置回 `base+MaxStack`,尾随 `RETURN B=0` 按 top 收就截断尾部——`return unpack({7,8,9})` 只回 `7, 8`,P1/P3/P4 全中(共用 `doTailCall`)。修法:传 `-1`(multret,callHost 的 multret 分支精确设 `top = funcIdx + n`)。独立 commit + conformance 用例 `tailcall_host_multret` 锁三层。
...
## 24. issue #112 mono 自尾调用段内循环(双架构,2026-07-10,PR #113)

### 24.1 问题

HeavyRecursion(collatz 自递归)升层后 6.48ms,比 P1 解释器的 5.81ms 慢 12%,是 README 表里唯一「升层净亏」的负载(arm64 差距 -46%)。根因精确到协议层:collatz 的全部递归调用是 TAILCALL(`return collatz(...)`),而 §21 的 `HelperTailCall` 每层递归都终止 native run——段退出 + `host.TailCall` + `executeFrom` + 每层一次 Go 重入。解释器同一条链在一个 `executeFrom` 循环里平坦跑完(PUC 尾调用帧复用),P4 对帧体(比较 + 算术)的加速小于它新增的每层退出税。对照:普通 CALL 递归(fib)有 seg2seg 段内直调所以 9.8×,TAILCALL 从未进过 seg2seg。

### 24.2 修法(issue #112 方向 1)

PUC 5.1 尾调用复用 caller 帧(`doTailCall`:搬 callee+args 到 funcIdx、pop、re-enter)。当 callee 就是正在运行的 closure 时,「复用后的帧」与「用搬移后的参数重进本段」**位级等价**——整个跨界往返塌缩成段内的参数 shuffle + jmp:

- **fast body**(`emitSelfTailCallLoop` / `emitSelfTailCallLoopArm64`,挂在 HelperTailCall exit-reason 之前):身份 guard → 参数搬移 `R(k) = R(A+1+k)`(升序,镜像 doTailCall)→ nil-fill `[nargs, MaxStack)` → `SelfTailCallHitCount` 白盒探针 → loopFuel dec+jnz → jmp BB 0(entry)。guard miss 直落原 HelperTailCall,行为不变。
...
- **性能**(amd64,-benchtime=2s -count=3 -cpu=1):HeavyRecursion force 6.48→**1.85ms**,auto 6.48→1.87ms(P1 5.81ms;0.88×→3.1× vs P1,1.38×→4.8× vs gopher);fib 1.08ms / binary-trees 27.9ms 无回归。README amd64 行单项更新 + 重测脚注;arm64 行待重测。
- **prove-the-path**:`TestI112_SelfTailLoopHits`(collatz / accumulator / 参数不足三形状,`SelfTailCallHitCount` delta > 0)/ `TestI112_GuardMissFallsBack`(非自尾调用探针恒 0 + `TailCallRunCount` 增长)/ `TestI112_ClosureGateDeclines`(CLOSURE 门拒绝 + 各闭包捕获各自迭代值 byte-equal 经 fallback)/ `TestI112_BudgetPreemptsSelfTailLoop`(预算抢占)。
- **difftest** 4 例三方 byte-equal:collatz / 参数不足 / 5000 深 accumulator / `p4_selftail_stale_local_clear`(跨迭代局部清理)。
- 三 build 全量绿;90s p4 fuzz smoke 干净(接受面动了按纪律重探);arm64 交叉编译过,运行时验证交 PR CI arm64 矩阵。
...
nightly `FuzzAutoPromote` 稳定 crasher:`function sum(n) for A=0,n do end end sum "7"` — auto run 2 报「'for' initial value must be a number」,P1 解释器正确 coerce string `"7"` 为 7 通过。
...
- **测试**:`test/regression/issue177_regression_test.go` 两个场景钉住 — crasher 本体(string limit 通过)与真非 number limit(table)错误信息 byte-equal;`testdata/fuzz/FuzzAutoPromote/8305a8ceb22b8f41` 入库常驻;三 build(default/oracle/p3/p4)全绿。
- **后续动作**:审计 p4Code 里所有其他 shape-template 的 deopt 路径,同规则扫是否也有「快路径省 spill + deopt 直调 helper」错配(本轮只修 FORPREP 一条);追 PJ10 native emit 侧 FORPREP 路径,核对它的 deopt slot 恢复约定是否已就位(本轮已排除它不是 #177 通道,语义一致性值得单独核对)。
- **教训**(详反思 [[../../../llmdoc/memory/reflections/2026-07-24-p4-template-forprep-deopt-round]]):① P4 build 有 PerOpCode / nativeCode / p4Code 三大类 code kind,分诊必须先用探针确认走哪条;② fast-path template 省略 spill 的 slot,deopt 路径必须显式 SetReg 恢复到 interpreter-shape 再调 host helper;③「A 场景通过 + B 场景失败」只定位触发前提,不能推「bug 在哪条代码路径」— tier 分诊里两个场景的通过/失败结果由「促升条件 × 促升到的 code kind × 该 code kind 的 fast-path 是否走通」三个变量共同决定;④ 改深层 JIT 数据流前,先读所选字段的定义 + 全部消费点,确认语义/编码与新用途一致。
