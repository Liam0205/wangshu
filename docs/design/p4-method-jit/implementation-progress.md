...
| PJ5 | amd64 CALL/TAILCALL + 跨层互调 + OSR exit 实装 | [04](./04-osr-deopt.md) + [05 §4.3](./05-system-pipeline.md) + [06 §3.5](./06-backends.md) | gibbous-jit 三向分派 + OSR exit 状态等价(V19)| 🔶 **2026-06-27 CALL 八子形态打通**(承 §9.14 — analyzeCallVoidForm 识别 setter 六子态(A0/B0/A1K/B1K/A1R/B1R,0/1 K/1 reg 参 0 返)+ getter 二子态(AR1/BR1,0 参 1 返)= 八子态 + P4HostState.CallBaseline 接口 + Run prelude CALL case 真接入 + P2 scope-aware AnalyzeProto 跨 Proto 传递 outer localFnAsts 打通真升层 + 4 e2e SpecCallVoidHits=1 prove-the-path 命中实证;剩 TAILCALL / 多参 / 含返 N 值 / OSR exit / 段内 EmitCallInline 留下一批) |
...
**形态范围**(双形态 callee × {0 参 0 返 / 1 K 参 0 返 / 1 reg 参 0 返 / 0 参 1 返} = 八子形态,luac 编译产物):

setter 子态(0 返值 retB=1):
- 形态 A0:`MOVE A B + CALL A 1 1 + RETURN 0 1` — parameter callee 形态 0 参(`function(g) g() end`)
- 形态 B0:`GETUPVAL A B + CALL A 1 1 + RETURN 0 1` — upvalue callee 形态 0 参(`local function noop()...end; local function invoker() noop() end`)
- 形态 A1K:`MOVE A B + LOADK A+1 Bx + CALL A 2 1 + RETURN 0 1` — parameter callee + 1 K 常量参(`function(g) g(K) end`)
- 形态 B1K:`GETUPVAL A B + LOADK A+1 Bx + CALL A 2 1 + RETURN 0 1` — upvalue callee + 1 K 常量参(`local function take(x)...end; local function tick() take(42) end`)
- 形态 A1R:`MOVE A B + MOVE A+1 B' + CALL A 2 1 + RETURN 0 1` — parameter callee + 1 reg 参(`function(g, x) g(x) end`)
- 形态 B1R:`GETUPVAL A B + MOVE A+1 B' + CALL A 2 1 + RETURN 0 1` — upvalue callee + 1 reg 参(`local function take(x)...end; local function tick(v) take(v) end`)

getter 子态(1 返值 retB=2):
- 形态 AR1:`MOVE A B + CALL A 1 2 + RETURN A 2 + dead RETURN` — parameter callee 0 参 1 返
- 形态 BR1:`GETUPVAL A B + CALL A 1 2 + RETURN A 2 + dead RETURN` — upvalue callee 0 参 1 返(`local function f()...end; local function get() local x = f(); return x end`)
...
- e2e 4 个(`internal/crescent/gibbous_pj5_call_e2e_test.go`):FormB_Upval(B0)+ FormB1K_UpvalArg(B1K + sum=2100)+ FormB1R_UpvalArg(B1R + sum=1275)+ FormBR1_GetterUpval(BR1 + s=2100)
- difftest-p4 5 个(`test/difftest/p4_test.go`):p4_call_void(A0)+ p4_call_void_upval(B0)+ p4_call_void_upval_1argk(B1K)+ p4_call_void_upval_1argreg(B1R)+ p4_call_getter_upval(BR1)— oracle / crescent / p4-jit 三方 byte-equal
