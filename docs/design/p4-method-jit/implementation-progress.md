...
| PJ5 | amd64 CALL/TAILCALL + 跨层互调 + OSR exit 实装 | [04](./04-osr-deopt.md) + [05 §4.3](./05-system-pipeline.md) + [06 §3.5](./06-backends.md) | gibbous-jit 三向分派 + OSR exit 状态等价(V19)| 🔶 **2026-06-27 CALL void 二十子 + TAILCALL 十四子形态打通**(承 §9.14/§9.15 — CALL void setter 14 子态(0/1 K/1 reg/2 参四组合)× 双 callee + getter 6 子态(0/1 K/1 reg 参 1 返)× 双 callee = 20;TAILCALL 0/1 K/1 reg/2 参四组合 × 双 callee = 14;P4HostState 加 CallBaseline + TailCall 三态分支接口;Run prelude CALL/TAILCALL case 真接入 + P2 scope-aware AnalyzeProto + 10+7 e2e SpecCallVoid/TailCallHits=1 命中实证 + 11+7 difftest-p4 三方 byte-equal;剩 含返 N 值 / 2+ 参 getter / 3+ 参 / OSR exit / 段内 EmitCallInline 留下一批) |
