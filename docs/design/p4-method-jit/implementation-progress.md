...
3 e2e 实证(N=4 返 0/1 K 参 + N=5 返 0 参)+ 2 difftest 三方 byte-equal(p4_self_spec_multiret_n4_0arg + n5_0arg)。

bench 实测(承 1eb520d):BenchmarkGibbousJIT_PJ5SelfCallSpecMultiRetN4 = 18786 ns/op,Cresc = 17175 ns/op,**P4 ratio 1.094x 慢 9.4%**(简单 method 体 count++ 形态,trampoline 占比主导,与 N=0 返 PJ5SelfCallSpec 1.12x 慢同款瓶颈;计算密集 method 体反超验证留后续 PJ5SelfCallHeavyBodyMultiRetN4)。
...
**e2e 实证累计**(11 用例 SpecSelfCallSpecHits 0→1,本批 +3):
...
- **MultiRetN4_0Param(form4 N=4 返)/ MultiRetN5_0Param(form4 N=5 返)/ MultiRetN4_1KArg(form5 N=4 返)** — 本批新增

**difftest 三方 byte-equal**(承 cc66452 + 84c7ed4):6 用例(p4_self_spec_multiret_0arg/1karg/3kargs/5kargs + multiret_n4_0arg/n5_0arg)oracle lua5.1 / crescent / p4-jit 全过。
