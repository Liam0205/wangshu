...
| PJ5 | amd64 CALL/TAILCALL + 跨层互调 + OSR exit 实装 | [04](./04-osr-deopt.md) + [05 §4.3](./05-system-pipeline.md) + [06 §3.5](./06-backends.md) | gibbous-jit 三向分派 + OSR exit 状态等价(V19)| 🔶 **2026-06-27 CALL void 四子形态打通**(承 §9.14 — analyzeCallVoidForm 识别 MOVE/GETUPVAL+CALL+RETURN void(0 参 A0/B0)与 MOVE/GETUPVAL+LOADK+CALL+RETURN void(1 K 参 A1K/B1K)四子形态 + P4HostState.CallBaseline 接口 + Run prelude CALL case 真接入(含 LOADK 装载到 R(callA+1))+ P2 scope-aware AnalyzeProto 跨 Proto 传递 outer localFnAsts 打通真升层 + 2 e2e SpecCallVoidHits=1 prove-the-path 命中实证;剩 TAILCALL / 多 K 参 / 含返 CALL / OSR exit / 段内 EmitCallInline 留下一批) |
...
**测试覆盖**:
- 单测 9 个(`compiler_pj5_call_test.go`):RecognizeMOVE / RecognizeGETUPVAL / Recognize1ArgK(形态 A1K)/ Reject(9 子测覆盖每条守卫)/ RunCallVoidPath(形态 A 端到端)/ RunCallVoidUpvalPath(形态 B 端到端)/ RunCallVoid1ArgKPath(形态 A1K 端到端 LOADK 装载)/ ErrPropagate / SupportsAllOpcodesGate / SpecCallVoidHits 探针
- e2e 2 个(`internal/crescent/gibbous_pj5_call_e2e_test.go`):FormB_Upval(形态 B,SpecCallVoidHits=1 实证)+ FormB1K_UpvalArg(形态 B1K + sum=50*42 语义正确)
- difftest-p4 3 个(`test/difftest/p4_test.go`):p4_call_void(形态 A)+ p4_call_void_upval(形态 B)+ p4_call_void_upval_1argk(形态 B1K)— oracle / crescent / p4-jit 三方 byte-equal
