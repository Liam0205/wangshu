...
> 状态:**PJ0-PJ4 + PJ5 CALL void 一百七十二子形态 + PJ5 TAILCALL 七十子形态 + PJ7 + PJ10 luajc 档突破已落地**(2026-06-27)。PJ3 FORLOOP 字节级 inline 实测 7.15-25.41x over gopher-lua,**完整超越 luajc 档 4.4x 基线**(承 §8)。**PJ4 表 IC 完整六路径**(GETTABLE/SETTABLE/SELF × ArrayHit/NodeHit)字节级 inline 主路径接入 + 严密 IsTable guard(承 §9.7-§9.10) + 整套层级 prove-the-path 守卫(承 §9.11)。**PJ8 arm64 字节级模板矩阵完整 + Compile 端真接入**(承 §9.13)。**PJ5 CALL void 一百七十二子形态打通**(2026-06-27):analyzeCallVoidForm 识别 setter 70 子态(0/1 K/1 reg/2/3/4/5 参组合)× 双 callee + getter 1 返 94 子态(0/1 K/1 reg/2/3/4/5 参组合)× 双 callee + getter N>=2 返值 8 子态(0/1 K/1 reg 参 × {N=2/N=3 返,N=3 仅 0 参} × 双 callee)= 172 子形态;P4HostState 加 CallBaseline 接口;P2 scope-aware AnalyzeProto 跨 Proto 传递 outer localFnAsts(承 §9.14)。**PJ5 TAILCALL 七十子形态打通**(2026-06-27):analyzeTailCallForm 识别长度 4..9 共 70 子态(0/1 K/1 reg/2/3/4/5 参组合)× 双 callee;P4HostState 加 TailCall 三态分支接口(承 §9.15)。共 **31+12 e2e SpecCallVoidHits/SpecTailCallHits=1 prove-the-path 命中实证 + 30+11 difftest-p4 三方 byte-equal**。**剩 PJ5 完整接入(SELF method call + N>=2 返值多参 + 6+ 参 + OSR exit + 段内 EmitCallInline)/ PJ8 剩余真接入(archSupportsSpec 翻面 + 物理 runner)/ PJ9(双架构差分套)** 留多会话累积(P4 设计文档 §0 自估 +1-2 人年)。
...
| PJ5 | amd64 CALL/TAILCALL + 跨层互调 + OSR exit 实装 | [04](./04-osr-deopt.md) + [05 §4.3](./05-system-pipeline.md) + [06 §3.5](./06-backends.md) | gibbous-jit 三向分派 + OSR exit 状态等价(V19)| 🔶 **2026-06-27 CALL void 一百七十二子 + TAILCALL 七十子形态打通**(承 §9.14/§9.15 — CALL void setter 70 + getter 1 返 94(加 5 参 64 新子)+ N>=2 返值 8 = 172;TAILCALL 70(加 5 参 32 新子);P4HostState 加 CallBaseline + TailCall 三态分支接口;Run prelude CALL/TAILCALL case 真接入 + P2 scope-aware AnalyzeProto + 31+12 e2e SpecCallVoid/TailCallHits=1 命中实证 + 30+11 difftest-p4 三方 byte-equal;剩 SELF method call / N>=2 返值多参 / 6+ 参 / OSR exit / 段内 EmitCallInline 留下一批) |
