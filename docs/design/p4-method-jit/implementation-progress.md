...
| PJ2 | amd64 算术 + 比较 + IsNumber×2 guard | [03](./03-speculation-ic.md) + [06 §3.2](./06-backends.md) | 双 number 快路径直发 `mulsd` 等;guard 失败 OSR exit 回解释 | 🔶 **2026-06-25/26 完整 SSE 字节级原语 + 双 number ADD 模板真 mmap+RX 跑通**(承 §7;jitContext struct 加 SetArenaBase/SetValueStackBase 真注入 + 完整 SSE2 emit 原语:movsd/addsd/subsd/mulsd/divsd/ucomisd/全档 jcc + mov 内存读写 + EmitArithSpeculativeAdd 拼接 29 字节模板 + callJITSpec trampoline 装 BX=valueStackBase + TestPJ2_SpeculativeAddRoundTrip 多档浮点实测 byte-equal 解释器(3+4=7 / -10+3.14=-6.86)。**未接入 SupportsAllOpcodes 主路径**——剩余 IsNumber guard codegen + OSR exit 路径 + analyzeShape 接入,留 PJ2 完整接入版,见 PJ10 行差距分析) |
