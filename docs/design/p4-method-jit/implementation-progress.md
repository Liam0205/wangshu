...
**性能实证(Xeon Platinum,`-benchtime=2s -count=3 -cpu=1` median,同机同轮,2026-07-07,over gopher-lua)**:fib **0.91ms** vs **9.32ms** = **10.3×**;spectral-norm **2.11ms** vs **33.3ms** = **15.8×**(内层 A/Av/Atv 走段到段;P4 auto 只有 2.14×,force 全升层才吃满);fannkuch **0.60ms** vs **4.15ms** = **6.9×**;binary-trees **38.3ms** vs **51.5ms** = **1.35×**、n-body **42.5ms** vs **59.9ms** = **1.41×**(二者分配 / GC 密集,递归 callee 含表 op 不进段到段,收益主要来自其余路径)。issue #50 点名的三个 call 密集内核(fib / fannkuch / binary-trees)全部反超 gopher,spectral-norm 额外大幅获益。fib(20) 全程段到段(21870 hits,0 exit-reason fast-hit)。difftest-p4 / crescent / conformance-p4 / peroptranslator 全套 + `-race` + 2 万随机脚本 diff 全绿(逐字节等价)。
