...
| PJ5 | amd64 CALL/TAILCALL + 跨层互调 + OSR exit 实装 | [04](./04-osr-deopt.md) + [05 §4.3](./05-system-pipeline.md) + [06 §3.5](./06-backends.md) | gibbous-jit 三向分派 + OSR exit 状态等价(V19)| 🔶 **2026-06-27 CALL void 六子形态打通**(承 §9.14 — analyzeCallVoidForm 识别 0 参(A0/B0)+ 1 K 参(A1K/B1K)+ 1 reg 参(A1R/B1R)六子形态 + P4HostState.CallBaseline 接口 + Run prelude CALL case 真接入(含 LOADK/MOVE 参数装载)+ P2 scope-aware AnalyzeProto 跨 Proto 传递 outer localFnAsts 打通真升层 + 3 e2e SpecCallVoidHits=1 prove-the-path 命中实证;剩 TAILCALL / 多参 / 含返 CALL / OSR exit / 段内 EmitCallInline 留下一批) |
...
**形态范围**(双形态 × 0 参 + 1 K 参 + 1 reg 参,六子形态,luac 编译产物):
...
**测试覆盖**:
- 单测 11 个(`compiler_pj5_call_test.go`):RecognizeMOVE / RecognizeGETUPVAL / Recognize1ArgK / Recognize1ArgReg / Reject(9 子测覆盖每条守卫)/ RunCallVoidPath(形态 A 端到端)/ RunCallVoidUpvalPath(形态 B 端到端)/ RunCallVoid1ArgKPath(形态 A1K 端到端 LOADK 装载)/ RunCallVoid1ArgRegPath(形态 A1R 端到端 MOVE 装载)/ ErrPropagate / SupportsAllOpcodesGate / SpecCallVoidHits 探针
- e2e 3 个(`internal/crescent/gibbous_pj5_call_e2e_test.go`):FormB_Upval(B0)+ FormB1K_UpvalArg(B1K + sum=2100)+ FormB1R_UpvalArg(B1R + sum=1275)
- difftest-p4 4 个(`test/difftest/p4_test.go`):p4_call_void(A0)+ p4_call_void_upval(B0)+ p4_call_void_upval_1argk(B1K)+ p4_call_void_upval_1argreg(B1R)— oracle / crescent / p4-jit 三方 byte-equal
