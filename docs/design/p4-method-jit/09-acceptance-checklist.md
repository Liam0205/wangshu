...
>
> 更新于 2026-07-01 (跟随 PR #30 PJ10 native emit 交付 + V15b 三本 P4 native > P3 wasm 达标)。
...
| V15b | heavy 3 脚本 (arith / recursion / floatloop) — 升层档真正发挥场景, 「P4 geomean ≥ P3 geomean ≥ 1.5x gopher」 | ✅ (P4 11.6x / 1.44x / 15.9x, geomean ~5.4x) | ⬜ 待跑 | ⬜ 待跑 | PR #30 PJ10 native emit 交付后 P4 走 CFG-based 原生 code 路径, 三本 kernel 全接; amd64 本机 P4 geomean ~5.4x over gopher 且 ≥ P3 (P3 geomean 2.32x) 双条件达标; 两 arm64 待 bench-acceptance workflow 三平台跑; 详 §3.V15b |
...
| # | 描述 | amd64 | linux/arm64 | darwin/arm64 | 如何验 |
|---|---|---|---|---|---|
| V19 | OSR exit 状态等价 (每 guard 强制失败, exit 后续跑 byte-equal) | ⬜ | ⬜ | ⬜ | `conformance/p4_test.go::TestOSRStateEquivalence` (table-driven 每 guard) — 测试未写, 归 followup, 见 §4 |
| V20 | deopt 风暴防抖 (反复 deopt → `P4StuckSpeculation` 吸收态, 不再投机但仍 byte-equal) | ⬜ | ⬜ | ⬜ | `conformance/p4_test.go::TestDeoptStormToStuck` — 测试未写, 归 followup, 见 §4 |
| V21 | **三方逐字节比对**: amd64 输出 = arm64 输出 = crescent 输出 | ⬜ | ⬜ | ⬜ | `difftest/p4_test.go::TestDualArchByteEqual` (跨架构) — 测试未写, 归 followup, 见 §4 |
| V22 | guard 漏判 fuzz (禁用每条 guard 一次, 看比对能不能抓出来) | ⬜ | ⬜ | ⬜ | `difftest/p4_test.go::FuzzGuardOmission` (build tag `wangshu_p4_guardfuzz`) — 测试未写, 归 followup, 见 §4 |
...
本机 amd64 Xeon Platinum 24 核, `-benchtime=2s -count=3` 取平均 (2026-07-01 实测, 跟随 PR #30 PJ10 native emit 交付重跑; CI 两 arm64 平台数据待 bench-acceptance workflow 三平台跑)。

来源: `benchmarks/heavy/` 三个 flat numeric kernel 脚本, 故意去除表 / 字符串 / 库 CALL / short-circuit `and-or` / `if-then-end` / `break` 等会让 P3 relooper 拒升或早期 P4 PJ7 shape 白名单不命中的元素。
...
| 脚本 | gopher-lua | P3 (wasm) | P4 (jit) |
|---|---|---|---|
| heavy_arith | 152.4 | 86.8 | **13.15** |
| heavy_recursion | 7.97 | 5.77 | **5.53** |
| heavy_floatloop | 274.8 | 51.1 | **17.24** |

**加速比 (gopher / X, >1 表示 X 比 gopher 快)**:

| 脚本 | P3 | P4 | P4 / P3 |
|---|---|---|---|
| heavy_arith | 1.76x | **11.6x** | 6.60x |
| heavy_recursion | 1.38x | **1.44x** | 1.04x |
| heavy_floatloop | 5.38x | **15.9x** | 2.96x |
| **geomean** | ~2.3x | **~5.4x** ✅ | ~2.7x |

**关键发现**:

1. **PJ10 native emit 把 V15b P4 从「落回 P1」搬回「真原生 code」**: PR #30 之前 P4 PJ7 shape 白名单不接 heavy kernel, 走 crescent 后 P4 数字 ≈ P1 (1.60x); PR #30 交付后 P4 kernel proto 直接进 CFG-based 原生 code, 三本 kernel 全接。P4 heavy_arith 11.6x / heavy_floatloop 15.9x 都是 P4 jit 自己算出来的速度, 不再是 P1 走垫。
2. **P4 heavy_floatloop 15.9x over gopher**: 嵌套 FORLOOP + 内层 while 单条件 + 浮点算术, PJ10 CFG 化 + native emit 后, 内层直接跑原生浮点算术 + 边界 label resolver 一次算好, 比 P3 wazero 翻译 (5.38x) 再快 ~3x。
3. **P4 heavy_recursion 1.44x 相对 flat**: 递归形态 CALL 边界成本占比高, 内层 kernel 计算量小, P4 native emit 的边际收益被 CALL 拖低; 但 P4 仍略胜 P3 (1.04x P4/P3), 且高于 gopher (1.44x)。
4. **两条独立断言全部达标**:
   - **P4 geomean ≥ P3 geomean**: ✅ P4 ~5.4x > P3 ~2.3x (P4/P3 geomean ~2.7x)
   - **P4 geomean ≥ 1.5x over gopher-lua**: ✅ P4 ~5.4x 远超 1.5x 门槛

**结论**: V15b amd64 ✅ PASS。PJ10 native emit 是 V15b 三本 P4 native > P3 wasm 达标的直接交付载体; V15 历史承诺 「P4 geomean ≥ P3 geomean ≥ 1.5x gopher」 在 heavy 升层档真发挥场景上第一次实测 P4 侧兑现。两 arm64 平台 (linux-arm64 / darwin-arm64) 数字待下一轮 bench-acceptance workflow 三平台跑, 非阻塞项。
...
- **V15 「≥1.5x over gopher」 已经在 V15b heavy 上兑现 (P4 ~5.4x)**: 设计文档 [./08-testing-strategy.md](./08-testing-strategy.md) §2.2 V15 「P4 geomean ≥ P3 geomean ≥ 1.5x over gopher-lua」 拆成两条:
  ...
  - V15b (heavy 3 脚本, 升层档真发挥): **P4 geomean ~5.4x ≥ 1.5x** ✅ 本机 amd64 (heavy_arith 11.6x / heavy_recursion 1.44x / heavy_floatloop 15.9x); **P4 geomean ≥ P3 geomean** ✅ (P4 ~5.4x > P3 ~2.3x, P4/P3 geomean ~2.7x); 两 arm64 平台数字待 bench-acceptance workflow 三平台跑

  **关键发现**: PR #30 PJ10 native emit 交付后, V15b P4 从「PJ7 shape 白名单不接 → 落回 P1」搬回「CFG-based 原生 code 真接」, V15 「P4 ≥ P3 ≥ 1.5x gopher」 两条独立断言在 heavy 升层档真发挥场景上第一次同时兑现 (amd64 本机)。realworld 上 P1+P3+P4 都达不到 1.5x, 是工作量结构原因 (helper-bound), 不是 P4 引入退化。

  **PJ11 接收**: V15a (realworld) 保留 「P4 ≥ P3」 判; V15b (heavy) amd64 ✅ 双条件全达标, 两 arm64 平台 ⬜ 待 bench-acceptance workflow 跑, 非阻塞。详 §3.V15a + §3.V15b。
...
- **V19-V22 测试尚未编写**: 四条 P4 增项测试 (`TestOSRStateEquivalence` / `TestDeoptStormToStuck` / `TestDualArchByteEqual` / `FuzzGuardOmission`) 在 codebase grep 均无匹配, 归 followup 工作项。当前 2026-07-01 验收快照不把 V19-V22 计入 §5 完成条件的阻塞项, 但 PJ11 最终宣告完成前须补齐; 补写归属放 [./implementation-progress.md](./implementation-progress.md) followup。
...
- [ ] §1 表格三平台 × V1-V22 全部 ✅
  - [x] V1-V14 三平台 ✅
  - [x] V15a 三平台 ⚠️「P4 ≥ P3」 判达标 (「≥1.5x gopher」 这条对 realworld 工作量结构不成立, 已在 §4 澄清)
  - [x] V15b amd64 ✅ (PR #30 PJ10 native emit, P4 ~5.4x 双条件达标); [ ] V15b linux-arm64 / darwin-arm64 ⬜ 待 bench-acceptance workflow 跑
  - [x] V16 amd64 ✅ 本机; [ ] V16 linux-arm64 / darwin-arm64 ⬜ 待 bench-acceptance workflow 跑
  - [x] V17 三平台 ✅ (PR #29 CI 三 variant 覆盖, `wangshu_profile` 独立 variant 缺口已在 §4 澄清)
  - [x] V18 三平台 ✅ (PR #29 CI matrix 三平台 -race)
  - [ ] V19-V22 测试尚未编写, 归 followup (见 §4), 非 2026-07-01 快照阻塞项
- [ ] §2 D2 P3 退役决议已拍板 (无论退役还是保留)
- [ ] §3 性能数字三平台全部填写完毕 (amd64 已填, 两 arm64 待 bench-acceptance workflow 跑)
- [x] §4 不一致的地方 mostly cleared, V19-V22 followup 已登记
