...
- **多 State 并发下 CodePage 引用计数 + 延迟 munmap 已落地 (2026-07-01)**: 原设计文档 [./05-system-pipeline.md](./05-system-pipeline.md) §2.1.3 承诺「留 PJ7 验收期落地引用计数 + 延迟 munmap」, 此前 `internal/gibbous/jit/amd64/codepage_linux.go::Munmap` 注释登记为 「多 State 并发下 UAF 开放缺口」。本批交付落地 refcount 协议 (承 [../../../llmdoc/memory/reflections/2026-07-01-p4-codepage-refcount.md](../../../llmdoc/memory/reflections/2026-07-01-p4-codepage-refcount.md), 若存在):
  ...
  - `code.Run` + `PerOpCode.Run` 顶部 `Enter` 失败即返错 (不撞已释放段), defer `Exit` 保证 Run 出口 refcount 回归
  - `p4Code.Dispose` + `PerOpCode.Dispose` 转 `CodePage.Dispose` 触发 flip flag + 延迟 munmap
