...
>
> 更新于 2026-07-01 (跟随 PR #30 PJ10 native emit 交付 + V15b 三本 P4 native > P3 wasm 达标 + D2 主动保留定下来 + V19-V22 引用统一)。
...
- `test/difftest/p4_test.go::p4Corpus` 共 116 个 P4 专属语料,按形式分类 (PJ5 调用族 89 个 / 表 IC 4 个 / 算术 6 个 / 比较 2 个 / FORLOOP 2 个 / 直线 opcode 3 个 / 等)
...
**(b) arm64 P4 主动回归**: PJ10 must-beat-P3 op 集扩面在 amd64 上补齐了 CALL / SETTABLE / NEWTABLE / UNM / GETUPVAL / SETUPVAL / GETGLOBAL / SETGLOBAL / 多值 RETURN 等一批 op (承 memory `project_pj10_native_longtask.md`), 但 **op 集扩面是 amd64-only**; arm64 侧未同步扩面。三平台 bench-acceptance run 28575495631 实测 arm64 P4 相对 P3 出现主动回归:
...
- **对 PJ11 状态的影响**: PJ11 已交付状态不撤 (交付时点数字全绿); arm64 回归归 followup (issue #37 / #40), 修复方向是把扩面 op 集 port 到 arm64 exit-reason 协议。

**(c) 权威现值**: 项目根 `README.md` perf table (commit f736c92, 2026-07-02) 是当前的三平台性能权威快照; 本文 §3 各 V 归档表是**验收时点的**证据快照, 两者并存不冲突 — §3 表证明「PJ11 宣告时点数字达标」, README perf table 反映「当前跟进后的现值」。若两处数字出现矛盾, 以 README perf table 为准 + 本 addendum 澄清成因。
...
- [x] §2 D2 P3 退役决议已定下来 (2026-07-01 用户选主动保留, §10.2 形式)
