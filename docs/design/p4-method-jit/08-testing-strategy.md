...
**P4 fuzz harness 的 step budget 设定(#224/#225,2026-08-04)**:`FuzzAutoPromote`(`fuzz_auto_test.go`,两个 State)与 `FuzzP4ForceAllPromote`(`fuzz_p4_test.go`,两个 State)四处 `SetStepBudget` 共用**一个**常量 `fuzzbudget.Steps`(`internal/fuzzbudget`,build tag 与消费方一致:`(wangshu_p3 || wangshu_p4) && wangshu_profile`),当前值 **`1<<16`**。它从 `1<<20` 降下来的理由是**余量**而不是正确性:`1<<20` 在 1 步 / 64 字节的比率下允许约 64 MiB 的 concat,本地每个 fuzz 子测试 0.7–1.3 秒,而 CI 运行器比本地慢约 10 倍(见 `internal/crescent/state.go` 的 `chargeBulkWork` 注释),于是 concat storm 家族最慢的 seed 投射到 CI 是 12–13 秒,撞上 go-fuzz 的 **10 秒 per-input 看门狗**——六个家族 seed 里两个已经超过、四个余量不到 1.4 倍,nightly 日志印证的正是那个看门狗(`panic: deadlocked`)。

**这个值是由「计费相同时最贵的写法」定的,不是由被开成 issue 的 seed 定的**:第一版只减半到 `1<<19`,那六个 seed 都拿到了 ≥1.8 倍余量,而审计量出它们的邻域仍在看门狗之上——按四次 Run × 慢 10 倍投射,`1<<19` 下 `out=out.."x"` 循环 14 秒、`t[tostring(i)]=i` 循环 18 秒、`s:gsub("%a","x")` 循环 **41 秒**(看门狗的四倍,比被修的 seed 还糟)。`gsub` 每次调用按大约两倍主串计费,可实际工作远多于等额计费的 concat,所以**等额计费不等于等额 wall-clock**。`1<<16` 让最坏那条降到 5.2 秒、余量 1.93 倍,是第一个满足数量级判据的值;corpus 全量重放 5.5 秒 → **0.85 秒**。**覆盖不损失是量出来的**:harness 自己的 seed corpus 在 `1<<20` / `1<<19` / `1<<16` 三个预算下 `PromotionCount` 完全相同(那些写法几次调用后就升层、从不接近任何一个上限);另外注入一个真实 P1-vs-P4 分歧确认 `1<<16` 下检测能力未受影响,90 秒引导式 fuzz 干净。据实记下变窄的一处:`1<<20` 时有两个 seed 会把 arena 推到上限、`1<<16` 时没有,所以那条 arena-cap 错误分支在这里覆盖到的写法变少了,但 `test/regression/issue144_regression_test.go` 直接覆盖 arena cap,且两个预算下 step budget 都先于 arena cap 触发。

判据与「余量必须差一个数量级」的算法见 [../p1-interpreter/12-testing-difftest.md](../p1-interpreter/12-testing-difftest.md) §4.9a2;回归 `test/regression/issue224_watchdog_margin_test.go`(五个用例,后三个才是真正约束预算的写法——只用被开成 issue 的那两个 seed 分辨不出 `1<<16` 与 `1<<19`;上界是推导出的 250 毫秒 = `10 秒 / 10 倍 / 4 次 Run`,已用变异实测确认它能同时抓住 `1<<19` 与 `1<<20`)。**动这个常量之前先重算余量**:它同时约束 `FuzzAutoPromote` 的 4 次 `prog.Run` 与 `FuzzP4ForceAllPromote` 的 2 次,调高会直接吃掉看门狗余量,而且要按同一计费额度下最贵的写法重算、不要只量手里的 seed。
