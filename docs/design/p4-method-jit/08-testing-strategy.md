...
**P4 fuzz harness 的 step budget 设定(#224/#225,2026-08-04)**:`FuzzAutoPromote`(`fuzz_auto_test.go`,两个 State)与 `FuzzP4ForceAllPromote`(`fuzz_p4_test.go`,两个 State)四处 `SetStepBudget` 共用**一个**常量 `fuzzStepBudget`(`fuzz_budget_test.go`,build tag 与消费方一致:`(wangshu_p3 || wangshu_p4) && wangshu_profile`),当前值 **`1<<19`**。它从 `1<<20` 减半的理由是**余量**而不是正确性:`1<<20` 在 1 步 / 64 字节的比率下允许约 64 MiB 的 concat,本地每个 fuzz 子测试 0.7–1.3 秒,而 CI 运行器比本地慢约 10 倍(见 `internal/crescent/state.go` 的 `chargeBulkWork` 注释),于是 concat storm 家族最慢的 seed 投射到 CI 是 12–13 秒,撞上 go-fuzz 的 **10 秒 per-input 看门狗**——六个家族 seed 里两个已经超过、四个余量不到 1.4 倍,nightly 日志印证的正是那个看门狗(`panic: deadlocked`)。减半之后最慢子测试 1.34 秒 → 0.56 秒、六个 seed 全部 ≥1.8 倍余量、corpus 全量重放 5.5 秒 → 2.9 秒,而**覆盖不损失**(这一族每种写法在两个预算下都触发,fuzzer 走同样的路径、只是更早停下;已用注入一个真实 P1-vs-P4 分歧的办法确认检测能力未受影响)。判据与「余量必须差一个数量级」的算法见 [../p1-interpreter/12-testing-difftest.md](../p1-interpreter/12-testing-difftest.md) §4.9a2;回归 `test/regression/issue224_watchdog_margin_test.go`。**动这个常量之前先重算余量**:它同时约束 `FuzzAutoPromote` 的 4 次 `prog.Run` 与 `FuzzP4ForceAllPromote` 的 2 次,调高会直接吃掉看门狗余量。
