...
- ✅ P1 期间的前瞻义务全部已完成(详见 §6;部分项目 P2 PB0 同批补齐——见 §6 修正对账)
...
|---|---|---|---|---|---|
| #1 | 精确 yield 分析(stdlib 白名单 + known local 调用图传递) | [03 §3.2 + §9 GAP-1](./03-compilability-analysis.md) | type/tostring/math.*/string.*/table.* 等不再标 unknown;known local 含 yield 父也判 F2 | ✅ | a20b8a7(白名单 + 12 档单测);PB3 cf0a326 已完成 known local 真实现 |
| #2 | 阈值实测校准 | [01 §5 + 03 §3.5/§3.6](./01-profiling.md) | 阈值常量保留设计建议值,P3 真完成后实测校准 | ✅(占位) | a5ebef1(threshold 占位测试 + 漂移防御断言) |
...
| #5 | F2-c ReasonSelfCall 占位位拆分(P4 PJ5 SELF inline 接上前置)| [03 §4.2 visitMethodCallExpr](./03-compilability-analysis.md) | visitMethodCallExpr 不再硬叠 callsUnknownFn,改标 sawSelfCall + ReasonSelfCall 占位位;recheckCompilabilityRuntime 占位位扩到 (ReasonBackendUnsupp \| ReasonSelfCall);std_logger formatReasons F2 多位合并加 selfCall;analyzer_test 加 TestAnalyze_F2c_SelfCall 验占位位置位 + ReasonUnknownCall 不叠加 | ✅ | ee17319(2026-06-28,P4 PJ5 SELF method call inline 接上前置)|
...
| #7 | PromotionGater 收益门(issue #39,P3 wasm helper 密度)| [04 §3.2 addendum 第 4 条](./04-try-compile-fallback.md) | bridge 加可选接口 WorthPromoting(auto-only,forceAll 绕过,拒绝 → TierStuck 吸收);P3 wasm Compiler 实现 helper 密度地板(plain-op/helper-bound-op ≥ 7)——45b8b53 解锁 nbody 热 proto 升层后 P3 auto 43.5→89.7ms(慢 ~2x),收益门拒之留在解释器;state_machine_test 加 2 档(auto 拒绝吸收不重复问 / forceAll 绕过不咨询)| ✅ | 本轮(2026-07-03,Xeon Platinum 实测 nbody auto 89.7→43.2ms、fib 24.9→10.9ms、binary-trees 104→38.4ms、spectral-norm 40→20.7ms;heavy 三本 P3 赢面不动)|
...
| # | 来源 | 内容 | 实施阶段 |
|---|---|---|---|
| RB-7 | [03 §10](./03-compilability-analysis.md) | 把 `SupportsAllOpcodes(proto) bool` 加进 `P3Compiler` 接口 | ✅ 已完成(本期 PB0)——P3 PR 实现该方法即可 |
...
设计文档 [00-overview §7](./00-overview.md) 声称 P1 已完成的若干前瞻义务,实代码扫描发现部分项目实际未完成——P2 PB0 同批补齐:

| 前瞻义务 | 文档原状态 | 实代码状态 | PB0 同批补齐 |
|---|---|---|---|
| 算术 IC 双计数(numHits/metaHits 挪用 shape/index/tableRef) | ✅ 已完成 | ❌ ICSlot 字段已建,但 doArith / UNM / LT/LE 路径**未写入** | ✅ PB2 commit `82881ea` 完成 |
...
| `enterLuaFrame` 函数入口采样钩 | ✅ 已完成 | ⚠️ enterLuaFrame 函数本身存在,但**钩点不存在** | ✅ PB1 commit `ade65f4` 接钩 |
| `Compile` 可编译性探测占位 | ✅ 已完成(恒「可解释」) | ✅ 真——`wangshu.Program` 公共 API 不变 | 无需补 |
| CallInfo bit50 gibbous 位 | ✅ 已完成(P1 恒 0) | ❌ callInfo 是普通 Go struct,无 word2 位打包 | ⚠️ PB0 commit-3 加占位 bool,但 lint 抓未使用,`ecffcb9` 删除——P3 trampoline 真完成时再加更合理(语义记在 installGibbous 注释里) |

**结论**:实现现状对账后,P2 PB0 不仅是「翻开关」,而是同批建立了上述钩点机制。设计文档 §7 的「✅ 已完成」声明在某些项目上是设计期的 wishful thinking,实际代码层面的 P2 PB0 = 「钩点首次建立 + 把 considerPromotion 占位」。文档 §7 已在本轮收尾保持原状,因为对未来读者来说「已完成」是当前状态(P2 PB0 之后)的事实。
...
| 决策 | 当前 | 校准条件 |
|---|---|---|
| `HotBackEdgeThreshold = 1000` | 建议值,bridge 包常量 | P3 真完成后用 pineapple 真实负载校准 |
...
| 表格主体 | 实际完成的 PR / 提交哈希 / 时间线 | 一样的 |
| 与设计文档的差异 | 已完成形式与设计文档的差异(如简化 / 留口) | 一样的(§3 P1 前瞻义务对账修正、§4.2 阈值待校准) |
| 后续维护 | 每轮里程碑完成后追加对账行 | 每轮 P2 后续优化或 P3 接入时追加对账行 |
...
1. P2 后续优化轮(精确 yield 分析 / 阈值校准 / (C) sync.Pool 双表混合启用 / fuzz 误判注入扩展 / megamorphic 主动识别;设计文档原称 `P2+`)完成时追加对账行;
