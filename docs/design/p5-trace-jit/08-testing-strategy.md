...
> 静默错果候选,组合空间远超 P4。差分主防线在 P5 到达顶点(承 09-acceptance-checklist §1)**。测试策略的一切设计都从
> 「差分是无歧义统一法庭」推出:三层 byte-equal 硬门禁(§2)、per-guard 强制失败注入(§3)、per-pass
...
> 时长决定」);
> [../../../llmdoc/must/design-premises.md](../../../llmdoc/must/design-premises.md)(原则 2 「投机错果
...
> [04-optimization-passes.md](./04-optimization-passes.md)(pass 开关支持,承 §4 差分矩阵);
> [05-register-allocation.md](./05-register-allocation.md)(regalloc verifier 单测,承 §6);
> [06-snapshot-deopt.md](./06-snapshot-deopt.md)(deopt 注入 hook 是 PT5 必交付,承 §3.5)。
...
承 [09-acceptance-checklist §1](./09-acceptance-checklist.md) 第一段字面:

> P5 的每一项机制——录制的类型假设、每个优化 pass 的语义保持、snapshot 的状态重建——都是「静默错果」
> 候选,且组合空间远超 P4。
...
| regalloc 与 snapshot 耦合错 | exit 时某 IR 值预期在寄存器实际已被覆写 | regalloc verifier(§6)+ deopt 注入联合验 |
...
|---|---|---|
| 验收编号 | V1-V22(V1-V18 P3 接续 + V19-V22 P4 增) | **T1..Tn**(承 §1.5;09 拥有具体编号)|
...
| deopt 注入 | P4 spec'd 但**从未落地**(承 P4 08 §5.2 2026-07-02 addendum) | **PT5 必交付**(§3.5);P5 侧没有替代路径 |
| pass 开关差分 | 无(P4 模板编译,无 pass) | **§4 全新增**——与 [04](./04-optimization-passes.md) §8 的开关契约配套 |
| fuzz 常驻 | nightly 2h | **常驻 fuzz 集群**(09-acceptance-checklist §1「升格为专用集群」)|
| `-race` 兼容 | P4 fuzz `-race` skip(承 P4 08 §3.4)| **同款 skip**——P5 mmap 段一样与 Go race detector 不兼容(§8.2)|

### 0.3 章节路标

| § | 主题 | 关键产物 |
|---|---|---|
| §1 | 测试哲学 + T-item 类目 | 差分四支柱 + 三层同 Proto oracle |
| §2 | 三层 byte-equal 差分(主防线)| Runner 扩展 + force-all-trace + IC 预热 + CI 硬门禁 |
| §3 | Deopt 注入(P5 主动踩热每条 exit)| PT5 必交付 hook + 每 guard 强制失败 + 联合验 |
| §4 | Pass 开关差分矩阵 | 与 04 §8 契约 + 首个 divergent config 定位 pass |
| §5 | FuzzP5ForceTrace(常驻 fuzz)| P4 fuzz 模式扩展 + 语料 + 崩溃回流纪律 |
| §6 | 单元级测试 | IR printer golden + FOLD 规则表测试 + regalloc verifier + snapshot roundtrip |
| §7 | 性能验证(引用 09) | 无回归门 + 常驻 workload 加速门 |
...
承 [architecture §4 不变式 2](../architecture.md)「层间逐字节差分是 CI 必过门禁」+ [design-premises 原则
2](../../../llmdoc/must/design-premises.md)「JIT 最危险 bug 类 = 投机错果 → 层间差分是主防线」。

P5 的差分不是「多做一层」,是**四层原则同时到位**:

| 差分口径 | 抓什么 | 强度 |
|---|---|---|
| **三方**(crescent vs gibbous vs fullmoon) | 同 Proto 三层执行 | ★★★★★(无实现差异噪声,任一分叉必是 P5 bug)|
| **同层跨 pass 组合**(record-only / +FOLD / +CSE / …) | 定位单条 pass 引入的错果 | ★★★★(直接指认 guilty pass)|
| **同 pass 组合跨输入种子**(FuzzP5ForceTrace) | 覆盖未想到的输入 | ★★★★(时间维度累积)|
| **同输入跨 guard 强制失败位置** | 每条 exit 物化无损 | ★★★(把稀疏的 deopt 变确定性)|

四层同时到位才是 P5 测试合格。缺任一层的等价影响见下表:

| 缺失层 | 后果 |
|---|---|
| 无三层 byte-equal | 录制期假设错 / 优化 pass 错果无法可靠抓住 |
| 无 pass 差分矩阵 | 撞出错果时无法定位是哪 pass 引入,回归修复困难 |
| 无常驻 fuzz | 只跑「想到的」用例,罕见输入静默通过 |
| 无 deopt 注入 | snapshot 物化路径基本没被真正测过(自然 deopt 稀疏) |

### 1.2 P1 是 oracle,永远

承 [P1 12 §9 不变式 10](../p1-interpreter/12-testing-difftest.md)「JIT 主防线复用 P1 harness——解释器永
不退役,是所有上层的 oracle」+ [00-overview §3](./00-overview.md)「fullmoon 的 deopt 着陆点是 crescent 而非
...
**P4 gibbous 是三方差分的独立参照物,不是 oracle**——P4 与 fullmoon 撞不同 bug 类,若两者不一致而
...
承 [P4 08 §3.4 2026-07-02 addendum](../p4-method-jit/08-testing-strategy.md):P4 fuzz 在 `-race` 构建下
...
**P5 同款约束**:fullmoon trace 也在 mmap 段跑,`-race` 不兼容原样继承。含义:
...
| **T-FUZZ**(常驻 fuzz) | FuzzP5ForceTrace / GC 压力联合 / 长跑天数 | §5 |
| **T-UNIT**(单元级) | IR printer / FOLD 表 / regalloc verifier / snapshot roundtrip | §6 |
| **T-PERF**(性能) | 无回归 + 常驻 workload 加速门 | §7(引用 09)|
...
承 [P1 12 §3.8](../p1-interpreter/12-testing-difftest.md) 的 Runner 抽象——P5 落地时**填 slot,不改
harness**:

```go
// test/difftest/runner.go —— P1 已建抽象;P3 已加 WangshuGibbous;P4 已加 WangshuGibbousJIT;
// P5 落地时新增 WangshuFullmoon(P1 12 §3.8 已注释预留):
...
**为什么 P5 也需要 force-all**:承 [P4 08 §3.7](../p4-method-jit/08-testing-strategy.md) + [prove-the-path-under-test §2(b)](../../../llmdoc/guides/prove-the-path-under-test.md)——正常热度阈值下 fullmoon 覆盖率与
...
**警告**(承 SEED §2「trace 录制的起点是热回边」+ P4 08 §3.7 RW-10 教训):**fullmoon force 录制模式需
要 IC 预热,不能开机即录制**:

- P5 录制依赖 IC 反馈(承 [02](./02-trace-recording.md) + [P4 03 §3](../p4-method-jit/03-speculation-ic.md)
  同源 IC feedback)决定投机类型 —— 循环里 `x + y` 若 IC 未热,不知道 x、y 是 number 还是 string,录制器
  没依据下 guard;
- P4 的 `SetForceAllPromoteJIT` 也吃了这个亏(承 [bridge.go 力争的 warmed-IC retry window](../p2-bridge/04-try-compile-fallback.md)):升 P4 前必须等 IC 计数够,否则投机模板生成质量差、大量 deopt;
...
crescent 一致但 fullmoon 覆盖率为零 —— 承 [prove-the-path-under-test §2(b)](../../../llmdoc/guides/prove-the-path-under-test.md) 正向 tier 断言:
...
承 [P1 12 §3.3](../p1-interpreter/12-testing-difftest.md) 三方矩阵扩展:
...
| == | == | ≠ | ≠ | 双 backend 同款错——可能是共享路径 bug(如 P2 IC feedback 错果传下去)|
| == | ≠ | any | any | crescent 独立错(P1/P3 已建防线应先抓)|
| ≠ | == | == | == | 三层皆偏离 oracle——如 gopher/官方偏差(承 P1 12 §4.7)|

**层间差分是硬门禁**:fullmoon 孤立错(第 2 行)PR 直接不合入。

### 2.5 CI 硬门禁

承 [architecture §4 不变式 2](../architecture.md) + [P4 08 §3.5](../p4-method-jit/08-testing-strategy.md):

| 阶段 | 跑什么 | 时长 | build tag |
|---|---|---|---|
| **PR** | T-DIFF 形状级单测 + force-all-trace 小语料 byte-equal + T-OSR 每 guard 强制失败一次 + T-PASS 小矩阵 | < 15 min | `wangshu_p5,wangshu_p4,wangshu_p3,wangshu_profile` |
...
## 3. Deopt 注入(P5 主动踩热每条 exit)

### 3.1 P5 侧「不注入 = 结构性失明」

承 [P4 08 §5.1](../p4-method-jit/08-testing-strategy.md):deopt 路径天然稀疏(投机 confidence ≥0.99 时
99% 走快路径,1% 才 deopt),普通 fuzz 撞不出。

P5 侧更严:

- fullmoon 的 snapshot 比 P4 复杂(承 [06-snapshot-deopt §3](./06-snapshot-deopt.md) 表:P4 = 空;P5 = frames[1..N] +
  slots{稀疏} + sinks[]);
- P4 的 OSR exit「栈槽即真相」,exit 序列几乎空——正常 e2e 撞到 deopt 也不测什么;
- **P5 每次 deopt 都是「按 snapshot 逐帧物化 + unsink」,是一整套编译期烧入的 store 序列**——每条 exit
  的物化配方都不同,必须每条独立踩一次才验证得到。

### 3.2 每 guard 强制失败一次

承 [P4 08 §5.2](../p4-method-jit/08-testing-strategy.md) + [prove-the-path-under-test §2(a)](../../../llmdoc/guides/prove-the-path-under-test.md) 毒化哨兵思路。
...
「全程 crescent」byte-equal 才通过,是 T-OSR 的核心口径。

### 3.5 PT5 必交付,不是 optional(P4 教训)

**关键警示**(承 [P4 08 §5.2 2026-07-02 addendum](../p4-method-jit/08-testing-strategy.md)):P4 侧的 spec
了 `SetGuardForceFailFirst` + `SetDeoptInjectEvery`——**两个 hook 均未实现**,P4 08 §5.2 复核标注为
followup。P4 靠 e2e 自然覆盖侥幸没炸,但 spec 的深度验证从未跑通。

**P5 侧不能重复这个错误**:

- P5 snapshot 物化路径的复杂度远超 P4(P4 = 空 vs P5 = frames+slots+sinks),自然 e2e 覆盖不到每条 exit;
- **PT5 必须交付 deopt 注入 hook**,并在 PT5 验收时跑通完整 T-OSR;
- 若 PT5 只出「hook 未实现」就往后推,PT9 端到端撞出 snapshot bug 时定位成本会指数级增加(承 06-snapshot-deopt §4
  「正确性收敛不可计划」的最坏形态)。

**PT5 里程碑必须包含**:①`SetGuardForceFailFirst` 落地 ② `TestEveryGuardForceFailOnce` 全绿 ③每 fb kind
× 每层 guard 至少 1 例(承 [P4 08 §4.3](../p4-method-jit/08-testing-strategy.md) 覆盖矩阵)。

### 3.6 GC 压力联合验(承 P4 08 §7.4)
...
纪律),高频 GC 会必现撞出——把偶发变必现(承 [P1 12 §5.2](../p1-interpreter/12-testing-difftest.md))。
...
承 [04-optimization-passes.md §8](./04-optimization-passes.md)(未来章):每条 pass 必须提供独立关闭开关,
接口约定:
...
按累积增益方向:record-only 基线 + 逐步加 pass。**首个出现差异的 config 直接指认 guilty pass**(承 SEED
§5 第三点字面):
...
承 [P4 08 §2.4 V22 表](../p4-method-jit/08-testing-strategy.md):P4 实现形态是 `FuzzP4ForceAllPromote`
(force-all promote + error-existence 差分 + byte-equal差分)。P5 侧延伸:

```go
//go:build wangshu_p5 && wangshu_profile && !race
// (race build tag exclusion 承 §1.4)

// FuzzP5ForceTrace —— 常驻 fuzz(build tag wangshu_p5,-race 下自动 skip)
// 承 P4 08 §2.4 V22 表 + 09-acceptance-checklist §1 「fuzz 时长决定正确性置信度」
func FuzzP5ForceTrace(f *testing.F) {
    // 语料 seed:P4 已建 corpus + P5 独有形状(§5.2)
    ...
    f.Fuzz(func(t *testing.T, seed []byte) {
        ...
        // ① error-existence 差分(承 V22 addendum)
...
P4 fuzz corpus 覆盖 F1-F7 边界与算术投机形状 —— 对 P5 不够。**P5 独有关注形状**(与
[02 §NYI](./02-trace-recording.md) 协调):

- **循环内跨函数调用**(trace 内联主场,承 [01-launch-judgment §2 第一类负载](./01-launch-judgment.md));
- **循环内类型不稳定**(loop 里 var 时 number 时 string,触发多变态 guard);
- **循环内 metamethod 触发**(force 走 helper vs sink 后合并);
- **循环内分配密集**(触发 sink 优化的物理形态);
- **循环内表增删**(触发 gen bump + trace IC 失效);
- **多层嵌套循环**(不同 anchor 交互);
- **深递归 + 循环**(trace 内联导致 snapshot frames 深度 > 3)。

seed corpus 位置:`testdata/fuzz/FuzzP5ForceTrace/`(与 P1/P4 fuzz corpus 同款目录规范)。

### 5.3 常驻 fuzz cluster(09-acceptance-checklist §1 承诺)

09-acceptance-checklist §1「持续 fuzz 作为常驻基础设施——P5 的正确性置信度 = fuzz 时长的函数,§8 的 nightly 长跑在 P5 期间
应升格为专用 fuzz 集群」。
...
- **`-cpu=1`**(与共用户不打架);
- 分片按 seed base 时间戳(与 [.github/workflows/nightly-diff-fuzz.yml](../../../.github/workflows/nightly-diff-fuzz.yml) 同款每日 rolling)
- **累积统计**入 `results/p5-fuzz-hours.md`(与 [09](./09-acceptance-checklist.md) 的 T-item 数字对齐);
- **fuzz cluster 是逻辑概念,不是物理独立机器**——共享机上按时段调度 + 记账。

### 5.4 崩溃 corpus 回流纪律

承 P4 08 §3.4 + issue #36 nightly crash corpus 先例:fuzz 撞出崩溃 →
最小化(承 [P1 12 §3.6](../p1-interpreter/12-testing-difftest.md))→ 入 `testdata/fuzz/FuzzP5ForceTrace/`
seed → 未来运行时自动重跑(fuzz 引擎语义)。

**关键**:seed 入库是**回归防线**,而非「fuzz 白撞白算」。P5 侧因组合空间超 P4,corpus 增长会更快,预
留 `testdata/fuzz/FuzzP5ForceTrace/` 目录容量与 CI 时长预算(每 seed 重跑 ~100ms,1000 seeds ≈ 100 秒
CI 时长)。

### 5.5 -race 明确 skip 与替代覆盖

承 §1.4:P5 fuzz `-race` 下 `t.Skip("mmap+morestack incompatible with race detector")`。**替代覆盖**:

- non-race build 下的 fuzz(时间维度累积);
- non-race build 下 `go test ./test/difftest/... -count=10`(多次执行撞 race condition);
- non-race build 下 nightly 长跑(数百万迭代累积);
...
承 [03-ir-design.md](./03-ir-design.md)(未来章)的 IR 文本形式:每条 pass 前后 dump IR,golden 比对。
...
golden 更新 diff 在 PR review 显眼(承 [P1 12 §4.8](../p1-interpreter/12-testing-difftest.md) 豁免清单
的可审计性原则)。

### 6.2 FOLD 规则表测试

每条 FOLD 规则(承 [04 §3](./04-optimization-passes.md))单测:

- 输入 IR(手构造,不经录制);
- 期望输出 IR(手写);
- 边界 case:NaN、-0、GC 移动语义、元方法可见性(某些「常量」不能 FOLD 因为可能有 metatable)。

### 6.3 regalloc verifier(承 [05 §8](./05-register-allocation.md))

regalloc 后的 IR 逐条断言:

- 每条 IR 用到的 vreg 都有物理寄存器或 spill 槽;
- exit 时活着的 IR 值都可被 snapshot 引用(承 [06 §4.2](./06-snapshot-deopt.md) regalloc 与 snapshot 的
  耦合契约);
- 无未定义使用(use before def);
- 无死写(def without use);
- callee-saved 语义正确(承 P4 06 §4.1.5 Go ABIInternal 兼容)。

verifier 是 dev-only assertion(build tag `wangshu_p5_verify`),不进 release build。

### 6.4 snapshot encode/decode roundtrip

承 [06 §3](./06-snapshot-deopt.md):snapshot 表按压缩形式编码。roundtrip test:
...
编码错误会直接把 deopt 物化搞错,是最容易撞的 bug 类之一,单元层就应该覆盖。
...
**方法学要点**(承 [P4 08 §8](../p4-method-jit/08-testing-strategy.md) + [perf-optimization-workflow](../../../llmdoc/guides/perf-optimization-workflow.md)):

- **基准形态硬约束**([P1 12 §6.1](../p1-interpreter/12-testing-difftest.md) 列内核):循环在 Lua 内、
  一次 CallFn 进 VM;
- **P4 无 regression 门**(P4 build 关或开-but-cold 都不能比 P4 单跑慢);
- **prey workload 加速门**(SEED §1.2 四类「P4 结构性吃不下的负载」,PT9 端到端时给具体数字);
- **profile 优先于目标数字**(承 [perf-optimization-workflow §7](../../../llmdoc/guides/perf-optimization-workflow.md)):PT9 若不达预期,先跑 cpuprofile 定位瓶颈,不强刷数字;
- **跨机器复测同硬件**([P4 08 §8.4](../p4-method-jit/08-testing-strategy.md));
- **常驻 workload 累积**:与 §5 常驻 fuzz 同款,常驻性能监控入 `results/p5-perf-timeline.md`。
...
承 [P4 09 §1](../p4-method-jit/08-testing-strategy.md#91-多-build-tag) 命名(`wangshu_p4`):
...
**为什么 `wangshu_p5` 依赖 `wangshu_p4`**:承 [00-overview §3](./00-overview.md) fullmoon 叠在 gibbous 之上——
...
| PR: race-non-p5 | wangshu_p5 + `-race` | ubuntu-latest amd64 | 5 min(P5 fuzz 自动 skip,承 §1.4)|
...
- **差分覆盖度量**:承 [P1 12 §11](../p1-interpreter/12-testing-difftest.md) 缺口——「如何度量 fuzz 的
  覆盖度」在 P5 更急迫(组合空间超大)。可能路径:coverage-guided fuzz(go-fuzz native 已支持)+ per-op
  的 trace 内联次数计数入 test summary。留 PT7 校准;
- **崩溃 corpus 的跨版本归属**:P5 seed corpus 撞出的 seed 若因 IR 变更失效(某 IR op 被删),归档策略:
...
- **anchor 表并发正确性的独立验证**:§5 `-race` skip 让 mmap 段跑不了 race detector,但 anchor 表 Go 端
  的并发操作(多 State 同时录制 / evict)应能在 race 下验——需要单独一个 `TestAnchorTableRace` 只操作
  Go 侧(不进 mmap 段),PT5 落地;
- **T-item 与 PT 里程碑映射**:本文只声明类目,具体 T-N × PT-N 表在 [09](./09-acceptance-checklist.md)。
  09 落地时若 T-item 数量差距较大(如 T1..T20),需要重新审视本文类目切分粒度是否合适;
- **fullmoon 与 P4 fuzz 的相互干扰**:同一 CI job 里若 force-all P4 + force-all P5 同时开,会不会出现
  「P4 段被跑热了但 P5 anchor 检测点覆盖不到」类交互——PT7 端到端阶段实测确认;
- **首个宿主 workload 的 T-PERF 具体数字**:与 [01-launch-judgment §3](./01-launch-judgment.md) 判定口径预登记一致,数字
  待 P4 验收时预登记,PT9 端到端时兑现;
- **`SetGuardForceFailFirst` 与 P4 未实现的等价路径**:P4 侧走 `internal/crescent/gibbous_pj5_self_e2e_test.go` 等 e2e 覆盖(承 [P4 08 §5.2 addendum](../p4-method-jit/08-testing-strategy.md))——P5 若也想走这
...
[04-optimization-passes.md](./04-optimization-passes.md)(pass 开关契约,承 §4) ·
[05-register-allocation.md](./05-register-allocation.md)(regalloc verifier,承 §6.3) ·
[06-snapshot-deopt.md](./06-snapshot-deopt.md)(deopt 注入 hook = PT5 必交付,承 §3.5) ·
...
线) ·
[../../../llmdoc/guides/prove-the-path-under-test.md](../../../llmdoc/guides/prove-the-path-under-test.md)
(P5 是该 guide 最大新用户 —— §2.3 IC 预热 / §3.5 PT5 deopt hook / §5.4 corpus 回流) ·
