...
所以单一的 `gofuzztime` 要么在 p1 上浪费余量、要么在 p4 上把链拉过上限 —— 这与 `native_fuzz_cap`
早就按层取值的理由是同一个(每层的 target 数与尾巴长度都不同)。现在三层各取一个值,
`workflow_dispatch` 的 `gofuzztime` 输入仍然覆盖全部三层(手动跑时以输入为准)。
**每层的总 fuzz 分钟数是上升的**,不是下降 —— 新步骤加的是覆盖面,不是从既有步骤挪走时间。
准确数值以 `.github/workflows/nightly-diff-fuzz.yml` 的 matrix 为准(`fuzztime` /
`native_fuzz_cap` / `tiered_fuzz_cap` 三个键),不在本文写死:这类会随 target 数变化的量写进文档
就会变成陈旧计数,而**「per X」类配置的 X 不会自我核验**(2026-08-19 那次「per target(4 targets)」
实际是 6 个,就是这么来的)。
