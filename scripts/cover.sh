...
#     are external test packages with no statements of their own. Plain
#     -cover credits them with "[no statements]" and DISCARDS every
...
#     test/ since the root-test migration. The root package itself rides
#     along so any unit test it grows later is instrumented the same way.
#
...
# would then silently never run them (review finding, run layout-r1).
#
...
# only the root package, so no block position appears in both files;
# within the second half every test package writes its own copy of the
# root blocks, which `go tool cover` merges by summing counts (the normal
# semantics of a multi-package -coverpkg profile).
set -euo pipefail
...
# Mirror -tags / -tags=... from the go test flags into go list.
tags=""
prev=""
for arg in "$@"; do
    case "$arg" in
