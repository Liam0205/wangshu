...
1. **官方测试套（按实际覆盖率读，不是「全套通过」）**：`test/luasuite/testdata/` 里的文件与上游 `lua5.1-tests` **逐字节相同**，跑到的部分**逐字节一致**——但**套件里的文件数少于上游，而且多数文件在某一行被截断，按行号算只跑了约一半，只有少数文件从头跑到尾**（`vararg` / `sort` / `pm` 是其中三个；最极端的 `events.lua` 只跑 3 行，因为它第 5 行就用 `setfenv`）。截断都是**刻意不实现的功能**（`setfenv`/`getfenv`、`debug` 高级面、io 对象模型、`string.dump`、`require`、真正的增量 GC），逐条登记在 `test/luasuite/luasuite_test.go` 的 `stopAt` 表与豁免清单里，不是隐藏的语义分歧。**先前这里写「官方测试套 byte-equal：13 个官方文件」是不够的**：它不带覆盖率，读起来像全套跑过了。还有一格更细：**「在套件里」不等于「在跑」**——有文件开头就是 `if T == nil then ... return end`（`T` 是官方发行里的 testC 调试库，望舒不提供），整文件零断言；所以按行号算的占比也不能当执行证据，要用执行侧的量（实际执行的断言次数）核。准确数字读 `stopAt` 表；口径与判据见 [12](docs/design/p1-interpreter/12-testing-difftest.md) §2.1a。（缺席的上游文件各有明确原因，不是没接：`code.lua`/`checktable.lua` 需要官方 `T` testC C 侧调试外挂；`verybig.lua` 需要 io 对象模型豁免里的四个函数；`api.lua` 同理。`db.lua` 是补齐 `lastlinedefined`/`activelines` 之后接进来的。）
...
6. **分层直接对内嵌 oracle 的差分 fuzz**（2026-08-29，`FuzzOracleDiffTiered`）：第 5 层那个 `FuzzOracleDiff` 测的是 **P1 对官方**。在这一层之前，**P3/P4 对官方的一致性全部是传递来的** —— `FuzzAutoPromote` / `FuzzP4ForceAllPromote` 断言的是「分层结果 == P1 结果」，「分层 == 官方」是经 「P1 == 官方」推出来的。传递性本身没错，但它**继承 P1 那次比较的每一个盲区**，而且对「P1 与官方一致、而两个 tier 各自也一致地错」这类情况没有直接观察能力；第 3 层那条按 tier 对真 `lua5.1` 跑的路走的是手写 generator（`vararg` / `goto` / `pcall` / `error` 的生成计数都是 0），覆盖的写法远窄于 go-fuzz 变异。这一层补的就是那条直接轴：任意变异源码在**强制升层**的 State 上跑，与内嵌 5.1.5 比对，**跳过集合与 P1 那一层完全相同**（不为 tier 开新的跳过——「tier 与官方分歧而 P1 不分歧」正是它存在的理由）。**升层是断言出来的而不是假设的**：`FuzzOracleDiff` 本来就能在 tier 的 build tag 下编译并通过（它构造普通 State、从不升层，tier 代码被链接进来但从未进入），所以一个忘记升层的分层 harness 与一个通过的测试在输出上完全一样、连一条 skip 都不留；配套守护测试断言升层计数增长并做过变异实测，CI 另加一条 required-target 存在性断言防 build tag 打错导致目标静默消失。**首轮结果**：两个 tier 都观察到升层，短时引导式 fuzz 未发现分歧 —— 这是第一个结果，不是一张清白证明。口径见 [12](docs/design/p1-interpreter/12-testing-difftest.md) §3.8a、[P3 08](docs/design/p3-wasm-tier/08-testing-strategy.md) §2.5a、[P4 08](docs/design/p4-method-jit/08-testing-strategy.md) §3.4a。
...
**2026-08-27 变动（索引表达式跨行时报错行号钉错，两处独立错误各修一处）**：#248 是 nightly 开出的 `FuzzOracleDiff` crasher，seed 带协程但协程是噪声——剥掉之后关键写法是 `A\n.x` 这种索引运算符与对象/键隔着换行的表达式，lua5.1 把错误归到**索引运算符所在行**（`.x` 所在行，那条 GETTABLE 出错的行），望舒原先归到表达式起始行。`internal/frontend/compile` 里是**两处独立错误**：① `exprIndex` 把对象交给 `exp2AnyReg` 时传了运算符的行（该传对象自己的行，改用 `e.Obj.Pos()`），使延迟加载的 GETGLOBAL 被盖上运算符的行；② `EIndexed` 这个 `expDesc` 不带行号，它的 GETTABLE 取的是「之后由谁 discharge 就取谁的行」（对 `local v = A\n.x` 就是 `LocalStmt` 所在行），加了 `opLine` 字段后改用运算符自己的行（**这个口径后来被 #252 推翻，见下一条**）。修前 dump 出的 `LineInfo` 两条指令行号都不对且方向相反，任一处单独修复仍然错。仓库里早就有 `TestIndexExprLineIsOperatorLine`，而它在两个 bug 存在期间一直是绿的——它用局部变量做对象，局部变量已经在寄存器里，`exprIndex` 不发射任何指令、GETTABLE 立即 discharge，两处错误在这条路径上都不可能出现；新测试改用全局（触发延迟加载）并核对整张 `LineInfo` 表。两条判据：**位置/行号类测试的载体必须触发延迟加载**，判据是「这个写法会让编译器发射我关心的那条指令吗」，不发射就测不到（[[prove-the-path-under-test]] §2.1）；**一处最终症状可能由两处独立错误叠加**，改完先核对整张中间数据结构，不能只看错误消息变没变（[[cross-backend-semantic-fix-sweep]]「一个症状可能是同一子系统内两处独立错误叠加」节）。落点 [09](docs/design/p1-interpreter/09-errors-pcall.md) §3.5.2、[04](docs/design/p1-interpreter/04-frontend-parser-codegen.md) §5.2.1。过程反思见 `llmdoc/memory/reflections/2026-08-27-issue248-index-line-across-newline.md`。

**2026-09-03 变动（同一个行号缺陷的第四格：GETTABLE 记的是 discharge 点的 lastline，不是索引运算符的行）**：#252 又是 nightly 开出的 `FuzzOracleDiff`，seed `(A.A` 换行 `)()` 经协程把错误打出来（issue 标题写「crash」但**不是崩溃**，两边都正确抛 `attempt to index global 'A'`，只有错误消息里的**行号**不同：PUC `:2:` / 望舒 `:1:`）。**上一条（#248）定下的「运算符所在行」口径是错的**——读 PUC 的 `lcode.c` 才看清：`luaK_codeABC`/`luaK_codeABx` **根本不接收行号参数**，一律用发射那一刻的 `fs->ls->lastline`，所以一条延迟发射的 GETTABLE 记的是**谁在哪一行 discharge 它**；「运算符的行」只是「索引写完就地被消费」时的近似。把这个近似排除掉的判别输入**完全没有括号**：`local v = A.x` 换行换行 `+1` 里运算符在第 1 行，而 `luac5.1` 把 GETTABLE 记在第 **3** 行（`+` 才是 discharge 点）——所以这跟括号无关，括号只是推迟 discharge 的一种方式。修法是删掉 `expDesc.opLine`、改由调用方传 lastline 语义的行，这需要给 AST 补结束位置（正是 #248 记下过、判定为「超出当时范围」而撤回的那个缺口；当时判断它不可见的依据是 SETGLOBAL/SETUPVAL/MOVE 不会 raise——没错，但 GETTABLE 会）。结构性事实：**表达式列表的每个元素在它后面那个分隔符被消费后各自 discharge**，`f(A.A` 换行 `,1` 换行 `)` 里两个参数分别落第 2、第 3 行，所以按元素记行、末元素交给调用方按闭合它的 token 填（泛型 for 必须在 `check_match(DO)` **之前**取，否则被 `do`/`end` 推过头）。**同一个缺陷一共躺在四个语法位置**：① 括号与调用参数（报上来的那个）② 赋值右侧 + `return` + 泛型 for（接线时给这三处传了 `nil`，理由是「不在报告的形状里」——实测三处**全都**偏，靠自查清单查出）③ 单目标赋值快速路径（`storeVar` 用**同一个** `line` 参数同时干「物化 RHS」和「发射 store」两件事，靠 PR #253 的远端评审查出）。四条判据：**自己从参照实现行为归纳出的口径，与别人对源码的陈述一样需要回到源码验证**（能读源码就去读，`luaK_codeABC` 五行就给出答案）；**用例要能区分两个候选模型**——为单目标赋值那一格写的 `x = A.x` 换行 在两个模型下期望值相同、零区分力，难点不在知道要构造判别输入而在**每加一格都重新问一次**；**给共用 helper 加「缺失时回退旧行为」的可选参数时，每个仍传缺省值的调用点都要实测**，因为回退默认值让「漏接线」与「不需要接线」在测试结果上完全同形；**同族旧语料要和新语料一起当验收门**——第一版修法把 #248 自己的常驻语料改红了，而 `test-all`/`conformance-all`/`difftest-all` 全绿，只有 `make fuzz-oracle`（重放 `testdata/fuzz/`）逮到。仍未对齐但**不可见**的残留（都是不会 raise 的指令，已登记 doc-gaps）：`storeVar` 的 store 行、`f{...}` 糖式调用的单参数物化行、数值 for 的 FORPREP/LOADK 行。落点 [04](docs/design/p1-interpreter/04-frontend-parser-codegen.md) §5.2.2、[09](docs/design/p1-interpreter/09-errors-pcall.md) §3.5.2 订正块。过程反思见 `llmdoc/memory/reflections/2026-09-03-issue252-discharge-line-is-lastline.md`。
