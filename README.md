...
| realworld small [^cat-realworld] | fib | 9.44 ms | 10.0 ms (0.94×) | 10.9 ms (0.87×) [^p3-gate] | 25.1 ms (0.38×) | 0.90 ms (**10.5×**) [^seg2seg] | 0.90 ms (**10.5×**) [^seg2seg] |
| | binary-trees | 33.7 ms | 36.8 ms (0.92×) | 38.4 ms (0.88×) [^p3-gate] | 103.1 ms (0.33×) | 39.4 ms (0.86×) | 38.9 ms (0.87×) |
| | spectral-norm | 22.6 ms | 18.6 ms (1.22×) | 20.7 ms (1.09×) [^p3-gate] | 47.3 ms (0.48×) | 18.1 ms (1.25×) | 17.7 ms (1.28×) |
| | fannkuch | 4.20 ms | 5.65 ms (0.74×) | 5.71 ms (0.74×) | 5.74 ms (0.73×) | 0.60 ms (**7.00×**) | 0.60 ms (**7.04×**) |
| | n-body | 51.9 ms | 45.4 ms (1.14×) | 43.2 ms (1.20×) [^p3-gate] | 87.0 ms (0.60×) | 44.2 ms (1.17×) | 44.3 ms (1.17×) |
...
[^seg2seg]: P4 段到段 CALL 直跳（issue #50，2026-07-04，仅 amd64）：自递归 / arith-callee（fib 形态）之前每次调用付一次跨界往返税（mmap RET → Go dispatch → host.CallBaseline → mmap 重入),现在 caller 段直接 `call` 进 callee 段、callee 段内组拆帧 + native 递归、全程不出 mmap。fib 从 0.87× 翻到 10.5×（同机同口径,`-benchtime=2s -count=3` median）。同机复测 fannkuch 6.9×、binary-trees 1.35×（binary-trees 是分配 / GC 密集,其递归 callee 含表 op 不进段到段,收益主要来自其余路径）。arm64 端镜像实现已写完并交叉编译通过(linux/arm64 + darwin/arm64),但开发机是 amd64 无 qemu-aarch64,arm64 机器码的真机 `-race` / difftest / benchmark 验收经 CI 三平台矩阵进行,追踪于 issue #61。
